...
/* **************************************************************
 * Setup a sparse real matrix structure. NR, and NC are already
 * set. Now that we know the number of non-zeros, get the storage.
 * ************************************************************** */

MSR *
msr_Setup (MSR * m, int nnz)
{
  m->nnz = nnz;
  ...
  if (nnz)
  {
    ...
    if (!m->d)
      m->d = (double *) GC_MAIOP ((m->nnz) * sizeof (double));
  }
  return (m);
}
...
msr_Copy (MSR * m)
{
  ...
  if (m->d)
  {
    new->d = (double *) GC_MAIOP (sizeof (double) * (m->nnz));
    memcpy (new->d, m->d, m->nnz * sizeof (double));
  }
...
msr_ReSparse (MSR * m)
{
  ...
  s = msr_Create (m->nr, m->nc);
  s->nnz = nnz;
  s->ia = newia;
  s->ja = (int *) GC_MAIOP (nnz * sizeof (int));
  s->d = (double *) GC_MAIOP (nnz * sizeof (double));
...
/* **************************************************************
 * Print out a sparse-real matrix.
 * ************************************************************** */

void
msr_Print (MSR * m, FILE * stream)
{
  int i, ia, j, n, row;
  int fwidth, fprec;
  ...
  for (i = 0; i < m->nr; i++)
  {
    ...
    for (j = 0; j < n; j++)
    {
      fprintf (stream, " (%i, %i)    \t%*.*g\n", row, m->ja[ia + j - 1],
	       fwidth, fprec, m->d[ia + j - 1]);
    }
    row++;
  }
...
/* **************************************************************
 * Write a sparse-real matrix in ASCII format to a file.
 * 1st column:    row IDs
 * 2nd column:    column IDs
 * 3rd column:    element value
 * ************************************************************** */

void
msr_WriteGeneric (MSR * m, FILE * fn)
{
  int i, ia, j, n, row;
  int fwidth, fprec;
  ...
  for (i = 0; i < m->nr; i++)
  {
    ...
    for (j = 0; j < n; j++)
    {
      ...
      if (strlen(MdsV0(fmt,0))>1)
        fprintf (fn, MdsV0(fmt,0), m->d[ia + j - 1]);
      else
        fprintf (fn,"%*.*f", fwidth, fprec, m->d[ia + j - 1]);
...
msr_Class (MSR * m)
{
  return (cpstr (RLAB_MEMBER_CLASS_NUM));
}
...
void *
msr_MemberRef (MSR * m, char *name, int *type)
{
  Ent *ne;
  void *rptr;
  ...
  {
    ne = ent_Create ();
    ent_data (ne) = mdr_CreateScalar (((double) m->nr) * ((double) m->nc));
    ent_SetType (ne, MATRIX_DENSE_REAL);
    rptr = (void *) ne;
    *type = ENTITY;
  }
...
msr_GetEl (MSR * m, int i, int j)
{
  ...
  if (i < 1 || j < 1)
  {
    ...
    rerror ("Internal runtime error (msr_GetEl): indices > 0 required !");
  }
...
msr_MatrixAssign (MSR * left, int *row, int *col, MSR * rhs)
{
  ...
  if (result->nnz)
  {
	...
	if (right->ia)
	{
	  /* Fill in right_cmap for this row. */
	...
	{
	  /* Assign into this lhs row. */
	  for (j = 0; j < jc; j++)
	  {
	    c = icol[j];
	    /* Pick up the original values. */
	    ...
	    {
	      result->ja[n] = c;
	      result->d[n] = right->d[right_cmap[cmap[c]] - 1];
	      n++;
	    }
	  }
    ...
    d_tmp = (double *) GC_MAIOP (n * sizeof (double));
    memcpy (d_tmp, result->d, n * sizeof (double));
    GC_FREE (result->d);
    result->d = d_tmp;
  }
...
MSR *
msr_VectorAssign (MSR * var, int *j, MSR * rhs)
{
  int k, *row, *col;
  MSR *lhs;
  ...
  /* Finally, just do it. */
  lhs = msr_MatrixAssign (var, row, col, rhs);
...
MSR *
msr_mdr_MatrixAssign (MSR * mlhs, int *irow, int *jcol, MDR * rhs)
{
  MSR *mtmp, *result;

  mtmp = msr_Sparse (rhs);
  result = msr_MatrixAssign (mlhs, irow, jcol, mtmp);
  msr_Destroy (mtmp);
  return (result);
}

MSR *
msr_mdr_VectorAssign (MSR * mlhs, int *j, MDR * rhs)
{
  MSR *mtmp, *result;

  mtmp = msr_Sparse (rhs);
  result = msr_VectorAssign (mlhs, j, mtmp);
  msr_Destroy (mtmp);
  return (result);
}

/* **************************************************************
 * Transpose a sparse real matri.
 * ************************************************************** */
...
msr_Transpose (MSR * m)
{
  ...
  if (m->nnz)
  {
    ...
    DGSTRN (m->ia, m->ja, m->d, &m->nr, &m->nc, new->ia, new->ja, new->d);
  }
  return (new);
}
...
MSR *
msr_RowPartition (MSR * m, int *row)
{
  int i, k, kk, n, nr, nnz;
  MSR *new;
  ...
  if (nnz)
  {
    ...
    new->d = (double *) GC_MAIOP (nnz * sizeof (double));
	...
	memcpy (new->d + k, m->d + kk, n * sizeof (double));
      }
      k += n;
    }
    new->ia[i - 1] = k + 1;
...
void *
msr_MatrixSub (MSR * m, int *row, int *col, int *type)
{
  MSR *new, *mtmp1, *mtmp2, *mtmp3;
  MDR *dnew;
  ...
  /* Handle requests for scalar values. */

  if ((row[0] == 1) && (col[0] == 1))
  {
    dnew = mdr_CreateScalar (msr_GetEl (m, row[1], col[1]));
  ...
  mtmp1 = msr_RowPartition (m, row);

  /* Now transpose. */

  mtmp2 = msr_Transpose (mtmp1);
  msr_Destroy (mtmp1);

  /* Now select columns (as rows). */
  mtmp3 = msr_RowPartition (mtmp2, col);
  msr_Destroy (mtmp2);

  /* Now transpose back. */
  new = msr_Transpose (mtmp3);
  msr_Destroy (mtmp3);
...
/* **************************************************************
 * Select rows from a sparse-real matrix.
 * ************************************************************** */

void *
msr_MatrixSubR (MSR * m, int *row, int *type)
{
  MSR *new;
  MDR *dnew;
  ...
  if ((row[0] == 1) && (m->nc == 1))
  {
    dnew = mdr_CreateScalar (msr_GetEl (m, row[1], 1));
    *type = MATRIX_DENSE_REAL;
    return (dnew);
  }

  new = msr_RowPartition (m, row);
  *type = MATRIX_SPARSE_REAL;
  return (new);
}

/* **************************************************************
 * Select columns from a sparse-real matrix.
 * ************************************************************** */

void *
msr_MatrixSubC (MSR * m, int *col, int *type)
{
  MSR *new, *mtmp1, *mtmp2;
  MDR *dnew;
  ...
  /* Handle the scalar condition. */

  if ((col[0] == 1) && (m->nr == 1))
  {
    dnew = mdr_CreateScalar (msr_GetEl (m, 1, col[1]));
    *type = MATRIX_DENSE_REAL;
    return (dnew);
  }

  /* Transpose. */
  mtmp1 = msr_Transpose (m);

  /* Now select columns (as rows). */
  mtmp2 = msr_RowPartition (mtmp1, col);
  msr_Destroy (mtmp1);

  /* Now transpose back. */
  new = msr_Transpose (mtmp2);
  msr_Destroy (mtmp2);
...
void *
msr_VectorSub (MSR * m, int *i, int *type)
{
  ...
  if (size == 1)
  {
    ...
    dnew = mdr_CreateScalar (msr_GetEl (m, rown, coln));
    *type = MATRIX_DENSE_REAL;
  ...
  new = msr_Create (1, size);	/* Most common. */
  msr_Setup (new, size);
...
msr_Stack (MSR * m1, MSR * m2)
{
  ...
  if (nnz > 0)
  {
    ...
    memcpy (new->d, m1->d, (m1->nnz) * sizeof (double));
    ...
    memcpy (new->d + m1->nnz, m2->d, (m2->nnz) * sizeof (double));
  }
...
/* **************************************************************
 * Append two sparse-real matrices together:  [ m1 , m2 ]
 * ************************************************************** */

MSR *
msr_Append (MSR * m1, MSR * m2)
{
  MSR *new, *mtmp1, *mtmp2, *mtmp3;
  ...
  mtmp1 = msr_Transpose (m1);
  mtmp2 = msr_Transpose (m2);
  mtmp3 = msr_Stack (mtmp1, mtmp2);
  msr_Destroy (mtmp1);
  msr_Destroy (mtmp2);
...
msr_Check (MSR * m)
{
  ...
  if ((m->nr * m->nc) < m->nnz)
  {
    ...
    fprintf (stderr, "sparse-matrix-handling: m->nr=%i, m->nc=%i, m->nnz=%i\n",
	     m->nr, m->nc, m->nnz);
    rerror ("terrible error in sparse matrix check (msr_Check)");
  }

  if (m->nnz)
  {
    /*
     * Check the column indices to make sure they
     * are valid.
     */
	...
	rerror ("Internal runtime error (msr_Check)");
      }
    }

    /*
     * Check the row/column pointers for validity.
     */
...
msr_Eq (MSR * m1, MSR * m2)
{
  ...
  else
  {
    ...
    size = m1->nr * m1->nc;
    tmp = msr_Create (m1->nr, m1->nc);
    msr_PatternUnion (m1, m2, &(tmp->ia), &(tmp->ja), &(tmp->nnz));
    ...
    for (i = 0; i < tmp->nr; i++)
    {
      ...
      for (j = 0; j < n; j++)
      {
        Mdr1 (dtmp, i + 1, tmp->ja[ia + j - 1]) =
            (double) (msr_GetEl (m1, i + 1, tmp->ja[ia + j - 1]) ==
                  msr_GetEl (m2, i + 1, tmp->ja[ia + j - 1]));
      }
    }
    new = msr_Sparse (dtmp);
    mdr_Destroy (dtmp);
...
//       size = m2->nr * m2->nc;
	...
	for (j = 0; j < n; j++)
	{
	  Mdr1 (dtmp, i + 1, m2->ja[ia + j - 1]) =
	    (double) (msr_GetEl (m2, i + 1, m2->ja[ia + j - 1]) != m1->d[0]);
	}
      }
      new = msr_Sparse (dtmp);
      mdr_Destroy (dtmp);
...
//       size = m1->nr * m1->nc;
	...
	for (j = 0; j < n; j++)
	{
	  Mdr1 (dtmp, i + 1, m1->ja[ia + j - 1]) =
	    (double) (msr_GetEl (m1, i + 1, m1->ja[ia + j - 1]) != m2->d[0]);
	}
      }
      new = msr_Sparse (dtmp);
      mdr_Destroy (dtmp);
...
//     size = m1->nr * m1->nc;
    ...
    for (i = 0; i < tmp->nr; i++)
    {
      ...
      {
	tmp->d[ia + j - 1] = (double)
	  (msr_GetEl (m1, i + 1, tmp->ja[ia + j - 1]) !=
	   msr_GetEl (m2, i + 1, tmp->ja[ia + j - 1]));
      }
    }
    new = msr_ReSparse (tmp);
...
void
msr_PatternUnion (MSR * m1, MSR * m2, int **IA, int **JA, int *nnz)
{
