c       Block Data
c
c *  This Block Data Routine Initializes Nminit (The Initial Number
c *  Of Mesh Points) And Uval0 (The Initial Value For The Trial
c *  Solution) To Their Default Values.
c
c       Double Precision Uval0
c       Common/Algprs/ Nminit, Iprint, Maxcon, Itsaim, Uval0
c
c       Data Nminit/7/
c       Data Iprint/0/
c       Data Itsaim/7/
...
C.... *****************************************************************
     ...
     +            Eps, Epsmin, ACFsub, ACDfsub, ACGsub, ACDgsub, Iflbvp,
     +            odeout, lodeout)

C       Modification of the original ACDC routine
C       1. Added two input variables, odeout and lodeout, which provide the
C       name of the output stream, and its length, to which all the runtime
C       messages are dumped.
C       2. Control block: if lodeout is 0 then the messaging level is set
C       to no messages at all.
C       by Marijan Kostrun, Project rlabplus, (C) 2005
...
C     End of it. KMK
      ...
      External ACFsub, ACDfsub, ACGsub, ACDgsub
...
C     Messages control block
     ...
     &      ' "acdc" for boundary value problem.'
...
C.... Invalid, Exit With The Flag Iflbvp Set To -1.
      ...
      Iflbvp = -1
      If (Ncomp .le. 0)  Goto 10000
      If (Nlbc .lt. 0 .or. Nlbc .gt. Ncomp) Goto 10000
      If (Nucol .le. 0) Goto 10000
      If (Aleft .ge. Aright) Goto 10000

      If (Nfxpnt .lt. 0)  Goto 10000
      If (Maxcon .lt. 0)  Goto 10000
      If (.not. Givmsh) Nmsh = 0
      If (Givmsh .and. Nmsh .lt. Nfxpnt+2) Goto 10000
      If (Givmsh .and. Xx(1) .ne. Aleft) Goto 10000
      If (Givmsh .and. Xx(Nmsh) .ne. Aright) Goto 10000
      If (Nfxpnt .gt. 0) Then
         If (Fixpnt(1) .le. Aleft) Goto 10000
         If (Fixpnt(Nfxpnt) .ge. Aright) Goto 10000
         Do 10 I = 1, Nfxpnt-1
            If (Fixpnt(I+1) .le. Fixpnt(I)) Goto 10000
   10    Continue
      Endif

      If (Ntol .lt. 1) Goto 10000
      Do 20 I = 1, Ntol
         If (Ltol(I) .lt. 0 .or. Ltol(I) .gt. Ncomp) Goto 10000
         If (Tol(I) .le. Zero) Goto 10000
   20 Continue

      If (Giveu .and. .not. Givmsh) Goto 10000

      If (Nudim .le. 0) Goto 10000
      If (Lwrkfl .le. 0 .or. Lwrkin .le. 0) Goto 10000
      If (Itsaim .lt. 1) Goto 10000
      If (.not. Giveps) Eps = 0.5d0
      If (Eps .le. Zero .or. Eps .ge. One) Goto 10000
      If (Epsmin .gt. Eps .or. Epsmin .le. Zero) Goto 10000
...
C.... Attempt To Solve The Latest Continuation Problem.
     ...
     *   ACFsub, ACDfsub, ACGsub, ACDgsub, Iflbvp, Eps)
...
C     Block to close the fortran stderr file
10000 If(Iprint .gt. 0)
     *   Close (6)
C     End of it. KMK
...
* File Bvps.f
     ...
     *   Voldmsh, ACFsub, ACDfsub, ACGsub, ACDgsub, Iflbvp, Eps)
...
*  Equations.
     ...
     *    ACFsub, ACDfsub, ACGsub, ACDgsub, Iflnwt, Eps)
...
*  Fval Is Called Within Newteq For The New U.)
     ...
     *        Ajac, Topblk, Botblk, Bhold, Chold, Ipvblk,
     *        ACFsub, ACDfsub, ACGsub, ACDgsub, Itnwt, Iflnwt, Eps)
...
*  At The Given U.
     ...
     *          Uint, Ftmp, Dftmp1, Dftmp2, Dgtm, Tmprhs, Xmerit,
     *          Ajac, Topblk, Botblk, Bhold, Chold, Ipvblk,
     *          ACFsub, ACDfsub, ACGsub, ACDgsub, Iter, Iflnwt, Eps)
        Endif
      Endif
...
*  Dimension Ncomp.
 ...
 20      Continue
         ...
         Call ACFsub (Ncomp,Xxc1,Tmp(1,1),Tmp(1,5), Eps)
         Call ACFsub (Ncomp,Xxc2,Tmp(1,2),Tmp(1,6), Eps)

         Call ACDfsub (Ncomp,Xxc1,Tmp(1,1),Df, Eps)
         Do 30 I = 1, Ncomp
 ...
 30      Continue
         ...
         Call ACDfsub(Ncomp,Xxc2,Tmp(1,2),Df, Eps)
         Do 35 I = 1, Ncomp
            Do 32 J = 1, Ncomp
               Dfij = Hmsh*Df(I,J)
...
*   The Array Tmp Is Workspace For 9 Intermediate Vectors.
 ...
 20         Continue
            ...
            Call ACFsub (Ncomp,Xxc1,Tmp(1,1),Tmp(1,7), Eps)
            Call ACFsub (Ncomp,Xxc2,Tmp(1,2),Tmp(1,8), Eps)
            Call ACFsub (Ncomp,Xxc3,Tmp(1,3),Tmp(1,9), Eps)
 ...
 40         Continue
            ...
            Call ACDfsub(Ncomp,Xxc3,Tmp(1,3),Df, Eps)
            Do 50 I = 1, Ncomp
                 Do 45 J =  1, Ncomp
                    Dfij = Hmsh*Df(I,J)
...
*  Blas: Dcopy, Dssq
      ...
      Parameter ( Xlarge = 1.0d+6, Huge = 1.0d+30, Lmtfrz = 16)
...
*  Two-Norm At The Trial Point.
     ...
     *   ACFsub, ACGsub, Rhstri, Rnsq, Fval, Ftmp, Uint, Eps)
...
*  Been Passed.
  ...
  905 Format(1h ,'Fixed Jacobian Convergence',I5,1pe11.3)
     ...
     *    ACFsub, ACDfsub, ACGsub, ACDgsub, Iflag, Eps)
...
*  Compute The Right-Hand Side Vector Rhs.
     ...
     *          ACFsub, ACGsub, Rhs, Rnsq, Fval, Ftmp, Uint, Eps)
...
*  New U Array Is Obtained By Adding Delu To U.
  ...
  901 Format(1h ,'Singular Matrix')
     ...
     *    Uint, Ftmp, Dftmp1, Dftmp2, Dgtm, Tmprhs, Xmerit,
     *    Ajac, Topblk, Botblk, Bhold, Chold, Ipivot,
     *    ACFsub, ACDfsub, ACGsub, ACDgsub, Iter, Iflag, Eps)
...
*  Only When An Improved Point Is Found.
         ...
         Call Fneval(Ncomp, Nmsh, Xx, Ncomp, Utrial, Fval, Eps)
         Call Rhscal (Ncomp, Nmsh, Nlbc, Xx, Ncomp, Utrial, Defcor,
     *      ACFsub, ACGsub, Rhstri, Rnsqtr, Fval, Ftmp, Uint, Eps)
...
*  In The Array Fval.
      ...
      Call ACFsub (Ncomp, Xx(1), U(1,1), Fval(1,1), Eps)
      Do 50 Im = 1, Nmsh-1
         Call ACFsub (Ncomp, Xx(Im+1), U(1,Im+1), Fval(1,Im+1), Eps)
   50 Continue
      Return
      End
...
*  Blas: Dcopy, Ddot
      ...
      Do 110 I = 1, Nlbc
         Call ACDgsub (I, Ncomp, U(1,1), Dgtm, Eps)
         Call Dcopy(Ncomp, Dgtm(1), 1, Topblk(I,1), Ncomp)
  110 Continue

      Call ACDfsub (Ncomp, Xx(1), U(1,1), Dftm1(1,1), Eps)
...
*  Arrays.
  ...
  120    Continue
         ...
         Call ACDfsub (Ncomp, Xhalf, Uint, Dftm2(1,1), Eps)
         Do 140 Ic = 1, Ncomp
            Do 130 Jc = 1, Ncomp
  ...
  140    Continue
         ...
         Call ACDfsub (Ncomp, Xx(Im+1), U(1,Im+1), Dftm1(1,1), Eps)
         Do 170 Ic = 1, Ncomp
            Do 160 Jc = 1, Ncomp
               Dsq = Ddot(Ncomp, Dftm2(Ic,1), Ncomp,
     *                         Dftm1(1,Jc), 1)
  ...
  200 Continue
      Do 220 I = Nlbc+1, Ncomp
         Call ACDgsub (I, Ncomp, U(1, Nmsh), Dgtm, Eps)
...
*  First, Process The Left-Hand Boundary Conditions.
      ...
      Do 20 I = 1, Nlbc
         Call ACGsub (I, Ncomp, U(1,1), Wg, Eps)
         Rhs(I) = -Wg
   20 Continue
...
*  Contains The Function Values From ACFsub At Xx And U.
   ...
   30    Continue
         ...
         Call ACFsub (Ncomp, Xhalf, Uint, Ftmp, Eps)
         Loc = (Im-1)*Ncomp + Nlbc
         Do 40 Ic = 1, Ncomp
   ...
   50 Continue
      ...
      Do 60 Ii = Nlbc+1, Ncomp
         Call ACGsub (Ii, Ncomp, U(1,Nmsh), Wg, Eps)
         Rhs(Nrhs+Ii) = -Wg
   60 Continue
