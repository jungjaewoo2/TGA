...
C---------------------------------------------------------
      ...
      CALL BIMD0(M,FCN,JAC,NMETH,STEP_ORD(indord),Y0,WORK(INDF0),
...
C------------------------------------------------------------------------------------------------------
      ...
      SUBROUTINE BIMD0(M,FCN,JAC,NMETH,KMAX,Y0,F0,T0,TEND,
     &                 H,RTOL,ATOL,ITOL,
...
C -------------------------------------------------------
      ...
      IF (IMAS.NE.0) THEN
C
C         kmk was here: mas is a matrix passed to the code rather than
C         a function. V-2007
C
C         CALL MAS(M,M0,LDMAS,IERR,RPAR,IPAR)
          DO I = 1,LDMAS
            DO J = 1,M
              M0(I,J) = MAS(I,J)
            END DO
...
210   CALL Dblendstep4(M,Y0,F0,Y,F,H,THETA,IPVT,ERR,
     &                GAMMA,LDLU,MLJAC,MUJAC,IJOB,IMAS,
     &                LDMAS,MLMAS,MUMAS,M0,TEMP)
      GOTO 300

220   CALL Dblendstep6(M,Y0,F0,Y,F,H,THETA,IPVT,ERR,
     &                GAMMA,LDLU,MLJAC,MUJAC,IJOB,IMAS,
     &                LDMAS,MLMAS,MUMAS,M0,TEMP)
      GOTO 300

230   CALL Dblendstep8(M,Y0,F0,Y,F,H,THETA,IPVT,ERR,
     &                GAMMA,LDLU,MLJAC,MUJAC,IJOB,IMAS,
     &                LDMAS,MLMAS,MUMAS,M0,TEMP)
      GOTO 300

240   CALL Dblendstep10(M,Y0,F0,Y,F,H,THETA,IPVT,ERR,
     &                 GAMMA,LDLU,MLJAC,MUJAC,IJOB,IMAS,
     &                 LDMAS,MLMAS,MUMAS,M0,TEMP)
      GOTO 300

250   CALL Dblendstep12(M,Y0,F0,Y,F,H,THETA,IPVT,ERR,
     &                 GAMMA,LDLU,MLJAC,MUJAC,IJOB,IMAS,
     &                 LDMAS,MLMAS,MUMAS,M0,TEMP)
...
310   CALL Dlocalerr4(M,F0,F,H,ERR,SCAL,NERR,NERRUP,NLINSYS,
     &               THETA,VMAX,IPVT,LDLU,MLJAC,MUJAC,IJOB,
     &               IMAS,LDMAS,MLMAS,MUMAS,M0,K,ORD_IND,
     &               INDEX1,INDEX2)
      GOTO 400

320   CALL Dlocalerr(M,F0,F,H,ERR,SCAL,NERR,NERRUP,NLINSYS,
     &              THETA,VMAX,IPVT,LDLU,MLJAC,MUJAC,IJOB,
     &              IMAS,LDMAS,MLMAS,MUMAS,M0,K,ORD_IND,
     &              INDEX1,INDEX2)
...
c     SPECTRAL RADIUS TOO LARGE
      ...
      CALL Derrdown(M,F0,F,H,ERR(1,2),SCAL,NERRDOWN,NLINSYS,
...
C     THE PROBLEM IS A QUADRATURE
        CALL DERRUP(M,K,ORD_IND,ERR,H,H0,H00,VMAX(7),NERRUP,
...
c IN THIS CASE, THE SPECTRAL RADIUS DOESN'T BEHAVES LIKE rho=rhoti/q.
             ...
             CALL DERRUP(M,K,ORD_IND,ERR,H,H0,H00,VMAX(7),NERRUP1,
     &                  SCAL,THETA,IPVT,LDLU,MLJAC,MUJAC,IJOB,
     &                  INDEX1,INDEX2)
             NLINSYS = NLINSYS + 1
