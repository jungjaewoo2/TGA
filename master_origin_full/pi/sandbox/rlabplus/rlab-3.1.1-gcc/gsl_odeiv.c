...
ent_gsl_odeiv (int nargs, Datum args[])
{
  ...
  if (!isfuncent(e2))
  {
    ...
    if (nargs > 4)
    {
      ...
      {
        // h
        node = btree_FindNode (ent_data (eo), RLAB_NAME_ODEIV_STEP);
        if (node != 0)
        {
        ...
        // ay
        node = btree_FindNode (ent_data (eo), RLAB_NAME_ODEIV_AY);
        if (node != 0)
        {
          ddummy = class_double (var_ent (node));
          if (ddummy > 0.0)
        ...
        // adydt
        node = btree_FindNode (ent_data (eo), RLAB_NAME_ODEIV_ADYDT);
        if (node != 0)
        {
          ddummy = class_double (var_ent (node));
          if (ddummy > 0.0)
        ...
        // erel
        node = btree_FindNode (ent_data (eo), RLAB_NAME_ODEIV_EREL);
        if (node != 0)
        {
          ddummy = class_double ( var_ent (node) );
          if (ddummy >= 0.0)
        ...
        // eabs
        node = btree_FindNode (ent_data (eo), RLAB_NAME_ODEIV_EABS);
        if (node != 0)
        {
          ddummy = class_double (var_ent (node));
          if (ddummy > 0.0)
        ...
        node = btree_FindNode (ent_data (eo), RLAB_NAME_ODEIV_DELTA_T);
        if (node != 0)
        {
          ddummy = class_double (var_ent (node));
          if (ddummy > 0.0)
        ...
        // phase_space
        node = btree_FindNode (ent_data (eo), RLAB_NAME_ODEIV_PHASE);
        if (node != 0)
          ips = 1;
        // standard output
  ...
  else
  {
    ...
    if (nargs > 5)
    {
      ...
      {
        // h
        node = btree_FindNode (ent_data (eo), "step");
        if (node != 0)
        {
          ddummy = class_double (var_ent (node));
          if (ddummy > 0.0)
        ...
        // erel
        node = btree_FindNode (ent_data (eo), "erel");
        if (node != 0)
        {
          ddummy = class_double ( var_ent (node) );
          if (ddummy >= 0.0)
        ...
        // eabs
        node = btree_FindNode (ent_data (eo), "eabs");
        if (node != 0)
        {
          ddummy = class_double (var_ent (node));
          if (ddummy > 0.0)
        ...
        // method
        node = btree_FindNode (ent_data (eo), "imethod");
        if (node != 0)
        {
          idummy = (int) class_double (var_ent (node));
        ...
        // delta t for integration that cannot be completed on [t_i,t_i+1]
        // because of the user error
        node = btree_FindNode (ent_data (eo), "delta_t");
        if (node != 0)
        {
          ddummy = class_double (var_ent (node));
          if (ddummy > 0.0)
        ...
        // phase_space
        node = btree_FindNode (ent_data (eo), "phase_space");
        if (node != 0)
          ips = 1;
        // standard output
        node = btree_FindNode (ent_data (eo), "stdout");
        if (node != 0)
        {
          if (ent_type(var_ent (node))== MATRIX_DENSE_STRING)
            outs = class_char_pointer(var_ent (node));
          if (isvalidstring(outs)<1)
            outs = 0;
        }
      }
      ent_Clean (eo);
