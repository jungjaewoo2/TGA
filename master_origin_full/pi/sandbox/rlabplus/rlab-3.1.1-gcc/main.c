...
Ent * Eval (int nargs, Datum args[])
{
  ...
  int i, len = isvalidstring(script);
  if (len < 1)
    goto _exit_eval;

  if (class_scope)
  {
    eval_string = GC_MALLOC((len+3)*sizeof(char));
    for (i=0; i<len; i++)
      eval_string[i] = script[i];
    eval_string[len] = '\n';    // add empty line for the parser
    eval_string[len+1] = 0xff;  // add EOF so that parser exist normally
    eval_string[len+2] = '\0';  // end this string
...
_exit_eval:
  ...
  if (!rent)
    rerror ("eval: Internal Error: Cannot evaluate input!");
...
static char
    *rlab_greeting_text[] = {
        ...
        "RLaB3 and rlabplus(C) 2004-2017 Marijan Kostrun, RLaB and RLaB2 (C) 1992-2001 Ian Searle",
...
static char
    *rlab_greeting_text[] = {
        ...
        "rlabplus(C) 2004-2017 Marijan Kostrun, RLaB and RLaB2 (C) 1992-2001 Ian Searle",
        "Please check http://rlabplus.sourceforge.net for the latest news, & c",
