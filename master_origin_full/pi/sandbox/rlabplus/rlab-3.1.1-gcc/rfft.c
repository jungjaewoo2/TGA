...
Ent * Filter (int nargs, Datum args[])
{
  ...
  int rtype1=UNDEF, rtype2=UNDEF, rtype3=UNDEF, rtype4=UNDEF;
  ...
  if (e1)
    rtype1 = ent_type(e1);
  if ( rtype1!=MATRIX_DENSE_REAL &&  rtype1!=MATRIX_DENSE_COMPLEX )
    rerror (THIS_SOLVER ": First argument 'x' must be dense real or complex vector or matrix!");

  e2 = bltin_get_ent (args[1]);
  if (e2)
    rtype2 = ent_type(e2);
  if ( rtype2!=MATRIX_DENSE_REAL &&  rtype2!=MATRIX_DENSE_COMPLEX )
    rerror (THIS_SOLVER ": Second argument 'b' must be dense real or complex vector!");
      ...
      if (ent_type(e4)==BTREE)
      {
        ...
        // user cannot provide more then one parameter
          if ((y0 != 0) + (zi !=0) + isper > 1)
            rerror (THIS_SOLVER ": Cannot provide more then one option 'zi', or 'y0' or 'peridic'\n");
      }
  }
...
//           fprintf(stderr, "vi[%i] = %g\n", k, vi[k]);
  ...
  else if (rtype1==MATRIX_DENSE_COMPLEX)
    cx = ent_data(e1);
