...
Ent * ent_findroot (int nargs, Datum args[])
{
  ...
  else if (ent_type (e1) == MATRIX_DENSE_REAL)
  {
    ...
    for (i=0; i<(nr-1); i++)
    {
      ...
      {
        fprintf(stderr, THIS_SOLVER ": row %i: %g >= %g\n", i, mdr0(x1,i,0), mdr0(x1,i+1,0));
        rerror ("findroot: first column of [x,y] has to be increasing!");
      }
    }

    MDR *w1 = mdr_Create(1, nz);

    for (i=0; i<(nr-1); i++)
    {
      if ((Mdr0(x1,i,1)-yoff)*(Mdr0(x1,i+1,1)-yoff) <= 0)
      {
        if (Mdr0(x1,i,1)==yoff || Mdr0(x1,i+1,1)==yoff)
        {
          if (Mdr0(x1,i,1)==yoff)
          {
            MdrV0(w1, nnz) = Mdr0(x1,i,0);
            nnz++;
          }
          if (Mdr0(x1,i+1,1)==yoff && i==nr-2)
          {
            MdrV0(w1, nnz) = Mdr0(x1,i+1,0);
            nnz++;
          }
        }
        else
