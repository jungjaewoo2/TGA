...
// rlabplus (C) 2003-2013, Marijan Kostrun
...
plwins = function( NWIN, dev, sz )
{
  ...
  if (!exist(NWIN))
  {
    ...
    if (length (WIN) == 0)
    { return <<available=[];default=[];device=blank(0,0)>>; }

    retl = <<>>;
    retl.default   = P;
    retl.available = [];
    retl.device    = blank(0,0);
    for(i in members(WIN))
    {
      retl.available = [retl.available, strtod(i)];
      retl.device    = [retl.device, WIN.[i].device];
    }
    idx = sort(retl.available).idx;
    retl.device    = retl.device[idx];
    retl.available = retl.available[idx];
    return retl;
  }
...
#
# original definition of showplwin
#
...
plline = function ( line_style )
{
  ...
  WIN.[P].lstyle[i;] = [1:5,1:5,1:5,1:5];
  return 1;
};
...
plot = function ( data )
{
  ...
  if (class (data) == "num" || ishist(data))
  {
    ...
    //
    pgsch (WIN.[P].char_height);       // Set character height
    ...
    pgbox (WIN.[P].gridx[p], xtick, nxsub, ...
           WIN.[P].gridy[p], ytick, nysub);
  ...
  // pglab(WIN.[P].xlabel[p], WIN.[P].ylabel[p], WIN.[P].title[p]);
  pglab(WIN.[P].xlabel[p], " ", WIN.[P].title[p]);
  pgmtxt ("L", 3, 0.5, 0.5, WIN.[P].ylabel[p]);
...
plcont = function ( CL, intval, minint )
{
  ...
  pgsci (WIN.[P].color[p;2]);          // Set line color.
  pgslw (WIN.[P].width[p]);            // Set line width.
...
plbar = function ( data )
{
  ...
  if (WIN.[P].xmin[p] != 1j) { xmin = real (WIN.[P].xmin[p]); }
  if (WIN.[P].xmax[p] != 1j) { xmax = real (WIN.[P].xmax[p]); }
  ...
  pgsfs (WIN.[P].fillstyle[p]);     // Set the fillstyle.
  ...
  for (i in 1:data.nr)          // Loop over each row (X-axis).
  {
    ...
    for (j in 2:data.nc)        // Loop over each column (N-bins per X-value).
    {
      ...
      pgsci (WIN.[P].color[p;j]);     // Set line color.
      pgrect (x1, x2, y1, y2);
    }
  }
...
plcopy = function ( ID, DEV )
{
  ...
  //
  if (!exist (ID)) { ID = 1; }
  if (!exist (DEV)) { error ("plcopy: must specify a plot device"); }
  if (!exist (WIN.[ID])){ error ("plcopy: must specify valid plot-object"); }
  ...
  if(DEV!="/XWIN")
  {
    ...
    if(sum(strindex(plwins().device,rDEV))>0)
    {
...
plaxis = function ( X_STR, Y_STR )
{
  ...
  if (exist (X_STR))
  {
    if (X_STR == "log") { WIN.[P].gridx[i] = "bcnstl"; }
  else
    WIN.[P].gridx[i] = grid_x_default;
  }
  ...
  if (exist (Y_STR))
  {
    if (Y_STR == "log") { WIN.[P].gridy[i] = "bcnstlv"; }
  else
    WIN.[P].gridy[i] = grid_y_default;
  }
  return P;
...
y_scales = function ( M, p )
{
  ...
  if (WIN.[P].ymin[p] != 1j) { xmin = real (WIN.[P].ymin[p]); }
  if (WIN.[P].ymax[p] != 1j) { xmax = real (WIN.[P].ymax[p]); }
...
z_scales = function ( M, p, xmin, xmax )
{
  ...
  if (WIN.[P].zmin[p] != 1j) { xmin = real (WIN.[P].zmin[p]); }
  if (WIN.[P].zmax[p] != 1j) { xmax = real (WIN.[P].zmax[p]); }
...
XYZ_scales = function ( X, Y, Z, p )
{
  ...
  if (WIN.[P].xmin[p] != 1j) { xmin = real (WIN.[P].xmin[p]); }
  if (WIN.[P].xmax[p] != 1j) { xmax = real (WIN.[P].xmax[p]); }
  if (WIN.[P].ymin[p] != 1j) { ymin = real (WIN.[P].ymin[p]); }
  if (WIN.[P].ymax[p] != 1j) { ymax = real (WIN.[P].ymax[p]); }
  if (WIN.[P].zmin[p] != 1j) { zmin = real (WIN.[P].zmin[p]); }
  if (WIN.[P].zmax[p] != 1j) { zmax = real (WIN.[P].zmax[p]); }
...
plot_matrix = function ( M, key, p, K, xmin, xmax, ymin, ymax, v )
{
  ...
  if (M.nc == 1)
  {
    ...
    pgsci (WIN.[P].color[p;k]);     // Set line color.
    pgsls (WIN.[P].lstyle[p;l]);    // Set line style.
    pgslw (WIN.[P].width[p;l]);     // Set line width.

    if (get_style (WIN.[P].style.[p], k-1) == "line")
    {
      pgline (M.nr, x, y);
    else if (get_style (WIN.[P].style.[p], k-1) == "point") {
      pgpt (M.nr, x, y, WIN.[P].pstyle[p;l]);
    ...
    for (i in ki)
    {
      ...
      pgptxt (xl[1]-(xmax-xmin)/25, yl[3], 0, 1, desc);
      pgslw (WIN.[P].width[p;l]);                         // Set previous line width.
...
plsurf = function (x, y, z, theta, phi)
{
  ...
  pgsci (10);                       // Set back to normal grid, label color
  pgslw (WIN.[P].width[p]);         // Line width
  pgscf (WIN.[P].font[p]);          // Set character font
  pgsch (WIN.[P].char_height);      // Set character height

  pgbox (WIN.[P].gridx[p], xtick, nxsub, ...
         WIN.[P].gridy[p], ytick, nysub);

  pgsci (WIN.[P].color[p;1]);       // Set line color.
...
pllseg = function ( V )
{
  ...
  pgsci (WIN.[P].color[p;1]);     // Set line color.
  pgsls (WIN.[P].lstyle[p;1]);    // Set line style.
  pgslw (WIN.[P].width[p;1]);     // Set line width.
...
plpnt3 = function ( PNTS, theta, phi, axis, new )
{
  ...
  if (new)
  {
    ...
    pgbox (WIN.[P].gridx[p], 0, 0, WIN.[P].gridy[p], 0, 0);
  }
  ...
  for (i in 1:pnts.nr)
  {
    pgpt (pnts.nr, pnts[;1], pnts[;2], WIN.[P].pstyle[p;1]);
