      SUBROUTINE FIXPDF(N, Y, IFLAG, ARCTOL, EPS,
     $     TRACE, A, NDIMA, NFE,
     $     ARCLEN, YP, YPOLD, QR,
     $     ALPHA, TZ, PIVOT,
     $     WT, PHI, P,
     $     PAR, IPAR,
     $     STDOUT, LSTDOUT, LIMITD, FACSQNP1, CURSW
     $     )
C
...
C   D1MACH,  F (OR  RHOA ),  FJAC (OR  RHOJAC ),  FODE,   ROOT,
C   SINTRP,  STEPS,  THE LAPACK ROUTINE  DGEQPF,  AUXILIARY LAPACK
C ROUTINES, AND THE BLAS FUNCTIONS  DAXPY,  DCOPY,  DDOT,  DGEMV,  DGER,
...
C ARRAYS NEEDED BY  FIXPDF , FODE , AND  DCPOSE .
      DOUBLE PRECISION YPOLD(N+1),A(N),QR(N,N+1),ALPHA(3*N+3),TZ(N+1),
     &     PAR(1)
...
C     Definition of stream control variables 256 appears in gsl_findroot.c
      ...
      DOUBLE PRECISION FACSQNP1, CURSW
...
40      ARCLEN=ARCLEN+S
     ...
     *    CLOSE (TRACE)
        RETURN
      ENDIF
50    IF (S .LE. FACSQNP1*SQNP1) GO TO 80
