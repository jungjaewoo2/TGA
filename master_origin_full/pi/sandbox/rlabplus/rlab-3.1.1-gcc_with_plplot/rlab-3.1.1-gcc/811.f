...
*  SE  PMFUN  CONPUTATION OF THE VALUE OF THE APPROXIMATED FUNCTION.
*         CALLING SEQUENCE: CALL PMFUN(NF,KA,X,FA) WHERE NF IS A NUMBER
*         OF VARIABLES, KA IS THE INDEX OF THE APPROXIMATED FUNCTION,
*         X(NF) IS A VECTOR OF VARIABLES AND FA IS THE VALUE OF THE
*         APPROXIMATED FUNCTION.
*  SE  PMDER  CONPUTATION OF THE GRADIENT OF THE APPROXIMATED FUNCTION.
*         CALLING SEQUENCE: CALL PMDER(NF,KA,X,GA) WHERE NF IS A NUMBER
*         OF VARIABLES, KA IS THE INDEX OF THE APPROXIMATED FUNCTION,
...
C     Messages control block
      IF (LSTDOUT .gt. 1)
     *  OPEN (12, FILE=STDOUT, STATUS='OLD' )
C     End of it. KMK
...
C     ..
C     .. External Subroutines ..
      EXTERNAL MXDSMI,MXVCOP,MXVSET,PA1MX2,PDDXQ1,PLLPB2,PLNEWS,
     +         PS0LA2,PUDBG1,PYTRFD
C     ..
...
C     .. Common blocks ..
      COMMON /STAT/NDECF,NRES,NRED,NREM,NADD,NIT,NFV,NFG,NFH
C     ..
      IF (ABS(IPRNT).GT.1) WRITE(12,FMT='(1X,''ENTRY TO PMIN :'')')
*
*     INITIATION
*
      KBF = 0
...
*
   ...
   50 CONTINUE
      IF (ABS(IPRNT).GT.1) WRITE (12,FMT=
...
*
   ...
   90 IF (IPRNT.GT.1 .OR. IPRNT.LT.0) WRITE (12,
     +    FMT='(1X,''EXIT FROM PMIN :'')')
      IF (IPRNT.NE.0) WRITE (12,FMT=
     +'(1X,''NIT='',I5,2X,''NFV='',I5,2X,''NFG='',I5,2X,       ''F ='',
     +D15.8,2X,''G ='',D11.4,2X,''ITERM='',I3)') NIT,NFV,NFG,F,GMAX,
     +    ITERM
      IF (IPRNT.LT.0) WRITE(12,FMT='(1X,''X ='',5D15.7:/(4X,5D15.7))')
     +    (X(I),I=1,NF)
      RETURN
...
*
* EXTERNAL SUBROUTINES :
*  SE  PMFUNDER  COMPUTATION OF THE VALUE AND THE GRADIENT OF THE
*         OBJECTIVE FUNCTION. CALLING SEQUENCE: CALL PMFUNDER(NF,X,F,G)
...
*         G(NF) IS THE GRADIENT OF THE OBJECTIVE FUNCTION.
*
      SUBROUTINE PBUNL(NF,NA,NB,NC,X,IX,XL,XU,CF,IC,CL,CU,CG,IA,RA,
...
C     ..
C     .. External Subroutines ..
      EXTERNAL PMFUNDER,MXVCOP,MXVDIF,MXVDIR,MXVSET,PDDBQ1,PLLPB2,
     +         PLNEWS,PS1L05
C     ..
C     .. Intrinsic Functions ..
      INTRINSIC ABS,MAX,MIN,SQRT
...
C     .. Common blocks ..
      COMMON /STAT/NDECF,NRES,NRED,NREM,NADD,NIT,NFV,NFG,NFH
C     ..
      IF (ABS(IPRNT).GT.1) WRITE(12,FMT='(1X,''ENTRY TO PBUN :'')')
*
*     INITIATION
*
      KBF = 0
...
*     FUNCTION
*
      CALL PMFUNDER(NF,X,F,G)

      NFV = NFV + 1
      NFG = NFG + 1
   40 CONTINUE
      IF (ABS(IPRNT).GT.1)
     +  WRITE(12,FMT='(1X,''NIT='',I5,2X,''NFV='',I5,2X,''NFG='',
     +         I5,2X,''F ='',
     +         D15.8,2X,''G ='',D11.4)')
     +         NIT,NFV,NFG,FP,GMAX
*
...
*
      CALL PS1L05(NF,X,XO,S,R,RP,F,FO,FP,P,PO,PP,TO,G,SNORM,
     +            RMIN,RMAX,
...
*         VARIABLES, F IS THE VALUE OF THE OBJECTIVE FUNCTION AND
*         G(NF) IS THE GRADIENT OF THE OBJECTIVE FUNCTION.
*  SE  PMHES  COMPUTATION OF THE HESSIAN MATRIX OF THE OBJECTIVE
*         FUNCTION. CALLING SEQUENCE: CALL PMHES(NF,X,H) WHERE
...
C     ..
C     .. External Subroutines ..
      EXTERNAL PMFUNDER,PMHES,MXDSMI,MXVCOP,MXVDIF,MXVDIR,MXVSET,
     +         PDDBQ2,PF1HS1,PLLPB2,PLNEWS,PS1L05
C     ..
C     .. Intrinsic Functions ..
      INTRINSIC ABS,MAX,MIN,SQRT
C     ..
C     .. Common blocks ..
      COMMON /STAT/NDECF,NRES,NRED,NREM,NADD,NIT,NFV,NFG,NFH
C     ..
      IF (ABS(IPRNT).GT.1) WRITE(12,FMT='(1X,''ENTRY TO PNEW :'')')
*
*     INITIATION
*
      KBF = 0
...
C     ..
C     .. External Subroutines ..
      EXTERNAL PMFUNDER,MXDPGF,MXDPGI,MXDSDA,MXDSMI,MXDSMM,MXDSMS,
...
C     .. Common blocks ..
      COMMON /STAT/NDECF,NRES,NRED,NREM,NADD,NIT,NFV,NFG,NFH
C     ..
      IF (ABS(IPRNT).GT.1) WRITE(12,FMT='(1X,''ENTRY TO PVAR :'')')
*
*     INITIATION
*
      KBF = 0
...
*
   ...
   80 GMAX = XNORM
      IF (IPRNT.GT.1 .OR. IPRNT.LT.0) WRITE(12,
...
*
* SUBPROGRAMS USED :
*  SE  PMFUN  COMPUTATION OF THE VALUE OF THE APPROXIMATED FUNCTION.
*  SE  PMDER  COMPUTATION OF THE GRADIENT OF THE APPROXIMATED FUNCTION.
*  S   MXVCOP  COPYING OF A VECTOR.
*  S   MXVNEG  COPYING OF A VECTOR WITH CHANGE OF THE SIGN.
*
...
C     ..
C     .. External Subroutines ..
      EXTERNAL PMDER,PMFUN,MXVCOP,MXVNEG
C     ..
C     .. Common blocks ..
      COMMON /STAT/NDECF,NRES,NRED,NREM,NADD,NIT,NFV,NFG,NFH
C     ..
...
*
* SUBPROGRAMS USED :
*  SE  PMFUNDER  OBJECTIVE FUNCTION AND SUBGRADIENT EVALUATION.
*  S   MXVCOP  COPYING OF A VECTOR.
*
...
C     ..
C     .. External Subroutines ..
      EXTERNAL PMFUNDER,MXVDIR,PNINT1
C     ..
C     .. Intrinsic Functions ..
      INTRINSIC ABS,MAX,MIN
C     ..
...
C     ..
C     .. External Subroutines ..
      EXTERNAL PMFUNDER,MXVCOP,MXVDIR,PNSTP2,PNSTP3
C     ..
C     .. Intrinsic Functions ..
      INTRINSIC ABS,MAX,MIN
C     ..
...
C     ..
C     .. External Subroutines ..
      EXTERNAL PMFUNDER,MXVDIR,PNINT1,PNSTP4,PNSTP5
C     ..
C     .. Intrinsic Functions ..
