...
ent_cluster_pao (int nargs, Datum args[])
{
  ...
  if (nargs != 2)
  {
    ...
    fprintf(rlab_stderr, THIS_SOLVER ": Format :\n");
    fprintf(rlab_stderr, THIS_SOLVER ":   c = cluster.pao(data,dist/,m,w/)\n");
  ...
  e1 = bltin_get_ent (args[0]);
  if (ent_type (e1) != MATRIX_DENSE_REAL)
    rerror ("cluster.pao: 'X' has to be a real matrix!");
  x = class_matrix_real (e1);
  ...
  e2 = bltin_get_ent (args[1]);
  if (ent_type (e2) != MATRIX_DENSE_REAL)
    rerror ("cluster.pao: 'd' has to be a real scalar!");
  d = class_double (e2);
...
/* ---------------------------------------------------------------------- */
static int
kmeanmed ( int nclusters, MDR * data, MDR * weight, int rowdominant, int npass, char dist,
           MDR ** cdata, MDR ** clusterid, double* error, MDR ** counts, int imean)
{
  int i, j, k;
...
Ent *
ent_cluster_nn (int nargs, Datum args[])
{
  Ent *e1=0, *e2=0, *e3=0;
