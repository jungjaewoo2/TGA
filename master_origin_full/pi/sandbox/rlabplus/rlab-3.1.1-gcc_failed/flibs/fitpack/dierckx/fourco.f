...
c
c  input parameters:
c    t    : real*8 array,length n, containing the knots of s(x).
c    n    : integer, containing the total number of knots. n>=10.
c    c    : real*8 array,length n, containing the b-spline coefficients.
c    alfa : real*8 array,length m, containing the parameters alfa(i).
c    m    : integer, specifying the number of integrals to be computed.
c    wrk1 : real*8 array,length n. used as working space
c    wrk2 : real*8 array,length n. used as working space
c
c  output parameters:
c    ress : real*8 array,length m, containing the integrals ress(i).
c    resc : real*8 array,length m, containing the integrals resc(i).
c    ier  : error flag:
