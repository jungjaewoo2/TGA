      SUBROUTINE FIXPNF(N, Y, IFLAG, ARCRE, ARCAE, ANSRE, ANSAE,
     &                  TRACE, A, NFE, ARCLEN,
     &                  YP, YOLD, YPOLD, QR,
     &                  ALPHA, TZ, PIVOT,
     &                  W, WP, Z0, Z1, SSPAR, PAR, IPAR,
     &                  STDOUT, LSTDOUT, LIMITD, CURSW
     &                  )
C
...
C THE SUBROUTINES  D1MACH,  F (OR  RHO ),  FJAC (OR  RHOJAC ),
C   ROOT,  ROOTNF,  STEPNF,  THE LAPACK ROUTINES  DGEQPF,  DORMQR,
C THEIR AUXILIARY ROUTINES, AND THE BLAS ROUTINES  DAXPY,  DCOPY,
C   DDOT,  DGEMM,  DGEMV,  DGER,  DNRM2,  DSCAL,  DSWAP,  DTRMM,  DTRMV,
C   IDAMAX.  ONLY  D1MACH  CONTAINS MACHINE DEPENDENT CONSTANTS.
...
C
     ...
     1   CURSW,CURTOL,D1MACH,DNRM2,H,HOLD,ONE,RELERR,S
      INTEGER IFLAG,IFLAGC,ITER,JW,LIMIT,N,NC,NFE,NFEC,NP1,
     1   TRACE
...
C
     ...
     &   QR(N,N+2),ALPHA(3*N+3),TZ(N+1),W(N+1),WP(N+1),Z0(N+1),
...
C      DOUBLE PRECISION FACSQNP1, CURSW, SAOLD


C     Messages control block
      IF (LSTDOUT .gt. 1) THEN
        TRACE = 6
        OPEN (TRACE, FILE=STDOUT, STATUS='OLD' )
      ELSE
        TRACE = 0
      ENDIF
C     End of it. KMK
...
C      POLSYS=.FALSE.
c      GO TO 11
c      ENTRY POLYNF(N,Y,IFLAG,ARCRE,ARCAE,ANSRE,ANSAE,TRACE,A,NFE,
c     &   ARCLEN,YP,YOLD,YPOLD,QR,ALPHA,TZ,PIVOT,W,WP,Z0,Z1,SSPAR,
c     &   PAR,IPAR)
c      POLSYS=.TRUE.
11    CONTINUE
C
...
217     FORMAT(' STEP',I5,3X,'NFE =',I5,3X,'ARC LENGTH =',F9.4,3X,
     &  'LAMBDA =',F7.4,5X,'X vector:',(1X,6E12.4))
      ENDIF
      NFE=NFEC
...
C CALCULATE FINAL ARC LENGTH.
        CALL DCOPY(NP1,Y,1,W,1)
        CALL DAXPY(NP1,-ONE,Z0,1,W,1)
