...
c                        DASSL, DAWTS.
c
c     The modification to interation matrix for overcoming the
...
C***ROUTINES CALLED  D1MACH, DDAINI, DDANRM, DDASTP, DDATRP, BDAWTS,
C                    XERMSG
...
C     Declare externals.
C
      EXTERNAL  D1MACH, DDAINI, DDANRM, DDASTP, DDATRP, BDAWTS, XERMSG
...
c     Check the input paramters related to an ABD Jacobian matrix.
...
c-----------------------------------------------------------------------
      CALL BDAWTS(INFO(2),RTOL,ATOL,Y,RWORK(LWT),RPAR,IPAR)
c-----------------------------------------------------------------------
      DO 305 I = 1,NEQ
...
c          write (*,*) 'ddassl: info(11) = ', info(11)
     ...
     *  INFO(10),NTEMP)
c                 write (*,*) 'ddassl: (after ddaini) idid = ', idid
...
C     LOAD Y AND H*YPRIME INTO PHI(*,1) AND PHI(*,2)
      ...
      DO 370 I = 1,NEQ
         RWORK(LPHI + I - 1) = Y(I)
c             write (*,*) '(ddassl) y(i) =', Y(I)
         RWORK(ITEMP + I - 1) = H*YPRIME(I)
c             write (*,*) '(ddassl) yp(i) =', YPRIME(I)
370      continue
C
c             write (*,*) '(ddassl) h_5 =', H, HO
c             write (*,*) 'ddassl: after 370'
390   GO TO 500
C
C-------------------------------------------------------
...
C     UPDATE WT
c-----------------------------------------------------------------------
510   CALL BDAWTS(INFO(2),RTOL,ATOL,RWORK(LPHI),
     *  RWORK(LWT),RPAR,IPAR)
c-----------------------------------------------------------------------
      DO 520 I=1,NEQ
...
c-----------------------------------------------------------------------
c               write (*,*) 'ddassl: (after ddastp) idid = ', idid
