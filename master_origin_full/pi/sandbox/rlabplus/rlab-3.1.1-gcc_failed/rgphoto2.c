...
#ifdef HAVE_GPHOTO2  /* Set in config.h */
...
//
Ent *
ent_gp_camera_config_options (int nargs, Datum args[])
{
  Ent *e1=0, *rent;
  MDS *rs=0;
  MDR *rr=0;
  ...
  if (nargs != 1)
  {
    fprintf (stdout,
             THIS_SOLVER ": gphoto2 wrapper library for RLaB\n");
    fprintf (stdout,
             THIS_SOLVER ": For a camera feature of type 'menu' or 'radio' provides list of configuration options\n");
    fprintf (stdout,
             THIS_SOLVER ": Format:\n");
    fprintf (stdout,
             THIS_SOLVER ":   " THIS_SOLVER "(feature),\n");
    fprintf (stdout,
             THIS_SOLVER ": where\n");
    fprintf (stdout,
  ...
  e1 = bltin_get_ent (args[0]);
  if (ent_type(e1) != MATRIX_DENSE_STRING)
    rerror(THIS_SOLVER ": First argument 'feature' has to be string!");
      ...
      case GP_WIDGET_TEXT:
        ...
        if (m)
        {
          ...
          for (i=0; i<m; i++)
          {
            ...
            MdsV0(rs, i) = cpstr((char *)gp_value_string);
          }
        }
        else
          rs = mds_Create(0,0);
...
//
Ent *
ent_gp_camera_config (int nargs, Datum args[])
{
  Ent *e1=0, *e2=0, *rent;
  MDS *rs=0;
  MDR *rr=0;
...
ent_gp_camera_info (int nargs, Datum args[])
{
  ...
  Ent *S = ent_Create ();
  ent_data (S) = cam_ops;
  ent_type (S) = MATRIX_DENSE_STRING;
  install (bw, "op", S);

  Ent *T = ent_Create ();
  ent_data (T) = cam_file_ops;
  ent_type (T) = MATRIX_DENSE_STRING;
  install (bw, "file_op", T);

  Ent *Q = ent_Create ();
  ent_data (Q) = cam_folder_ops;
  ent_type (Q) = MATRIX_DENSE_STRING;
  install (bw, "folder_op", Q);

  Ent *P = ent_Create ();
  ent_data (P) = cam_status;
...
//
Ent *
ent_gp_camera_init (int nargs, Datum args[])
{
  Ent *e1=0, *rent;
  char *camera_model=0;
  ...
  if (nargs == 1)
  {
    e1 = bltin_get_ent (args[0]);
    if (ent_type(e1) == MATRIX_DENSE_STRING)
      camera_model  = class_char_pointer(e1);
  }
