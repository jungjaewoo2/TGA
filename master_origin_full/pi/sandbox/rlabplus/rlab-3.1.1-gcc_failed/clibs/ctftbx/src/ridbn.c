...
static int ctftbx_Binomial_Kernel(double *RIDBN, int Kernel_Length)
{
 double *Kernel_temp=0;
 int i;

 Kernel_temp = (double *) GC_malloc (Kernel_Length * sizeof(double));
 if (!Kernel_temp)
   return 1;
...
int ctftbx_ridbn (type_signal Signal, double *WindowT, int WindowT_Length,
                  double *WindowF, int WindowF_Length, type_TFR tfr)
{
 ...
 /*--------------------------------------------------------------------*/
  ...
  for (column = 0; column < tfr.N_time; column++)
  {
    ...
    RIDBNKernel = (double *) GC_malloc( RIDBNKernel_Length * sizeof(double) );
    ...
    for (tau = 1; tau <= taumax; tau++)
    {
      ...
      RIDBNKernel = (double *) GC_realloc (RIDBNKernel, RIDBNKernel_Length*sizeof(double));

      ctftbx_Binomial_Kernel(RIDBNKernel, RIDBNKernel_Length);
      ...
      for(mu = -mumin ; mu <= mumax ; mu++)
      {
        ...
        if (Signal.is_complex == TRUE)
        {
          R1 = R1 + Signal.signal[time+tau-mu] * conj(Signal.signal[time-tau-mu])
              * RIDBNKernel[tau+mu] * WindowT[half_WindowT_Length + mu]/ normK;
    ...
    RIDBNKernel = (double *) GC_malloc ( (2*tau+1) * sizeof(double) );
