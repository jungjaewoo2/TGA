...
ent_gsl_sf_erf (int nargs, Datum args[])
{
  ...
  if (nargs == 0)
  {
    printf (THIS_SOLVER ": Error function. Format:\n");
    printf (THIS_SOLVER ":   Erf(x)\n");
    rerror (THIS_SOLVER ": No parameters given!");
  }
  //
  // get x
  //
  ...
  //
  for (i = 0; i < nr*nc; i++)
    MdrV0 (w, i) = gsl_sf_erf (mdrV0 (x1, i));
...
//
#undef  THIS_SOLVER
#define THIS_SOLVER "InvErf"
Ent * ent_gsl_sf_inverf (int nargs, Datum args[])
{
  Ent *e1 = 0;
  int nr, nc, i;
  MDR *x1, *w;
  ...
  for (i = 0; i < nr*nc; i++)
  {
    ...
    MdrV0 (w, i) = M_SQRT1_2 * gsl_cdf_ugaussian_Pinv (0.5 * mdrV0 (x1, i) + 0.5);
  }
