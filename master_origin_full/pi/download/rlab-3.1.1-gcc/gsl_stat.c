...
Ent * ent_mean (int nargs, Datum args[])
{
  ...
  if (nargs==2)
  {
    ...
    {
      RLABCODE_PROCESS_BTREE_ENTRY_BOOL(e2,node,RLAB_NAME_STAT_FLAT,flat);

      RLABCODE_PROCESS_BTREE_ENTRY_MD(e2,node,RLAB_NAME_STAT_FACTOR,factor_idx,MDR,IsFinite,1,NULL);

      RLABCODE_PROCESS_BTREE_ENTRY_MD(e2,node,RLAB_NAME_STAT_IGNOREIDX,ig_idx,MDR,IsFinite,1,NULL);

      RLABCODE_PROCESS_BTREE_ENTRY_MD(e2,node,RLAB_NAME_STAT_USEIDX,iuse_idx,MDR,IsFinite,1,NULL);

      RLABCODE_PROCESS_BTREE_ENTRY_BOOL(e2,node,RLAB_NAME_STAT_IGNOREINF,ignore_infs);

      RLABCODE_PROCESS_BTREE_ENTRY_BOOL(e2,node,RLAB_NAME_STAT_ROWDOM,rowdominant);
    }
  }
    ...
    if (node1 && node2)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          for (i=0;i<MNC(x);i++)
            MdrV0(w,i) = gsl_stats_wmean ( &MdrV0(t, i), MNR(x), &MdrV0(x, i), MNR(x), MNC (x));
        }
      }
    }
...
Ent * ent_variance (int nargs, Datum args[])
{
  ...
  ListNode *node=0, *node1=0, *node2=0;
  int i, j, rowdominant=0, ignore_infs=1, unbias=0, flat=0;
  ...
  if (nargs==2)
  {
    ...
    if (ent_type (e2) == BTREE)
    {
      ...
      int bias=0;
      RLABCODE_PROCESS_BTREE_ENTRY_BOOL(e2,node,RLAB_NAME_STAT_BIAS,bias);
      unbias = 1 - bias;

      RLABCODE_PROCESS_BTREE_ENTRY_MD(e2,node,RLAB_NAME_STAT_MEAN,m,MDR,IsFinite,1,NULL);
    }
  }
  ...
  else if (ent_type (e1) == BTREE)
  {
    ...
    if (node1 && node2)
    {
      ...
      if (EQVECT(x))
      {
        if (m)
        {
          w = mdr_Create (1, SIZE(m));
          if (unbias)
          {
            for (i=0; i < SIZE(m); i++)
            {
              MdrV0 (w, i) = gsl_stats_wvariance_m (MDRPTR(t), 1, MDRPTR(x),
                     1, SIZE(x), mdrV0_safe(m,i));
            }
          }
          else
          {
            for (i = 0; i < SIZE(fm); i++)
            {
              MdrV0 (w, i) = gsl_stats_wvariance_with_fixed_mean (MDRPTR(t), 1, MDRPTR(x),
                     1, SIZE(x), mdrV0_safe(m,i));
            }
          }
        }
        ...
        {
          w = mdr_CreateScalar (gsl_stats_wvariance (MDRPTR(t), 1, MDRPTR(x), 1, SIZE(x)));
        }
      }
      else
      {
        if (m)
        {
          if (rowdominant)
          {
            ...
            {
              for (i=0;i<MNR(x);i++)
                MdrV0 (w, i) = gsl_stats_wvariance_m ( &MdrV0(t,i), MNR(t),
                       &MdrV0(x,i), MNR(x), MNC(x), mdrV0_safe(m,i) );
            }
            else
            {
              for (i=0;i<MNR(x);i++)
                MdrV0 (w, i) = gsl_stats_wvariance_with_fixed_mean (&MdrV0(t,i), MNR(x),
                       &MdrV0(x,i), MNR(x), MNC(x), mdrV0_safe(m,i));
            }
          }
          else
          {
            ...
            {
              for (i=0;i<MNC(x);i++)
                MdrV0 (w, i) = gsl_stats_wvariance_m (&MdrV0(t,i*MNR(t)), 1,
                       &MdrV0(x,i*MNR(x)), 1, MNR(x), mdrV0_safe(m,i));
            }
            else
            {
              for (i=0;i<MNC(x);i++)
                MdrV0 (w, i) = gsl_stats_wvariance_with_fixed_mean (&MdrV0(t,i*MNR(t)), 1,
                       &MdrV0(x,i*MNR(x)), 1, MNR(x), mdrV0_safe(m,i));
            }
          }
        }
        else
        {
          if (rowdominant)
          {
            w = mdr_Create(MNR(x),1);
            for (i=0;i<MNR(x);i++)
              MdrV0 (w, i) = gsl_stats_wvariance (&MdrV0(t,i), MNR(x), &MdrV0(x,i), MNR(x), MNC(x));
          }
          else
          {
            w = mdr_Create(1,MNC(x));
            for (i=0;i<MNC(x);i++)
              MdrV0 (w, i) = gsl_stats_wvariance (&MdrV0(t,i*MNR(t)), 1, &MdrV0(x,i*MNR(x)), 1, MNR(x));
          }
        }
      }
    }
...
//
#undef  THIS_SOLVER
#define THIS_SOLVER "absdev"
Ent * ent_absdev (int nargs, Datum args[])
{
  Ent *e1=0, *e2=0, *e_last=0;
  MDR *x=0, *t=0, *w=0, *m=0;
  ...
  if ((ent_type(e1)!=MATRIX_DENSE_REAL)&&(ent_type(e1)!=BTREE))
    rerror ( THIS_SOLVER ": " RLAB_ERROR_ARG1_STAT_OR_MDR_VECTOR "\n");

  // check last argument
  e_last = bltin_get_ent (args[nargs-1]);
  if (ent_type (e_last) == BTREE)
  {
    RLABCODE_PROCESS_BTREE_ENTRY_BOOL(e_last,node1,RLAB_NAME_STAT_ROWDOM,rowdominant);
    RLABCODE_PROCESS_BTREE_ENTRY_MD(e_last,node1,RLAB_NAME_STAT_MEAN,m,MDR,IsFinite,1,NULL);
  }
  if ((nargs==3)&&(!m))
  {
    RLABCODE_PROCESS_ARG_MD_IGNORE_ERROR(THIS_SOLVER,1,e2,MATRIX_DENSE_REAL,m,class_matrix_real,SIZE,<,1,NULL);
  }
  if ( (nargs==2) && (ent_type (e_last) == MATRIX_DENSE_REAL) )
  {
    m = ent_data(e_last);
    if (SIZE(m) < 1)
      m = NULL;
    ...
    if (node1 && node2)
    {
      ...
      if (EQVECT(x))
      {
        if (m)
        {
          ...
          for (i=0; i < SIZE(m); i++)
          {
            MdrV0 (w, i) = gsl_stats_wabsdev_m (MDRPTR(t), 1, MDRPTR(x),
                    1, SIZE(x), mdrV0_safe(m,i));
          }
        }
        else
        {
          w = mdr_CreateScalar(gsl_stats_wabsdev (MDRPTR(t), 1, MDRPTR(x), 1, SIZE(x)));
        }
      }
      else
      {
        if (m)
        {
          if (rowdominant)
          {
            w = mdr_Create(MNR(x),1);
            for (i=0;i<MNR(x);i++)
              MdrV0 (w, i) = gsl_stats_wabsdev_m ( &MdrV0(t,i), MNR(t),
                      &MdrV0(x,i), MNR(x), MNC(x), mdrV0_safe(m,i) );
          }
          else
          {
            w = mdr_Create(1,MNC(x));
            for (i=0;i<MNC(x);i++)
              MdrV0 (w, i) = gsl_stats_wabsdev_m (&MdrV0(t,i*MNR(t)), 1,
                      &MdrV0(x,i*MNR(x)), 1, MNR(x), mdrV0_safe(m,i));
          }
        }
        else
        {
          if (rowdominant)
          {
            w = mdr_Create(MNR(x),1);
            for (i=0;i<MNR(x);i++)
              MdrV0 (w, i) = gsl_stats_wabsdev (&MdrV0(t,i), MNR(x), &MdrV0(x,i), MNR(x), MNC(x));
          }
          else
          {
            w = mdr_Create(1,MNC(x));
            for (i=0;i<MNC(x);i++)
              MdrV0 (w, i) = gsl_stats_wabsdev (&MdrV0(t,i*MNR(t)), 1, &MdrV0(x,i*MNR(x)), 1, MNR(x));
          }
        }
      }
    }
  ...
  else if (ent_type (e1) == MATRIX_DENSE_REAL)
  {
    ...
    if (EQVECT(x))
    {
      if (m)
      {
        ...
        for (i=0; i < SIZE(m); i++)
        {
          MdrV0 (w, i) = gsl_stats_absdev_m (MDRPTR(x), 1, SIZE(x), mdrV0_safe(m,i));
        }
      }
      else
      {
        w = mdr_CreateScalar(gsl_stats_absdev (MDRPTR(x), 1, SIZE(x)));
      }
    }
    else
    {
      if (m)
      {
        if (rowdominant)
        {
          w = mdr_Create(MNR(x),1);
          for (i=0;i<MNR(x);i++)
            MdrV0 (w, i) = gsl_stats_absdev_m (&MdrV0(x,i), MNR(x), MNC(x), mdrV0_safe(m,i) );
        }
        else
        {
          w = mdr_Create(1,MNC(x));
          for (i=0;i<MNC(x);i++)
            MdrV0 (w, i) = gsl_stats_absdev_m (&MdrV0(x,i*MNR(x)), 1, MNR(x), mdrV0_safe(m,i));
        }
      }
      else
      {
        if (rowdominant)
        {
          w = mdr_Create(MNR(x),1);
          for (i=0;i<MNR(x);i++)
            MdrV0 (w, i) = gsl_stats_absdev (&MdrV0(x,i), MNR(x), MNC(x));
        }
        else
        {
          w = mdr_Create(1,MNC(x));
          for (i=0;i<MNC(x);i++)
            MdrV0 (w, i) = gsl_stats_absdev (&MdrV0(x,i*MNR(x)), 1, MNR(x));
        }
      }
    }
  }
...
//
#undef  THIS_SOLVER
#define THIS_SOLVER "median"
Ent * ent_median (int nargs, Datum args[])
{
  Ent *e1=0;
  MDR *x=0, *w=0;
  int nr, nc, i, j;
  ...
  {
    fprintf(stdout,
            THIS_SOLVER ": Finds a column-wise median of a real data matrix or\n");
    fprintf(stdout,
            THIS_SOLVER ": a median of a data vector.\n");
    fprintf(stdout,
            THIS_SOLVER ": Format:\n");
    fprintf(stdout,
            THIS_SOLVER ":   y = median(x),\n");
    fprintf(stdout,
            THIS_SOLVER ": where 'x' is a data vector or a real data matrix.\n");
  ...
  if (ent_type (e1) == MATRIX_DENSE_REAL)
  {
    ...
    {
      // flat
      x = mdr_Copy2double_and_Sort(x, -1);
      w = mdr_CreateScalar (gsl_stats_median_from_sorted_data (MDRPTR(x), 1, nr * nc));
      mdr_Destroy (x);
    }
    else
    {
      x = mdr_Copy2double_and_Sort(x, 0);
      // find column-wise medians
      w = mdr_Create (1, nc);
      for (i=0; i < nc; i++)
      {
        MdrV0 (w, i) = gsl_stats_median_from_sorted_data ( &MdrV0(x,i*nr), 1, nr);
      }
      mdr_Destroy (x);
    }
  }
  ...
  {
    int neg_inf=0, pos_inf=0, sub_min=0, sup_max=0;
    ListNode *node1=0, *node2=0, *node3=0;

    // median of a gsl-type histogrammed variable <<bin;range>>
    node1 = btree_FindNode (ent_data (e1), RLAB_NAME_HIST1D_BIN);
    node2 = btree_FindNode (ent_data (e1), RLAB_NAME_HIST1D_RANGE);
    node3 = btree_FindNode (ent_data (e1), RLAB_NAME_HIST1D_INFO);
    if (node3)
    {
      ListNode *node4=0;
      node4 = btree_FindNode (ent_data (var_ent(node3)), RLAB_NAME_HIST1D_PINFS);
      if (node4)
        pos_inf = class_double(var_ent(node4));
      node4 = btree_FindNode (ent_data (var_ent(node3)), RLAB_NAME_HIST1D_NINFS);
      if (node4)
        neg_inf = class_double(var_ent(node4));
      node4 = btree_FindNode (ent_data (var_ent(node3)), RLAB_NAME_HIST1D_TRASH_MAX);
      if (node4)
        sup_max = class_double(var_ent(node4));
      node4 = btree_FindNode (ent_data (var_ent(node3)), RLAB_NAME_HIST1D_TRASH_MIN);
      if (node4)
        sub_min = class_double(var_ent(node4));
    }
    ...
    if (node1 != 0 && node2 !=0)
    {
      ...
      // get the size
      int nbins=SIZE(range)-1;

      if (SIZE(bin) != nbins)
        rerror (THIS_SOLVER ": invalid first argument <<bin;range>>");

      //
      // construct interpolation table
      // 
      MDR * t = mdr_Create(nbins+1,2);
      Mdr0(t,0,0) = mdrV0(range,0);
      Mdr0(t,0,1) = neg_inf + sub_min;
      for (j=1; j<=nbins; j++)
      {
        Mdr0(t,j,0) = mdrV0(range,j);
        Mdr0(t,j,1) = Mdr0(t,j-1,1) + MdrV0(bin,j-1);
      }
      double totsum = Mdr0(t,nbins,1) + pos_inf + sup_max;
      for (j=0; j<=nbins; j++)
      {
        Mdr0(t,j,1) /= totsum;
      }
      ...
      // solve for median:
//       mdr_Print(t,stdout)
      w=mdr_FindRootsFromInterpolationTable(t, 0.5);
      mdr_Destroy(t);
...
Ent * ent_quantile (int nargs, Datum args[])
{
  Ent *e1=0, *e2=0;
  MDR *x1=0, *x=0, *w=0, *f=0;
  int nf, n=0, i, ikill_x=0;
  ...
  if (nn>0)
  {
    if (rlab_sort_get_nans()==RLAB_SORT_NAN_ONBOTTOM)
    {
    ...
    else if (rlab_sort_get_nans()==RLAB_SORT_NAN_ONTOP)
    {
      w = mdr_Create_SameSize (f);
      for (i=0; i < nf; i++)
      {
...
int histogram_clip(double * range, double * bin, int n, double xlo, double xhi, double ** new_range, double ** new_bin, int * new_n)
{
  int rval=1, i, iend=0, istart=0;
  ...
  *new_n = iend - istart;
  if (*new_n > 0)
  {
    *new_range = (double *) GC_MALLOC((*new_n + 1) * sizeof(double));
    *new_bin   = (double *) GC_MALLOC((*new_n    ) * sizeof(double));
    for (i=istart; i<iend; i++)
    {
      if (i == istart)
      {
        (*new_range)[i-istart] = xlo;
        (*new_bin)[i-istart] = (range[i+1] - xlo) / (range[i+1] - range[i]) * bin[istart];
        continue;
      }
      (*new_range)[i-istart] = range[i];
      if (i == iend-1)
      {
        (*new_range)[i-istart+1] = xhi;
        (*new_bin)[i-istart] = round((xhi - range[i]) / (range[i+1] - range[i]) * bin[i]);
        break;
      }
      (*new_bin)[i-istart]   = bin[i];
    }
  }
  rval = 0;
...
#undef  THIS_SOLVER
#define THIS_SOLVER "clip"
Ent * ent_gsl_clip (int nargs, Datum args[])
{
  Ent *e1=0, *e2=0, *e3=0, *eret=0;
  MDR *w=0, *x=0, *xlim=0, *new_b=0, *new_r=0;
  double xlo, xhi;
  Btree *b=0;
  ...
  if ((nargs != 2) && (nargs != 3))
  {
    fprintf (stdout, THIS_SOLVER ": clip vector of data\n");
    fprintf (stdout, THIS_SOLVER ": Format:\n");
    fprintf (stdout, THIS_SOLVER ":   h = clip(x, xlo, xhi),  h = clip(x, [xlo, xhi])\n");
  ...
  if (ent_type (e1) == MATRIX_DENSE_REAL)
  {
    ...
    if (size > 0)
    {
      xmin = rlab_dmin_mdr(x, 1);
      xmax = rlab_dmax_mdr(x, 1);
    }
    if (xlo <= xmin)
      xlo = xmin;
    if (xhi >= xmax)
    ...
    for (i=0; i<SIZE(x); i++)
    {
      if (mdrV0(x,i) >= xlo && mdrV0(x,i) <= xhi)
        count++;
    }
    ...
    {
      int count_again=0;
      w = mdr_Create(1,count);
      for (i=0; i<SIZE(x); i++)
      {
        if (mdrV0(x,i) >= xlo && mdrV0(x,i) <= xhi)
        {
          MdrV0(w,count_again++) = mdrV0(x,i);
    ...
    eret = ent_Assign_Rlab_MDR(w);
  }

  if (ent_type (e1) == BTREE)
  {
    Btree *bw = ent_data(e1);
    ...
    MDR *bin = class_matrix_real (var_ent (node));
    ...
    MDR *range = class_matrix_real (var_ent (node));
    ...
    double *new_range=0, *new_bin=0;
    int new_n=0;

    if (histogram_clip(MDRPTR(range), MDRPTR(bin), n, xlo, xhi, &new_range, &new_bin, &new_n))
      goto exit;

    new_r = mdr_Create(0,0);
    MDPTR(new_r) = new_range; new_range=0;
    MNR(new_r) = 1;
    MNC(new_r) = new_n+1;
    new_b = mdr_Create(0,0);
    MDPTR(new_b) = new_bin; new_bin=0;
    MNR(new_b) = 1;
    MNC(new_b) = new_n;

    b = btree_Create();
    // bins
    install (b, RLAB_NAME_HIST1D_BIN, ent_Assign_Rlab_MDR(new_b));
    // range
    install (b, RLAB_NAME_HIST1D_RANGE, ent_Assign_Rlab_MDR(new_r));
    //
    eret = ent_Assign_Rlab_BTREE(b);
  }
...
Ent * ent_histogram_create (int nargs, Datum args[])
{
  Ent *e1=0, *e2=0, *e3=0, *ent_info=0;
  MDR *x=0, *r=0, *range=0, *bin=0;
  int i, nbins=10, size=0, count_pos_inf=0, count_neg_inf=0, count_nan=0, count_below_min=0;
...
Ent * ent_histogram2d_create (int nargs, Datum args[])
{
  ...
  if (nargs == 1)
  {
    ...
    x1min = rlab_dmin_vector(nrows, 1, &MdrV0(x,0), 1);
    x1max = rlab_dmax_vector(nrows, 1, &MdrV0(x,0), 1);
    x2min = rlab_dmin_vector(nrows, 1, &MdrV0(x,nrows), 1);
    x2max = rlab_dmax_vector(nrows, 1, &MdrV0(x,nrows), 1);
