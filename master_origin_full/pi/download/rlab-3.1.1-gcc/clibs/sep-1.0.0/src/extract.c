...
//   srand(1);
 ...
 exit:
  ...
  if (status != RETURN_OK)
  {
    ...
    //if ((cdscan != NULL) && (cdscan != dummyscan) && (cdscan != dbuf.midline))
    if ((conv != NULL) && (cdscan != dummyscan))
      free(cdscan);
	   ...
	   int deblend_nthresh, double deblend_mincont, double gain)
{
  ...
  status = deblend(objlist, 0, &objlistout, deblend_nthresh, deblend_mincont,minarea);
  if (status)
  {
