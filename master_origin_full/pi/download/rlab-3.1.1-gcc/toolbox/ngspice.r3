...
NGSPICE_CMD = reads("|" + WHICH + " ngspice 2>/dev/null");
if (isempty(NGSPICE_CMD))
{ clear(NGSPICE_CMD); }
if (strlen(NGSPICE_CMD)<strlen("ngspice"))
{ clear(NGSPICE_CMD);}
...
//
if (exist(NGSPICE_CMD))
{
  spice.exec = function (my_script, fn_scp, fn_log, fn_err)
  {
    eraseme = 0;
    ...
    if (exist(my_script))
    {
      if (class(my_script)!="string")
      { return 1; }
      open(f1,"w");
      for (i in 1:my_script.n)
      {
        fprintf(f1, "%s\n", my_script[i]);
      }
      close(f1);
  ...
  spice.fork = function (my_script, fn_scp, fn_raw, fn_log)
  {
    eraseme = 0;
    my_ngspice_cmd = NGSPICE_CMD;
...
else
{
  spice.exec = function (my_script, fn_scp, fn_log, fn_err)
  {
    printf("spice.exec: ngspice not found on this system!\n");
    printf("spice.exec: function is not available!\n");
    return 1;
  };

  spice.fork = function (my_script, fn_scp, fn_raw, fn_log)
  {
    printf("spice.fork: ngspice not found on this system!\n");
    printf("spice.fork: function is not available!\n");
    open(fn_raw,"w");
    fprintf(fn_raw,"\n");
    close(fn_raw);
  };
