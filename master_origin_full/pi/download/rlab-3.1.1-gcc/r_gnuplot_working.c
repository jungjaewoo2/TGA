...
static int    gnuplot_fd_w  [ GNUPLOT_MAX_NUM_WINDOWS ] = {0};
static int    gnuplot_fd_r  [ GNUPLOT_MAX_NUM_WINDOWS ] = {0};
static int    gnuplot_pid   [ GNUPLOT_MAX_NUM_WINDOWS ] = {0};
...
static int gnuplot_pipe_init_and_redirect ( char * errname, pid_t * pid, int * fd_rw )
{
  ...
  if (pipe(wpipefd)==-1)
  {
    perror(THIS_SOLVER ": Creation of pipe to GNUPLOT failed. Cannot continue!\n");
    return (1);
  }
  if (pipe(rpipefd)==-1)
  {
  ...
  *pid = fork();
  ...
  if (*pid ==0)
  {
    // for child our wpipefd[0] will be input instead of STDIN
    while ((dup2(wpipefd[0], STDIN_FILENO) == -1) && (errno == EINTR));
    close(wpipefd[0]);
    close(wpipefd[1]);

    // for child our rpipefd[1] will be output instead of STDOUT
    while ((dup2(rpipefd[1], STDOUT_FILENO) == -1) && (errno == EINTR));
    close(rpipefd[1]);
    close(rpipefd[0]);

    // start a GNUPLOT session
    int res = execl(pipcmd, pipcmd, (char*)NULL);

    // exit when session is over
    exit (res);
  }

  // no need for parent to access entrance to the READ
  close(rpipefd[0]);
  // no need for parent to access exit from the WRITE
  close(wpipefd[1]);
...
static FILE * gnuplot_pipe_init( char * errname )
{
  FILE *  handle ;
  char pipcmd[128] = {'\0'};
