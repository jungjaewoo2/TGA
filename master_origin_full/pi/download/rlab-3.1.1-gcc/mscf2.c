...
superlu_msc_SolveCC (MSC * a, MDC * b, char *type)
{
  ...
  //
  zCreate_CompRow_Matrix (&A, n, n, ne, (doublecomplex *) a->c, a->ja, a->ia, SLU_NR, SLU_Z,
			  SLU_GE);
  //
  // Options
  //
...
MDC * msc_SpSolve (MSC * a, MDC * b, double diag_pivot_thresh, int *perm_c)
{
  int i;
  Matrix A, B;
  Matrix *L, *U;
...
//   DNformat *Bstore;
  ...
  //Astore->rowind = a->ja;
  //Astore->colptr = a->ia;
  zCreate_CompCol_Matrix (&A, n, n, ne, (doublecomplex *) a->c, a->ja, a->ia, SLU_NC, SLU_Z,
			  SLU_GE);
 ...
 * Factor
 * ************************************************************** */
Btree * msc_Factor (MSC * a, char *type)
{
  Btree *x;
...
MDC * msc_Backsub (Btree * bt, MDC * b)
{
  MSC *L = 0, *U = 0;
  MDC *sol;
  ListNode *ltmp;
