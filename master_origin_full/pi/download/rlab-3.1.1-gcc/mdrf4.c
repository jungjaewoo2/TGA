...
MDR * mdr_vec_local_extrema(MDR *x, double erel, double eabs, int is_max, int check_edges)
{
  MDR *w=0;
  int nx=SIZE(x)-1, i, j=0, j0, k;
  int (*f)(double, double, double, double);
  double xprev;

  // remove trailing nan's and +/-inf's 
  while (!isfinite(mdrV0(x,nx)))
  {
    nx--;
  }

  // skip leading infs or nans that may appear on edges of the array, e.g.,
  // if the array was obtained from histograms
  j = 0;
  while (!isfinite(mdrV0(x,j)))
  {
    j++;
  }

  // on arrays shorter then 3 elements no point in looking for extremae
  if (nx - j < 3)
    goto exit;
...
//   printf("%i,%i\n", j+1, nx+1);
  ...
  //
  // populate vector with local extremae
  //
  j = j0;
  w = mdr_Create(1,nx+1);
  i = 0;
  // left edge of the interval: check_edges = 1(Left edge only), 3 (Left and Right edge)
  k = j+1;
  if ((check_edges&1) == 1)
  {
    if (f(mdrV0(x,j),mdrV0(x,k),erel,eabs))
    {
      // isolated extremum
      MdrV0(w,i++) = j + 1;
    }
    else if (approxequal(mdrV0(x,j),mdrV0(x,k),erel,eabs))
    {
      // flat portion of data then we don't know how to handle yet
      while (approxequal(mdrV0(x,j),mdrV0(x,k),erel,eabs))
      {
        k++;
        if (k == nx)
          break;
      }
      if ((k == nx) || f(mdrV0(x,j),mdrV0(x,k),erel,eabs))
      {
        MdrV0(w,i++) = ((j+k)>>1);
      }
    }
  ...
  while (j<nx)
  {
    if (f(mdrV0(x,j),xprev,erel,eabs))
    {
      k = j+1;
      while (approxequal(mdrV0(x,j),mdrV0(x,k),erel,eabs))
      {
      ...
      if (  ((k==nx+1)&&(check_edges>1)) || ((k<=nx) && (f(mdrV0(x,j),mdrV0(x,k),erel,eabs))) )
      {
        MdrV0(w,i++) = ((j+k)>>1);
      }
      xprev = mdrV0(x,j);
  ...
  // right edge of the interval: check_edges = 2 (Right edge only), 3 (Left and Right edge)
  if ((check_edges>1) && (j==nx))
  {
    if (f(mdrV0(x,nx),mdrV0(x,nx-1),erel,eabs))
    {
      MdrV0(w,i++) = nx + 1;
    }
  }
  ...
  if ( (i < nx+1) && (i > 0) )
  {
    // we just cheat here and relabel the length of the array
    // gc keeps track of its actual size
    MNC(w) = i;
  }
  else
  {
...
//
int mdr_mpow1 (MDR * A0, int j, MDR * B)
{
  int i, n;
  MDR *A=0, *C=0;
...
//
static int matvec (double *x, double *y)
{
  int i, j;
  int n = mexp_a->nrow;
