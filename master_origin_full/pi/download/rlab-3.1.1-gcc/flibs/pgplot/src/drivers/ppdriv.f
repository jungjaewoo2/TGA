...
* 9-Nov-2017  - Patched by M. Kostrun
*-----------------------------------------------------------------------
...
C
      ...
      INTEGER CDEFLT(3,0:15), JUNK, ICOL
...
C
C     MK was here, 2017: Adjust sizes to your maximum expected picture
      INTEGER PIXDIM_X,PIXDIM_Y
      PARAMETER (PIXDIM_X=2848,PIXDIM_Y=1424)
      INTEGER PIXMAP(PIXDIM_X,PIXDIM_Y)
C
...
C
  ...
  900 CALL GRWARN('Unimplemented function in PPM device driver:'
     1    //MSG)
      NBUF = -1
      RETURN
C
...
!        PIXMAP = 0
!     END IF
C     -- initialize to background color
!     MK was here, 2017
      ICOL = CTABLE(1,0) + 256*CTABLE(2,0) + 256*256*CTABLE(3,0)
      CALL GRPP03(1, 1, BX, BY, ICOL, BX, BY, PIXMAP)
