...
void figdisp_convbufout(unsigned short *, int);
void figdisp_convbufin(unsigned short *buf, int len);
int figdisp_opencomm(int maxbuf,int dev);
...
/* the command buffer */
/* number of shorts in the command buffer */
void figdisp_sendcommand(short unsigned *buffer,int len)
{
...
/* the number of shorts in the response */
unsigned short *figdisp_getresponse(int *len)
{
  XEvent event; /* for the X events */
  unsigned short *retval;  /* the return value */
  ...
  while(fd_dispopen && !xerror)
  {
    ...
    if (event.xproperty.atom == selatom && event.xproperty.state == PropertyNewValue)
    {
...
int figdisp_opencomm(int maxbuf,int dev)
{
	...
	if (!fd_dispopen)
	{
		...
		if ((lockatom=XInternAtom(display,&lockname[0],False)) == None)
			...
			if (((XGetSelectionOwner(display,selatom) != XGetSelectionOwner(display,lockatom))
        && (XGetSelectionOwner(display,selatom) != window)) || (XGetSelectionOwner(display,selatom) == None)   )
      {
...
void figdisp_convbufout(unsigned short *buf, int len)
{
