...
C  XTICK(4) (input) : world coordinate interval between major tick marks
...
C                       [xtick,lc,lw,lt]
C  NXSUB(4)  (input)  : the number of subintervals to divide the major
...
C  YTICK(4)  (input)  : like XTICK for the Y axis.
C  NYSUB(4)  (input)  : like NXSUB for the Y axis.
C
...
C 18-Nov-2017 - [MK,rlabpatch] configure and plot minor and major grid ticks (color,width,linetype)
C-----------------------------------------------------------------------
      INCLUDE  'pgplot.inc'
      CHARACTER*20  CLBL
      CHARACTER*64  OPT
      ...
      LOGICAL  IRANGE, MAJOR, XOPTLS, YOPTLS, PGNOTO, XOPTH, YOPTH
      REAL     TAB(9), UTAB(9)
      INTEGER  I, I1, I2, J, NC, NP, NV, KI, CLIP, ILC, ILW, ILT
                ...
                IF (IRANGE(XBLC,XVAL,XTRC)) THEN
                    ...
                    ELSE
                        IF (XOPTH) THEN
                          ...
                          CALL GRQLW(ILW)
                          CALL GRQLS(ILT)
                          IF (NXSUB(2).NE.-1) CALL GRSCI(NXSUB(2))
                          IF (NXSUB(3).NE.-1) CALL GRSLW(NXSUB(3))
                          IF (NXSUB(4).NE.-1) CALL GRSLS(NXSUB(4))
                          CALL GRMOVA(XVAL, YBLC+TIKL)
                          CALL GRLINA(XVAL, YTRC-TIKL)
                          CALL GRSCI(ILC)
                          CALL GRSLW(ILW)
                          CALL GRSLS(ILT)
                        ENDIF
...
C
          END IF
C
C Draw X grid through major X tics
C
      IF (XOPTG) THEN
          ...
          CALL GRQLS(ILT)
          CALL GRSCI( 0 )
          CALL GRSLW( MAX(NXSUB(3),INT(XTICK(3))) )
          CALL GRSLS( 1 )
          DO 20 I=I1+1,I2-1
              CALL GRMOVA(REAL(I)*XINT, YBLC+TIKL)
              CALL GRLINA(REAL(I)*XINT, YTRC-TIKL)
   20     CONTINUE
          CALL GRSCI(ILC)
          CALL GRSLW(ILW)
          CALL GRSLS(ILT)
          IF (XTICK(2).NE.-1.0) CALL GRSCI( INT(XTICK(2)) )
          IF (XTICK(3).NE.-1.0) CALL GRSLW( INT(XTICK(3)) )
          IF (XTICK(4).NE.-1.0) CALL GRSLS( INT(XTICK(4)) )
          DO 21 I=I1+1,I2-1
              CALL GRMOVA(REAL(I)*XINT, YBLC+TIKL)
              CALL GRLINA(REAL(I)*XINT, YTRC-TIKL)
   21     CONTINUE
                ...
                IF (IRANGE(YBLC,YVAL,YTRC)) THEN
                    ...
                    ELSE
                        IF (YOPTH) THEN
                            ...
                            CALL GRQLW(ILW)
                            CALL GRQLS(ILT)
                            IF (NYSUB(2).NE.-1) CALL GRSCI(NYSUB(2))
                            IF (NYSUB(3).NE.-1) CALL GRSLW(NYSUB(3))
                            IF (NYSUB(4).NE.-1) CALL GRSLS(NYSUB(4))
                            CALL GRMOVA(XBLC+TIKL, YVAL)
                            CALL GRLINA(XTRC-TIKL, YVAL)
                            CALL GRSCI(ILC)
                            CALL GRSLW(ILW)
                            CALL GRSLS(ILT)
                        ENDIF
...
C
      IF (YOPTG) THEN
          CALL PGBOX1(YBLC, YTRC, YINT, I1, I2)
              ...
              CALL GRQCI(ILC)
              CALL GRSCI( 0 )
              CALL GRSLW( MAX(NYSUB(3),INT(YTICK(3))) )
              CALL GRSLS( 1 )
              DO 111 I=I1+1,I2-1
                  CALL GRMOVA(XBLC+TIKL, REAL(I)*YINT)
                  CALL GRLINA(XTRC-TIKL, REAL(I)*YINT)
  111         CONTINUE
              IF (YTICK(2).NE.-1.0) CALL GRSCI( INT(YTICK(2)) )
              IF (YTICK(3).NE.-1.0) CALL GRSLW( INT(YTICK(3)) )
              IF (YTICK(4).NE.-1.0) CALL GRSLS( INT(YTICK(4)) )
              DO 110 I=I1+1,I2-1
                  CALL GRMOVA(XBLC+TIKL, REAL(I)*YINT)
                  CALL GRLINA(XTRC-TIKL, REAL(I)*YINT)
  110             CONTINUE
