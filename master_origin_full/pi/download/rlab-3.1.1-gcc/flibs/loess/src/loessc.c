...
//     LL = Calloc((*n)*(*n), double);
    hat_matrix = (double*) GC_malloc((*n)*(*n) * sizeof(double));
    LL = (double*) GC_malloc((*n)*(*n) * sizeof(double));
    F77_SUB(lowesb)(x, y, weights, diagonal, &one, iv, &liv, &lv, v);
...
loess_dfit(double *y, double *x, double *x_evaluate, double *weights,
           ...
           int *drop_square, int *sum_drop_sqr, int *d, int *n, int *m, double *fit)
{
  int	zero = 0, one = 1;
...
//   iv = Calloc(liv, int);
//   v = Calloc(lv, double);
  iv = (int *)    GC_malloc(liv * sizeof(int));
  v  = (double *) GC_malloc(lv  * sizeof(double));
