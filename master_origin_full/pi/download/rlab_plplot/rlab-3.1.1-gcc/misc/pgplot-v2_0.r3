...
PLOT_ALT_GRID_Y_DEFAULT_LOG = "lcstv";
// 3D grids
...
convert_gnuformat_to_plot_format = function(s)
{
  ...
  if (length(_ip)==1)
  {
    ...
    if (length(_ilb)==1)
    {
      ...
      ts = 1;         // default text size for printing labels
    ...
    rval.contour = <<>>;
    if (!isempty(zlevels))
    { rval.contour.zlevels = zlevels; }
    if (exist(labels))
    { rval.contour.labels = labels; }
    if (exist(ts))
    { rval.contour.ts = ts; }
    if (exist(sp))
    { rval.contour.sp = sp; }
...
reset_active_plot_object = function ( nx, ny, xleng, yleng, xoff, yoff )
{
  ...
  pobj = PLOT_WINDOWS.[PLOT_ACTIVE_WIN];
...
plwin = function ( N, arg2, arg3)
{
  ...
  else if (length(arg3)==4)
  {
    vp = arg3;
  }
  else
  {
    // left and bottom axis titles
...
pgplot = function ( data )
{
  ...
  pglab(PLOT_WINDOWS.[PLOT_ACTIVE_WIN].xlabel[p], " ", PLOT_WINDOWS.[PLOT_ACTIVE_WIN].title[p]);
  pgmtxt ("L", 3, 0.5, 0.5, PLOT_WINDOWS.[PLOT_ACTIVE_WIN].ylabel[p]);
...
plcont = function ( CL, intval, minint )
{
  ...
  pgsci (PLOT_WINDOWS.[PLOT_ACTIVE_WIN].color[p;2]);          // Set line color.
  pgslw (PLOT_WINDOWS.[PLOT_ACTIVE_WIN].width[p]);            // Set line width.
...
plbar = function ( data )
{
  ...
  for (i in 1:data.nr)          // Loop over each row (X-axis).
  {
    ...
    for (j in 2:data.nc)        // Loop over each column (N-bins per X-value).
    {
      ...
      pgsci (PLOT_WINDOWS.[PLOT_ACTIVE_WIN].color[p;j]);     // Set line color.
      pgrect (x1, x2, y1, y2);
    }
  }
...
plhist = function ( data, nbin )
{
  ...
  pgsls (1);                        // Set line style
  pgslw (1);
  pgscf (PLOT_WINDOWS.[PLOT_ACTIVE_WIN].font[p]);          // Set font.
  ...
  if (PLOT_WINDOWS.[PLOT_ACTIVE_WIN].ymin[p] != 1j) { ymin = PLOT_WINDOWS.[PLOT_ACTIVE_WIN].ymin[p]; }
  if (PLOT_WINDOWS.[PLOT_ACTIVE_WIN].ymax[p] != 1j) { ymax = PLOT_WINDOWS.[PLOT_ACTIVE_WIN].ymax[p]; }
...
plcopy = function ( ID, DEV )
{
  ...
  PLOT_WINDOWS.[PLOT_ACTIVE_WIN].device = DEV;
...
{
  if (!exist (bx)) { PLOT_BASE_X = 2; } else { PLOT_BASE_X = bx; }
  if (!exist (by)) { PLOT_BASE_Y = 2; } else { PLOT_BASE_Y = by; }
  if (!exist (h)) { PLOT_BASE_Z = 4; } else { PLOT_BASE_Z = h; }
};
...
plformat = function ( fmts )
{
  ...
  for (i in 1:length(fmts))
  {
    ...
    if (exist(rval.contour))
    {
      if (exist(rval.contour.zlevels))
      { _zlevels[i] = rval.zlevels; }
    }
    else
    {
      _zlevels[i] = [];
  ...
  PLOT_WINDOWS.[PLOT_ACTIVE_WIN].zlevels.[p] = _zlevels;
  return 0;
};
...
pltext = function (s, loc, vel, opts )
{
  ...
  PLOT_WINDOWS.[PLOT_ACTIVE_WIN].pltext.[i] = ...
      [PLOT_WINDOWS.[PLOT_ACTIVE_WIN].pltext.[i], rval];
...
plot = function ( data, donotclear)
{
  ...
  if (is3dlist(data))
  {
    ...
    _plformat[2] = _z.max;
    _plformat[3] = _z.min;
    _pgprintf(data, _plformat, _logx, _logy, PLOT_WINDOWS.[PLOT_ACTIVE_WIN].zlevels.[p][1]);
  ...
  else if (class (data) == "list")
  {
    ...
    else
    {
      ...
      //  and go along with that assumption
      _plformat = PLOT_WINDOWS.[PLOT_ACTIVE_WIN].plformat.[p];
      _zlevels = PLOT_WINDOWS.[PLOT_ACTIVE_WIN].zlevels.[p];
      _axes = PLOT_WINDOWS.[PLOT_ACTIVE_WIN].axes.[p];
      _cols = PLOT_WINDOWS.[PLOT_ACTIVE_WIN].cols.[p];
    }
      ...
      for (_i in range(_idx_x2y1))
      {
        ...
        else if (is3dlist(data.[_m_i]))
        {
          _pgprintf(data.[_m_i], _plformat[_idx_x2y1[_i];], _logx, _logy, ...
              PLOT_WINDOWS.[PLOT_ACTIVE_WIN].zlevels.[p][_idx_x2y1[_i]]);
        }
        else
        {
      ...
      for (_i in range(_idx_x2y2))
      {
        ...
        else if (is3dlist(data.[_m_i]))
        {
          _pgprintf(data.[_m_i], _plformat[_idx_x2y2[_i];], _logx, _logy, ...
              PLOT_WINDOWS.[PLOT_ACTIVE_WIN].zlevels.[p][_idx_x2y2[_i]]);
        }
        else
        {
      ...
      for (_i in range(_idx_x1y2))
      {
        ...
        else if (is3dlist(data.[_m_i]))
        {
          _pgprintf(data.[_m_i], _plformat[_idx_x1y2[_i];], _logx, _logy, ...
              PLOT_WINDOWS.[PLOT_ACTIVE_WIN].zlevels.[p][_idx_x1y2[_i]]);
        }
        else
        {
      ...
      for (_i in range(_idx_x1y1))
      {
        ...
        if (is3dlist(data.[_m_i]) && (_plformat[_idx_x1y1[_i];1]==3))
        {
          _pgprintf(data.[_m_i], _plformat[_idx_x1y1[_i];], _logx, _logy, ...
              PLOT_WINDOWS.[PLOT_ACTIVE_WIN].zlevels.[p][_idx_x1y1[_i]]);
        }
      }
  ...
  {
    for (_i in 1:length(PLOT_WINDOWS.[PLOT_ACTIVE_WIN].pltext.[p]))
    {
