...
static int rlab_plplot_exit (const char *message)
{
  char * compiler_please_shut_up = (char *) message;
  rerror ((char *)compiler_please_shut_up);
...
Ent * _plot_plprint (int nargs, Datum args[])
{
...
*/
Ent * _plot_plreplot (int nargs, Datum args[])
{
  plreplot ();
...
#undef  THIS_SOLVER
#define THIS_SOLVER "_plprec"
Ent * ent_plplot_plprec (int nargs, Datum args[])
{
  Ent *e1=0;
  PLINT setp=0, prec=0;
  if (nargs != 1)
...
#undef THIS_SOLVER
#define THIS_SOLVER "_plstring"
Ent * ent_plplot_plstring (int nargs, Datum args[])
{
  Ent *e1=0, *e2=0, *e3=0;
  MDR *x=0, *y=0;
  char *s=0;
  ...
  {
    n = MNR(x);
    plstring(n, &Mdr0(x,0,0), &Mdr0(x,0,1), s);
  }
  else if (nargs==3)
  {
    e3 = bltin_get_ent (args[2]);
    if (ent_type(e3)!=MATRIX_DENSE_REAL)
      goto _exit_plstring;
    y = ent_data(e2);
    if ( (EQVECT(x)) && (EQVECT(y)) && (SIZE(x)==SIZE(y)) )
    {
      n = SIZE(x);
      plstring(n, MDRPTR(x), MDRPTR(y), s);
    }
  }
...
#undef THIS_SOLVER
#define THIS_SOLVER "_plstring3"
Ent * ent_plplot_plstring3 (int nargs, Datum args[])
{
  Ent *e1=0, *e2=0, *e3=0, *e4=0;
  MDR *x=0, *y=0, *z=0;
  char *s=0;
  ...
  {
    n = MNR(x);
    plstring3(n, &Mdr0(x,0,0), &Mdr0(x,0,1), &Mdr0(x,0,2), s);
  }
  else if (nargs==4)
  {
    e3 = bltin_get_ent (args[2]);
    ...
    if (ent_type(e4)!=MATRIX_DENSE_REAL)
      goto _exit_plstring;
    z = ent_data(e4);
    if ( (EQVECT(x)) && (EQVECT(y)) && (EQVECT(z)) && (SIZE(x)==SIZE(y)) && (SIZE(x)==SIZE(z)) )
    {
      n = SIZE(x);
      plstring3(n, MDRPTR(x), MDRPTR(y), MDRPTR(z), s);
    }
  }
...
#undef THIS_SOLVER
#define THIS_SOLVER "_plstyl"
Ent * ent_plplot_plstyl (int nargs, Datum args[])
{
  Ent *e1=0, *e2=0;
  MDR *x=0, *y=0;
  int n, do_recast_x=0, do_recast_y=0;
  if ((nargs != 1) && (nargs != 2))
    goto _exit_plstring;
  ...
  {
    n = MNR(x);
    plstyl (n, &Mdi0(x,0,0), &Mdi0(x,0,1));
  }
  else if (nargs==2)
  {
    e2 = bltin_get_ent (args[1]);
...
#undef THIS_SOLVER
#define THIS_SOLVER "_plspal1"
Ent * ent_plplot_plspal1 (int nargs, Datum args[])
{
  Ent *e1=0, *e2=0;
  char *s=0;
  int n=1;
...
#undef THIS_SOLVER
#define THIS_SOLVER "_plspal0"
Ent * ent_plplot_plspal0 (int nargs, Datum args[])
{
  Ent *e1=0;
  char *s=0;
  if ((nargs != 1))
...
#undef THIS_SOLVER
#define THIS_SOLVER "_plsmin"
Ent * ent_plplot_plsmin (int nargs, Datum args[])
{
  Ent *e1=0, *e2=0;
  int scale=1;
  double d=0;
...
#undef THIS_SOLVER
#define THIS_SOLVER "_plsmaj"
Ent * ent_plplot_plsmaj (int nargs, Datum args[])
{
  Ent *e1=0, *e2=0;
  int scale=1;
  double d=0;
...
static void affine_rot_pltr( PLFLT x, PLFLT y, PLFLT *tx, PLFLT *ty, PLPointer * tr )
{
  double *coef = (double *) tr;
  *tx = coef[0] * x + coef[1] * y + coef[2];
  *ty = coef[3] * x + coef[4] * y + coef[5];
}


#undef THIS_SOLVER
#define THIS_SOLVER "_plshades"
Ent * ent_plplot_plshades (int nargs, Datum args[])
{
  Ent *e1=0, *e2=0, *e3=0, *e4=0, *e5=0, *e6=0;
  MDR *a=0, *c=0;
  double xmin,xmax,ymin,ymax;
  double fill_width=2, cont_color=0, cont_width=0.0;
  FILE *rlab_stderr = (!RLAB_STDERR_DS) ? stderr : RLAB_STDERR_DS;
...
//     goto _exit_pls;
  ...
  if (ent_type(e2)!=MATRIX_DENSE_REAL)
    goto _exit_pls;
  xmin = class_double(e2);
  //
  e3 = bltin_get_ent (args[2]);
  if (ent_type(e3)!=MATRIX_DENSE_REAL)
    goto _exit_pls;
  xmax = class_double(e3);
  //
  e4 = bltin_get_ent (args[3]);
  if (ent_type(e4)!=MATRIX_DENSE_REAL)
    goto _exit_pls;
  ymin = class_double(e4);
  //
  e5 = bltin_get_ent (args[4]);
  if (ent_type(e5)!=MATRIX_DENSE_REAL)
    goto _exit_pls;
  ymax = class_double(e5);

  e6 = bltin_get_ent (args[5]);
  if (ent_type(e6)!=MATRIX_DENSE_REAL)
    goto _exit_pls;
  c = ent_data(e6);
  if (!MD_TYPE_DOUBLE(c) || SIZE(c)<2)
  {
    fprintf(rlab_stderr, THIS_SOLVER ": " RLAB_ERROR_ARG6_MDR_VECTOR "\n");
    goto _exit_pls;
  }

  plfshades( (PLF2OPS) plf2ops_grid_col_major(), &zp, zp.nx, zp.ny, NULL,
           xmin, xmax, ymin, ymax,
           (const PLFLT *) MDRPTR(c), SIZE(c),
           fill_width, cont_color, cont_width, plfill, 1, NULL, NULL);
