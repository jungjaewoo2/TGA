...
c  x     : real*8 array of dimension at least (m).
c  y     : real*8 array of dimension at least (m).
c  z     : real*8 array of dimension at least (m).
...
c  w     : real*8 array of dimension at least (m). before entry, w(i) must
c          be set to the i-th value in the set of weights. the w(i) must
c          be strictly positive. unchanged on exit.
c  xb,xe : real*8 values. on entry xb,xe,yb and ye must specify the bound-
...
c          computer representation of a real*8 number is q, then 10**(-q)
...
c  tx    : real*8 array of dimension nmax.
...
c  ty    : real*8 array of dimension nmax.
c          on succesful exit, this array will contain the knots of the
...
c          see also the restrictions (ier=10).
c  c     : real*8 array of dimension at least (nxest-kx-1)*(nyest-ky-1).
...
c  wrk1  : real*8 array of dimension (lwrk1). used as workspace.
...
c  wrk2  : real*8 array of dimension (lwrk2). used as workspace, but
