...
C
      ...
      NYSUB(3) = 1
      NYSUB(4) = 1
      CALL PGBOX('G',30.0,NXSUB,'G',0.2,NYSUB)
      CALL PGSCI(5)
      NXSUB(1) = 3
      CALL PGBOX('ABCTSN',90.0,NXSUB,'ABCTSNV',0.0,NYSUB)
C
...
C-----------------------------------------------------------------------
      REAL PI
      PARAMETER (PI=3.14159265359)
      INTEGER  I, ISEED, NXSUB(4), NYSUB(4)
...
C
      ...
      NYSUB(3) = 1
      NYSUB(4) = 1
      CALL PGBOX('BST', 0.0, NXSUB, ' ', 0.0, NYSUB)
C
C Label the plot.
C
      CALL PGLAB('Variate', ' ',
...
C
      ...
      NYSUB(3) = 1
      NYSUB(4) = 1
      CALL PGBOX ('ABCTS', 90.0, NXSUB, 'ABCTSV', 0.0, NYSUB)
C
C Draw labels
C
      CALL PGSCI (RED)
      CALL PGBOX ('N',90.0, NXSUB, 'VN', 0.0, NYSUB)
C
C Draw SIN line
C
      DO 10 I=1,360
...
C
      ...
      NYSUB(3) = 1
      NYSUB(4) = 1
      CALL PGBOX ('ABCTSM', 60.0, NXSUB, 'VABCTSM', 1.0, NYSUB)
C
C Draw SIN line
C
      CALL PGSCI (WHITE)
