...
C-----------------------------------------------------------------------
      INTEGER I,J,NXSUB(4),NYSUB(4),CPARAM(6)
      REAL F(40,40),FMIN,FMAX,ALEV(23),TR(6)
      DATA TR/0.,1.,0.,0.,0.,1./
C
...
C
      ...
      NYSUB(3) = 1
      NYSUB(4) = 1
      CALL PGBOX('bcts',0.0,NXSUB,'bcts',0.0,NYSUB)
...
C
      ...
      CPARAM(6) = 1
      DO 30 I=1,21
          ALEV(I) = FMIN + (I-1)*(FMAX-FMIN)/20.0
   30 CONTINUE
      ALEV(22) = FMIN
      ALEV(23) = FMAX
      CALL PGCONT(F,40,40,1,40,1,40,ALEV,21,TR,CPARAM)
      CALL PGSLW(1)
      CALL PGSLS(1)
      CALL PGSCI(1)
      CALL PGEBUF
...
C         WRITE (LABEL,'(F8.2)') ALEV
          CALL PGCONL(F,40,40,1,40,1,40,ALEV,21,TR,CPARAM,
     1                I,LABEL,1.5,32,16)
 50   CONTINUE
      CALL PGSCI(1)
      CALL PGEBUF
...
C
      CALL PGSCI(3)
      CALL PGCONT(Z,NX,NY,1,NX,1,NY,C,NC,TR,CPARAM)
C
C Labels and box.
C
      CALL PGSCI(1)
      ...
      NYSUB(3) = 1
      NYSUB(4) = 1
      CALL PGBOX('bctsin',1.0,NXSUB,'bctsinv',1.0,NYSUB)
      CALL PGSCH(1.0)
