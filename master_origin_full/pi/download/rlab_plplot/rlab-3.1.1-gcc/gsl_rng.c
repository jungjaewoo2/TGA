...
/**
  * rlabplus:
  */
struct _rlab_irand rlab_gsl_irand[RLAB_IRNG_MAXNO] =
{
  {.index=-1, .generator=NULL, .seed=-1.0, .offset=0, .range=0, .min_val=-1, .max_val=-1}
};
...
bltin_iran[] =
{
  // index, name, would have put the pointer to the generator but cannot do it
  {-1, "none" },
  { 1, "borosh13" },
  { 2, "cmrg" },
  { 3, "coveyou" },
...
//
unsigned long int iseed_uran (void)
{
  unsigned long int seed = 0;
  // getting the random number seed from /dev/urandom
  FILE *fh = (FILE *) fopen ("/dev/urandom", "r");
  fread (&seed, sizeof (seed), 1, fh);
  fclose (fh);
  return seed;
}

int rlab_setup_default_gsl_irng (int idx)
{
  if (rlab_gsl_irand[idx].index < 1)
  {
    //
    // setting up new generator
    //
    rlab_gsl_irand[idx].index = RLAB_IRNG_DEFAULT_INDEX;
    rlab_gsl_irand[idx].generator = gsl_rng_alloc(RLAB_GSL_IRNG_DEFAULT);
    gsl_rng_set(rlab_gsl_irand[idx].generator, iseed_uran());
    rlab_gsl_irand[idx].offset = 0;
    rlab_gsl_irand[idx].range  = 0;
    rlab_gsl_irand[idx].min_val = gsl_rng_min(rlab_gsl_irand[idx].generator);
    rlab_gsl_irand[idx].max_val = gsl_rng_max(rlab_gsl_irand[idx].generator);
...
ent_gsl_irng (int nargs, Datum args[])
{
  ...
  char *s1=0;
  ListNode *node=0;
  int i, n, ix, ifoundit=0, index=-1;

  double dseed=create_nan(),minval=create_nan(),maxval=create_nan(), range=0, offset=0;
  ...
  if (nargs==0)
  {
    ...
    for (i=0; i<RLAB_IRNG_MAXUSERNO; i++)
    {
      if (rlab_gsl_irand[i].index > 0)
      {
        ...
        fprintf(stdout, "%3i: %s, size = %6i\n", i+1,
                gsl_rng_name (rlab_gsl_irand[i].generator),
                              (int) gsl_rng_size (rlab_gsl_irand[i].generator)
               );
      }
    }
    for (i=RLAB_IRNG_MAXUSERNO; i<RLAB_IRNG_MAXNO; i++)
    {
      if (rlab_gsl_irand[i].index > 0)
      {
        n = gsl_rng_size (rlab_gsl_irand[i].generator);
        fprintf(stdout, " system %3i: %s, size = %6i\n",
                i+1, gsl_rng_name(rlab_gsl_irand[i].generator),
                n);
      }
    }
  ...
  //
  // two arguments  : irng(I,opts)
  //
  e2 = bltin_get_ent (args[1]);
  if (ent_type (e2) != BTREE)
  {
    fprintf(stdout, THIS_SOLVER ": list of options <<name;seed;offset;range>> expected !\n");
  ...
  //
  RLABCODE_PROCESS_BTREE_ENTRY_S(e2,node,RLAB_NAME_RNG_NAME,s1,1,"?");
  if (!s1)
    rerror (THIS_SOLVER ": Bad name!\n" );

  if (!strcmp (s1, "help") || !strcmp (s1, "?"))
  {
    fprintf (stdout, "irng: names of the available integer random generators"
        " (second argument):\nirng: ");
  ...
  for (i = 0; bltin_iran[i].index && !ifoundit; i++)
  {
    if (!strcmp (s1, bltin_iran[i].name))
    {
      index = bltin_iran[i].index;
      ifoundit=1;
    }
  }

  // wrong name. what do we do?
  if (!ifoundit)
  {
    fprintf(stdout, THIS_SOLVER ": Use <<name=\"?\">> to get the names of available generators!\n");
    rerror (THIS_SOLVER ": Bad name!\n" );
  }
  ...
  // what values to use?
  //
  if (rlab_gsl_irand[ix].index == index)
  {
    // use previous values if the generator previously instantiated
    minval = rlab_gsl_irand[ix].min_val;
    maxval = rlab_gsl_irand[ix].max_val;
  }
  else
  {
    rlab_gsl_irand[ix].index = index;
    if(rlab_gsl_irand[ix].index > 0)
    {
      gsl_rng_free(rlab_gsl_irand[ix].generator);
      rlab_gsl_irand[ix].seed = -1;
      rlab_gsl_irand[ix].min_val = -1;
      rlab_gsl_irand[ix].max_val = -1;
    }
  ...
  RLABCODE_PROCESS_BTREE_ENTRY_DOUBLE(e2,node,RLAB_NAME_RNG_SEED,dseed);
  RLABCODE_PROCESS_BTREE_ENTRY_DOUBLE(e2,node,RLAB_NAME_GEN_MIN,minval);
  RLABCODE_PROCESS_BTREE_ENTRY_DOUBLE(e2,node,RLAB_NAME_GEN_MAX,maxval);
  RLABCODE_PROCESS_BTREE_ENTRY_DOUBLE(e2,node,"range",range);
  RLABCODE_PROCESS_BTREE_ENTRY_DOUBLE(e2,node,"offset",offset);

  if (isnand(minval))
  {
    minval = gsl_rng_min(rlab_gsl_irand[ix].generator);
  }
  if (isnand(maxval))
  {
    maxval = gsl_rng_max(rlab_gsl_irand[ix].generator);
  }
  if (isnand(dseed))
  {
    if (rlab_gsl_irand[ix].seed == -1)
    {
      dseed = iseed_uran ();
    }
  }
  if (!isnand(dseed))
  {
    gsl_rng_set(rlab_gsl_irand[RLAB_IRNG_DEFAULT].generator, (unsigned long int) dseed);
    rlab_gsl_irand[ix].seed = dseed;
  }
  rlab_gsl_irand[RLAB_IRNG_DEFAULT].range  = range;
  rlab_gsl_irand[RLAB_IRNG_DEFAULT].offset = offset;

  //
  // adjust the default generator if it got erased with "none"
  // start from the top; assign 0 if nothing found
  //
  if (rlab_gsl_irand[RLAB_IRNG_DEFAULT].index < 1)
  {
    for (i = RLAB_IRNG_MAXUSERNO; i>0; i--)
    {
      RLAB_IRNG_DEFAULT = i-1;
      if (rlab_gsl_irand[RLAB_IRNG_DEFAULT].index>0)
        break;
    }
  }
...
ent_gsl_irng_state (int nargs, Datum args[])
{
  ...
  if (nargs == 1)
  {
    ...
    s1     = (char*) gsl_rng_name (rlab_gsl_irand[RLAB_IRNG_DEFAULT].generator);
    n      = gsl_rng_size (rlab_gsl_irand[RLAB_IRNG_DEFAULT].generator);
    state  = gsl_rng_state(rlab_gsl_irand[RLAB_IRNG_DEFAULT].generator);
  ...
  rlab_gsl_irand[ix].index = irng;
  n      = gsl_rng_size (rlab_gsl_irand[ix].generator);
  state  = gsl_rng_state(rlab_gsl_irand[ix].generator);
...
//
#undef  THIS_SOLVER
#define THIS_SOLVER "irand"
Ent * ent_gsl_irand (int nargs, Datum args[])
{
  Ent *e1=0, *e2=0;
  int nr=1, nc=1, i, n;
  MDR *w=0;
  ...
  w = mdi_Create (nr, nc);
  n = nr * nc;
  if (rlab_gsl_irand[RLAB_IRNG_DEFAULT].range)
  {
    if (rlab_gsl_irand[RLAB_IRNG_DEFAULT].offset)
    {
      for (i = 0; i < n; i++)
        MdiV0 (w, i) = rlab_gsl_irand[RLAB_IRNG_DEFAULT].offset
            + (int) gsl_rng_uniform_int(rlab_gsl_irand[RLAB_IRNG_DEFAULT].generator,
               rlab_gsl_irand[RLAB_IRNG_DEFAULT].range);
    }
    else
    {
      for (i = 0; i < n; i++)
        MdiV0 (w, i) = (int) gsl_rng_uniform_int(rlab_gsl_irand[RLAB_IRNG_DEFAULT].generator,
               rlab_gsl_irand[RLAB_IRNG_DEFAULT].range);
    }
  }
  else
  {
    for (i = 0; i < n; i++)
      MdiV0 (w, i) = (int) gsl_rng_get (rlab_gsl_irand[RLAB_IRNG_DEFAULT].generator);
  }
...
//
int rlab_setup_rng (int *idx_rng, int *idist, int *nparam, double *param, int *irng)
{
  int i;

  // does irng exist? if not set it up
  ...
  rlab_bltin_rng[*idx_rng].idist  = *idist;
  rlab_bltin_rng[*idx_rng].irng   = *irng;
  if (rlab_bltin_rng[*idx_rng].nparam != *nparam)
  {
    // reallocate memory for param arrays
    if (rlab_bltin_rng[*idx_rng].param)
      GC_free(rlab_bltin_rng[*idx_rng].param);

    // create new param array
    rlab_bltin_rng[*idx_rng].param
      = (double *) GC_MALLOC(*nparam * sizeof(double));

    rlab_bltin_rng[*idx_rng].nparam = *nparam;
  }

  // copy
  for (i=0; i<*nparam;i++)
    rlab_bltin_rng[*idx_rng].param[i] = param[i];

  return 1;
}


void gsl_random_number_generator (int *nr, int *nc, double *w, int *idx)
{
  int i, n = (*nr) * (*nc);
  int idrng, irng;
  ...
  if (rlab_bltin_rng[idrng].idist<1)
  {
    ...
    rlab_setup_rng (&idrng, &idist, &nparam, dparam, &RLAB_IRNG_DEFAULT);
  }
  ...
  switch (idist)
  {
    ...
    case  1:
      for (i = 0; i < n; i++)
        w[i] = param[0]
            + gsl_ran_gaussian (rlab_gsl_irand[irng].generator, param[1]);
      break;
    ...
    case  2:
      for (i = 0; i < n; i++)
        w[i] =
            param[0]
            + gsl_ran_gaussian_tail (rlab_gsl_irand[irng].generator,
                                     param[2] - param[0],
                                     param[1]);
      break;
    ...
    case  3:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_exponential (rlab_gsl_irand[irng].generator, param[0]);
      break;

    case  4:
      for (i = 0; i < n; i++)
        w[i] = param[0]
            + gsl_ran_laplace (rlab_gsl_irand[irng].generator, param[1]);
      break;

    case  5:
      for (i = 0; i < n; i++)
        w[i] = param[0]
            + gsl_ran_exppow (rlab_gsl_irand[irng].generator, param[1], param[2]);
      break;

    case  6:
      for (i = 0; i < n; i++)
        w[i] = param[0]
            + gsl_ran_cauchy (rlab_gsl_irand[irng].generator, param[1]);
      break;

    case  7:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_rayleigh (rlab_gsl_irand[irng].generator, param[0]);
      break;

    case  8:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_rayleigh_tail (rlab_gsl_irand[irng].generator, param[0], param[1]);
      break;

    case  9:
      for (i = 0; i < n; i++)
        w[i] = param[0]
            + gsl_ran_levy (rlab_gsl_irand[irng].generator, param[1], param[2]);
      break;

    case 10:
      for (i = 0; i < n; i++)
        w[i] = param[0]
            + gsl_ran_levy_skew (rlab_gsl_irand[irng].generator,
                                 param[1], param[2], param[3]);
      break;

    case 11:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_gamma (rlab_gsl_irand[irng].generator, param[0], param[1]);
      break;

    case 12: case 13:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_flat (rlab_gsl_irand[irng].generator, param[0], param[1]);
      break;

    case 14:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_lognormal (rlab_gsl_irand[irng].generator, param[0], param[1]);
      break;

    case 15:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_chisq (rlab_gsl_irand[irng].generator, param[0]);
      break;

    case 16:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_fdist (rlab_gsl_irand[irng].generator, param[0], param[1]);
      break;

    case 17:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_tdist (rlab_gsl_irand[irng].generator, param[0]);
      break;

    case 18:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_beta (rlab_gsl_irand[irng].generator, param[0], param[1]);
      break;

    case 19:
      for (i = 0; i < n; i++)
        w[i] = param[0]
            + gsl_ran_logistic (rlab_gsl_irand[irng].generator, param[1]);
      break;

    case 20:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_pareto (rlab_gsl_irand[irng].generator, param[0], param[1]);
      break;

    case 21:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_weibull (rlab_gsl_irand[irng].generator, param[0], param[1]);
      break;

    case 22:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_gumbel1 (rlab_gsl_irand[irng].generator, param[0], param[1]);
      break;

    case 23:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_gumbel2 (rlab_gsl_irand[irng].generator, param[0], param[1]);
      break;

    case 24:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_poisson (rlab_gsl_irand[irng].generator, param[0]);
      break;

    case 25:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_binomial (rlab_gsl_irand[irng].generator, param[0], param[1]);
      break;

    case 26:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_negative_binomial (rlab_gsl_irand[irng].generator, param[0], param[1]);
      break;

    case 27:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_geometric (rlab_gsl_irand[irng].generator, param[0]);
      break;

    case 28:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_hypergeometric (rlab_gsl_irand[irng].generator,
                                       param[0], param[1], param[2]);
      break;

    case 29:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_logarithmic (rlab_gsl_irand[irng].generator, param[0]);
  }
  return;
}
...
gsl_random_number_generator_int (int *nr, int *nc, unsigned int *w, int *idx)
{
  ...
  switch (idist)
  {
    ...
    case 25:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_binomial (rlab_gsl_irand[irng].generator,
                                 param[0], param[1]);
      break;

    case 26:
      for (i = 0; i < n; i++)
        w[i] = gsl_ran_negative_binomial (rlab_gsl_irand[irng].generator,
                                          param[0], param[1]);
      break;
...
gslrngnf_ ( void )
{
  ...
  return gsl_rng_uniform (rlab_gsl_irand[irng].generator);
}
...
gslrngns_ (int *nr, int *nc, double *w)
{
  ...
  if (w)
  {
    for (i=0; i<*nr; i++)
      for (j=0; j<*nc; j++)
        w[i +(*nr)*j]= gsl_ran_gaussian (rlab_gsl_irand[irng].generator, 1.0);
    return 0;
  }
...
// uniform matrix
#undef  THIS_SOLVER
#define THIS_SOLVER "gslrngus_"
int gslrngus_ (int *nr, int *nc, double *w)
{
  int irng = RLAB_IRNG_UNIFORM - 1;
  int idx  = RLAB_RNG_UNIFORM - 1;
  int i, j;
  ...
  if (w)
  {
    for (i=0; i<*nr; i++)
      for (j=0; j<*nc; j++)
        w[i +(*nr)*j]= gsl_rng_uniform (rlab_gsl_irand[irng].generator);
...
int gslrngb_ ( int n, double p )
{
  ...
  return gsl_ran_binomial (rlab_gsl_irand[RLAB_IRNG_STAT].generator, p, n);
}

#undef THIS_SOLVER
#define THIS_SOLVER "rng"
Ent * ent_gsl_rng (int nargs, Datum args[])
{
  Ent *e1=0, *e2=0, *e3=0, *e4=0;
  MDR *p=0;
  int i, irng, idist, npar, idx;
  char *s1=0;
  ...
  //
  if (nargs != 4 && nargs!=1 && nargs!=3)
    rerror(THIS_SOLVER ": " RLAB_ERROR_ONE_OR_THREE_OR_FOUR_ARG_REQUIRED "\n");
...
*/
  ...
  e1 = bltin_get_ent (args[0]);
  if (ent_type(e1)!=MATRIX_DENSE_REAL)
    rerror(THIS_SOLVER ": " RLAB_ERROR_ARG1_RNG_INDEX_OUT_OF_BOUNDS "\n");
  idx = (int) class_double (e1);
  if (idx < 1 || idx > RLAB_RNG_MAXNO)
  {
    rerror(THIS_SOLVER ": " RLAB_ERROR_ARG1_RNG_INDEX_OUT_OF_BOUNDS "\n");
  }
  idx--;
  ...
  //
  RLABCODE_PROCESS_ARG_MD_IGNORE_ERROR(THIS_SOLVER,1,e2,MATRIX_DENSE_STRING,s1,class_char_pointer,isvalidstring,<,1,NULL);
  if (!s1)
    goto _exit_rng;
...
*/
  ...
  if (!idist)
  {
    rerror ("rng: distribution name cannot be recognized!\n");
  }
...
#undef THIS_SOLVER
#define THIS_SOLVER "rand"
Ent * ent_gsl_rand (int nargs, Datum args[])
{
  // call parameters:
  //  e1   - rng name
  //  [e2] - seed
  ...
  // no arguments: return a single rng,
  // for one argument return random number matrix of the same size from the
  // default rng
  if (nargs == 1)
  {
    e1 = bltin_get_ent (args[0]);
...
#undef  THIS_SOLVER
#define THIS_SOLVER "uniform"
Ent * ent_gsl_urand (int nargs, Datum args[])
{
  // call parameters:
  //  e1   - rng name
  //  [e2] - seed
...
ent_gsl_shuffle (int nargs, Datum args[])
{
  ...
  gsl_ran_shuffle (rlab_gsl_irand[irng].generator, idx, n, sizeof (int));
...
ent_gsl_sample (int nargs, Datum args[])
{
  ...
  if (ent_type (e1) == MATRIX_DENSE_REAL)
  {
    ...
    gsl_ran_sample (rlab_gsl_irand[irng].generator, sidx, ns, idx, n, sizeof (int));
...
ent_gsl_drand (int nargs, Datum args[])
{
      ...
      for (j = 0; j < nc; j++)
        Mds0 (ws, i, j) =
            cpstr (MdsV0 (drng_string[igen],
                   gsl_ran_discrete (rlab_gsl_irand[irng].generator, drng_g[igen])));
  ...
  if (drng_idx[igen] == 2 && drng_real[igen])
  {
    ...
    if (drng_real[igen]->type == RLAB_TYPE_INT32)
    {
      ...
      for (i = 0; i < nr; i++)
        for (j = 0; j < nc; j++)
          Mdi0 (wr, i, j) =
              MdiV0 (drng_real[igen], gsl_ran_discrete (rlab_gsl_irand[irng].generator, drng_g[igen]));
    }
    else
    {
      // real
      ...
      for (i = 0; i < nr; i++)
        for (j = 0; j < nc; j++)
          Mdr0 (wr, i, j) =
              MdrV0 (drng_real[igen], gsl_ran_discrete (rlab_gsl_irand[irng].generator,
                     drng_g[igen]));
    }
    return ent_Assign_Rlab_MDR(wr);
  }
  if (drng_idx[igen] == 3 && drng_complex[igen])
  {
    ...
    for (i = 1; i <= nr; i++)
      for (j = 1; j <= nc; j++)
      {
        i1 = gsl_ran_discrete (rlab_gsl_irand[irng].generator, drng_g[igen]);
...
ent_gsl_hrand (int nargs, Datum args[])
{
  ...
  if (hrng_g[last_hrng])
  {
    ...
    {
      r = gsl_rng_uniform(rlab_gsl_irand[irng].generator);
