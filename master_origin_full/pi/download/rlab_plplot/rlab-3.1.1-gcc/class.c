...
static OpDef matrix_double_method[NCL];
...
class_init (void)
{
 ...
 /*
   ...
   * Matrix-Double value.
   */
  matrix_double_method[MATRIX_DENSE_REAL].type = 0;
  matrix_double_method[MATRIX_DENSE_REAL].op = (void *) mdr_MatrixDouble;
...
Complex class_complex (Ent * e)
{
  ...
  if (!e)
    rerror ("coerce-to-complex operation not supported");
...
MDR * class_matrix_double (Ent * e)
{
  ...
  type = ent_type (e);
  vfptr = (VFPTR) matrix_double_method[type].op;

  if (vfptr == 0)
  {
    fprintf (stderr, "Entity type: %s\n", etd (e));
    rerror ("coerce-to-matrix-double operation not supported");
  }
...
Ent * class_append (Ent * e1, Ent * e2)
{
  ...
  {
    int nr1, nc1, nr2, nc2, nr=0, nc=0, i, j;
