...
MDC * mdc_Create_SameSize (MD *x)
{
  ...
  if (EQNULL(x))
    new = md_Create (0, 0, RLAB_TYPE_COMPLEX);
  else
    new = md_Create (nrow, ncol, RLAB_TYPE_COMPLEX);
...
Complex mdc0_safe(MD *m, int k, int j)
{
...
Complex mdc1_safe(MD *m, int k, int j)
{
  k = MAX(MIN(MNR(m), k),1);
  j = MAX(MIN(MNC(m), j),1);
...
double mdcV0r(MD *m, int k)
{
  Complex cval=mdcV0(m,k);
  return RE(cval);
}

double mdcV0i(MD *m, int k)
{
  Complex cval=mdcV0(m,k);
  return IM(cval);
}
...
double mdcV1r(MD *m, int k)
{
  Complex cval=mdcV1(m,k);
  return RE(cval);
}

double mdcV1i(MD *m, int k)
{
  Complex cval=mdcV1(m,k);
  return IM(cval);
}
