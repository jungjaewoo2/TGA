...
ent_bspline_fit (int nargs, Datum args[])
{
  ...
  if (nargs > 3)
  {
    ...
    {
      // knot
      node = btree_FindNode (ent_data(e4), RLAB_NAME_DIERCKX_KNOT);
      if (node != 0)
      {
        if (ent_type(var_ent (node)) == MATRIX_DENSE_REAL)
        {
          x3 = (MDR *) ent_data (var_ent (node));
          if (!MD_TYPE_DOUBLE(x3))
          {
            printf(THIS_SOLVER ": options '"RLAB_NAME_DIERCKX_KNOT"' must be real vector!\n");
            x3=0;
          }
        }
      }
      ...
      // smooth
      node = btree_FindNode (ent_data(e4), RLAB_NAME_DIERCKX_SMOOTH);
      if (node != 0)
      {
        ddummy = class_double (var_ent (node));
        if (ddummy > 0)
