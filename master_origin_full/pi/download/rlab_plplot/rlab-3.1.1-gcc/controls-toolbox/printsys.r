...
//
//----------------------------------------------------------------------
require abcdchk poly2str printmat tfchk
...
printsys = function(a,b,c,d,ulab,ylab,xlab)
{
  ...
  if (nargs==2) {		
    ...
    num = T.numc;
    den = T.denc;
    if (!isstring(c)) { error("Transform variable must be a string."); }
    tvar = c;
  else if (nargs==4) {
    ...
    nb = b.nr;
    nu = b.nc;
    if (!isstring(ulab)) { error("ULAB must be a string."); }
    if (!isstring(ylab)) { error("YLAB must be a string."); }
    if (!isstring(xlab)) { error("XLAB must be a string."); }
