...
#include "buffer.h"
char buffer[SERIAL_MAX_NUM_BYTES];
...
#include <stdarg.h>
#undef THIS_FUNCTION
#define THIS_FUNCTION "get_rfile_ds"
...
#ifdef HAVE_SQLITE
  if (rftype == RFILE_SQLITE)
  {
    int status;
    char *real_name=0;
    //
    // operations with files and pipes:
    //    rf = get_rfile_ds(fname, RFILE_SQLITE, flags)
    //
    va_start(ap, rftype);
    rf->sql_flags = va_arg(ap, int);     // argument No. 3
    va_end(ap);

    // ignore 'sqlite://' in front of the file name
    if (!strncmp(name, "sqlite://", 9))
      real_name = &name[9];
    else
      real_name = name;
    ...
    status = sqlite3_open_v2(real_name, &rf->sql_db, rf->sql_flags, NULL);
    if (status != SQLITE_OK)
    {
      printf(THIS_FUNCTION ": Failed!\n");
      rfile_Destroy (name);
      return NULL;
    }
...
Ent * ReadM (int nargs, Datum args[])
{
  ...
  RLABCODE_CHECK_IF_FILENAME_EXISTS(rf,fname,close_after_rw);
...
//               ent_Clean (P);
  ...
  else if (rf->filetype == RFILE_CURL)
  {
    ...
    rfile_curl_config (rf, fname, e2);
    curl_easy_setopt(rf->curl, CURLOPT_CUSTOMREQUEST, "GET");
    if (!rf->curl_write_fd)
    {
    ...
    // close file to which readm was writing
    if (rf->curl_write_fd)
    {
      fflush(rf->curl_write_fd);
      fclose(rf->curl_write_fd);
      rf->curl_write_fd = NULL;
    }
    else
    {
      GC_free (rf->buffer);
  ...
  int min_line_len=1, iskiprows=0, join_rows=1, skipdatarows=0;
  MDS *start=0, *stop=0;
  ...
  if (nargs >= 2)
  {
    ...
    else if (ent_type (e2) == BTREE)
    {
      ...
      RLABCODE_PROCESS_BTREE_ENTRY_S(e2,node,RLAB_NAME_READM_CSP,delim,1,0);

      RLABCODE_PROCESS_BTREE_ENTRY_F_S(e2,node,RLAB_NAME_READM_MIN_LINE_LEN,min_line_len,class_int,1,1);

      RLABCODE_PROCESS_BTREE_ENTRY_F_S(e2,node,RLAB_NAME_READM_JOINROWS,join_rows,class_int,1,1);

      RLABCODE_PROCESS_BTREE_ENTRY_S(e2,node,RLAB_NAME_READM_JOINCSP,join_csp,1,0);

      RLABCODE_PROCESS_BTREE_ENTRY_MD(e2,node,RLAB_NAME_READM_COMMENT,comment,MDS,SIZE,1,0);

      RLABCODE_PROCESS_BTREE_ENTRY_MD(e2,node,RLAB_NAME_READM_NOTE,note,MDS,SIZE,1,0);

      RLABCODE_PROCESS_BTREE_ENTRY_MD(e2,node,RLAB_NAME_READM_LSTRIP,lstrip,MDS,SIZE,1,0);

      RLABCODE_PROCESS_BTREE_ENTRY_MD(e2,node,RLAB_NAME_READM_START,start,MDS,SIZE,1,0);

      RLABCODE_PROCESS_BTREE_ENTRY_MD(e2,node,RLAB_NAME_READM_STOP,stop,MDS,SIZE,1,0);

      RLABCODE_PROCESS_BTREE_ENTRY_F_S(e2,node,RLAB_NAME_READM_SKIPROWS,iskiprows,class_int,1,0);

      RLABCODE_PROCESS_BTREE_ENTRY_MD(e2,node,RLAB_NAME_READM_USEROWS,userows,MDR,SIZE,1,0);

      RLABCODE_PROCESS_BTREE_ENTRY_MD(e2,node,RLAB_NAME_READM_USECOLS,usecols,MDR,SIZE,1,0);

      RLABCODE_PROCESS_BTREE_ENTRY_MD(e2,node,RLAB_NAME_READM_GREP,grep,MDS,SIZE,1,0);

      RLABCODE_PROCESS_BTREE_ENTRY_F_S(e2,node,RLAB_NAME_READM_SKIPDATAROWS,skipdatarows,class_int,1,0);
    }
  }
  ...
  if ((m = mdr_ReadGeneric (rf->fileds_f, block_size, iskiprows, delim, min_line_len,
       ...
       usecols, start, stop, skipdatarows)) == 0)
...
Ent * WriteM (int nargs, Datum args[])
{
  ListNode *node=0;
  char *name;
  int type;
  ...
  if (!rf)
  {
    ...
    {
      // open database rw and create it if it doesn't already exist
      rf = get_rfile_ds (name, RFILE_SQLITE,
                         SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE);
      // then delete its previous content, because it will be closed after this
      // operation
      sql_delete_all_tables(rf);
    }
#endif
  ...
  e2 = bltin_get_ent (args[1]);
  type = ent_type (e2);
  if ((type == UNDEF) && rf->filetype != RFILE_H5 && rf->filetype != RFILE_CURL)
  ...
  if (rf)
  {
    ...
    else if (rf->filetype == RFILE_CURL)
    {
      ...
      MDR *w=0;
      ListNode *node=0;
      int istatus=0, ido=0, iwhat=0;
      CURLcode res;
      char *method=0;

      if (e2)
      {
        // process curl arguments unrelated to what is actually being transferred
        istatus = rfile_curl_config (rf, name, e2);

        // did user specify method:
        //  PUT, POST, DELETE
        // are supported
        RLABCODE_PROCESS_BTREE_ENTRY_S(e2,node,"CURLOPT_CUSTOMREQUEST",method,1,NULL);
        if (method)
        {
          if (!strcmp(method, "delete") || !strcmp(method, "DELETE"))
            iwhat = 1;
          else if (!strcmp(method, "put") || !strcmp(method, "PUT"))
            iwhat = 2;
        ...
        if (!iwhat)
          RLABCODE_PROCESS_BTREE_ENTRY_BOOL(e2,node,"CURLOPT_DELETE",ido);
        if (ido)
        {
          iwhat = 1;
          ido = 0;
        ...
        if (!iwhat)
          RLABCODE_PROCESS_BTREE_ENTRY_BOOL(e2,node,"CURLOPT_PUT",ido);
        if (ido)
        {
          iwhat = 2;
          ido = 0;
        ...
        if (!iwhat)
          RLABCODE_PROCESS_BTREE_ENTRY_BOOL(e2,node,"CURLOPT_UPLOAD",ido);
        if (ido)
        {
          iwhat = 2;
          ido = 0;
        ...
        if (!iwhat)
          RLABCODE_PROCESS_BTREE_ENTRY_BOOL(e2,node,"CURLOPT_POST",ido);
        if (ido)
        {
          iwhat = 3;
          ido = 0;
        ...
        if (iwhat!=1)
        {
          // is file that will be written to url provided?
          // if not, then third argument has to be provided which is the string matrix
        ...
        switch (iwhat)
        {
          case 1:
            // delete
            printf("delete\n");
            istatus |= curl_easy_setopt(rf->curl, CURLOPT_CUSTOMREQUEST, "DELETE");
            break;

          case 2:
            // put
            printf("put\n");
            istatus |= curl_easy_setopt(rf->curl, CURLOPT_UPLOAD, 1L);
            if (s)
            {
              curl_easy_setopt(rf->curl, CURLOPT_READDATA, s);
          ...
          case 3:
            // post
            printf("post\n");
            istatus |= curl_easy_setopt(rf->curl, CURLOPT_POST, 1L);
            if (s)
            {
              curl_easy_setopt(rf->curl, CURLOPT_POSTFIELDS, s);
            }
        }
      }
      res = curl_easy_perform(rf->curl);
...
#endif
#ifdef HAVE_SQLITE
    else if (rf->filetype == RFILE_SQLITE)
    {
      // writing a string matrix to a database connection
      MDS *s=0;
      int nr=0, nc=0, rval=0, i, j, rc;
      sqlite3_stmt *res;
      ...
      if (nargs == 2)
      {
        //  writem(db, statements)
        //    where 'statements' is string array of valid SQL statements
        //    that are complete (no parameters and such), and no reply
        //    is expected besides whether their execution was successful
        //    or not
        for(i=0; i<nr*nc; i++)
        {
          if (isvalidstring(MdsV0(s,i))<5)
            continue;

          rc = sqlite3_prepare_v2(rf->sql_db, MdsV0(s,i), -1, &res, 0);
          if (rc != SQLITE_OK)
            continue;
          ...
          if (rc != SQLITE_DONE)
          {
            printf(THIS_SOLVER ": " "Execution of SQLite statement '%s' ",
                   MdsV0(s,i));
      ...
      {
        //  writem(db, table, col_name1, val1, ...)
        char * table = class_char_pointer(e2);
        if (isvalidstring(table)<1)
          goto _clean_sqlite;

        // total number of arguments has to be greater than 2, and even number,
        // 4,6, ...
        ...
        if (close_after_rw)
        {
          // if db was not opened earlier, then drop the table if it exists
          sql_drop_table_if_exists(rf, table);
        }

        // now go over remaining arguments and add them to the table
        sprintf(string_buff, "INSERT OR REPLACE INTO %s (", table);
        for (i=2; i<nargs; i+=2)
        {
          Ent * ecol = bltin_get_ent(args[i]);

          sprintf(string_buff + isvalidstring(string_buff),
                  "%s", class_char_pointer(ecol));
          ...
          if (i+2<nargs)
            ...
            sprintf(string_buff + isvalidstring(string_buff),") VALUES (");
        }
        for (i=3; i<nargs; i+=2)
        {
          sprintf(string_buff + isvalidstring(string_buff), "?");
        ...
        // this statement can be prepared now
        rc = sqlite3_prepare_v2(rf->sql_db, string_buff, -1, &res, 0);
        if (rc != SQLITE_OK)
          goto _clean_sqlite;
        ...
        while (1)
        {
          ...
          {
            Ent * ecol = bltin_get_ent(args[i]);
            if (ent_type(ecol)==MATRIX_DENSE_STRING)
            {
              MDS *m = ent_data(ecol);
              rc = sqlite3_bind_text(res, (i-1)/2, mdsV0_safe(m, j),-1,SQLITE_TRANSIENT);
              if (j>=SIZE(m))
                done++;
            }
            else if (ent_type(ecol)==MATRIX_DENSE_REAL)
            {
              MDR *m = ent_data(ecol);
              if (m->type == RLAB_TYPE_DOUBLE)
              {
                rc = sqlite3_bind_double(res, (i-1)/2, mdrV0_safe(m, j));
              }
              else if (m->type == RLAB_TYPE_INT32)
              {
                rc = sqlite3_bind_int(res, (i-1)/2, mdiV0_safe(m, j));
              }
              if (j>=SIZE(m))
                done++;
            }
          ...
          if( rc != SQLITE_DONE)
          {
            printf(THIS_SOLVER ": " "Execution of 'sqlite3_step' returned %s (%i)\n",
                   sqlite3_errmsg(rf->sql_db), rc);
            rval = 1;
            break;
          }
...
#undef  THIS_SOLVER
#define THIS_SOLVER "fread"
Ent * Fread (int nargs, Datum args[])
{
  double double_tmp;
  char *fnstring, *type;
  char *term_char=0;
  ...
  // process filename
  RLABCODE_PROCESS_ARG1_S(THIS_SOLVER,0,e1,fnstring,1);
  ...
  else if (!strcmp (type, "char") || !strcmp (type, "schar")
            || !strcmp (type, "int8_t") || !strcmp (type, "integer*1") )
  {
    char char_tmp;
    if (nitems == -1)
    {
  ...
  else if ( (!strcmp (type, "short int"))  || (!strcmp (type, "int16_t"))
            || (!strcmp (type, "int16")) || (!strcmp (type, "integer*2")) )
  {
    short int int_tmp;
    if (nitems == -1)
    {
  ...
  else if ( (!strcmp (type, "int"))  || (!strcmp (type, "int32_t"))
              || (!strcmp (type, "int32")) || (!strcmp (type, "integer*4")) )
  {
    int int_tmp;
    if (nitems == -1)
    {
      ...
      if (count < nitems)
      {
        ...
        else
        {
          ...
          m = mdr_Create(0,0);
              ...
              ||  (!strcmp (type, "single"))  || (!strcmp (type, "real*4")) )
  {
    float real_tmp;
    if (nitems == -1)
    {
...
#undef  THIS_SOLVER
#define THIS_SOLVER "fwrite"
Ent *
Fwrite (int nargs, Datum args[])
{
  char *fnstring, *type="int";
  int i, data_size=-1, dummy;
  int close_after_rw=0;
  Rfile *rf=0;
  FILE *fn=0;
  Ent *e1=0, *e2=0, *e3=0, *rent;
  ...
  {
    e2 = bltin_get_ent (args[1]);
    if ((ent_type (e2) != MATRIX_DENSE_REAL) && (ent_type (e2) != MATRIX_DENSE_STRING)
         && (ent_type (e2) != MATRIX_DENSE_COMPLEX))
      rerror (THIS_SOLVER ": " RLAB_ERROR_ARG2_MD_DENSE "\n");

    data_size = SIZE(ent_data(e2));
  }

  if ( (nargs>2) && ((ent_type (e2) == MATRIX_DENSE_REAL) || (ent_type (e2) == MATRIX_DENSE_COMPLEX)))
  {
    e3 = bltin_get_ent (args[2]);
    if (ent_type(e3) != MATRIX_DENSE_STRING)
      rerror(THIS_SOLVER ": " RLAB_ERROR_ARG3_MDS_SCALAR "\n");
    type = class_char_pointer (e3);
  }

  if (ent_type (e2) == MATRIX_DENSE_REAL)
  {
    if ( (strstr (type, "char")) || (strstr (type, "byte"))
          || (strstr (type, "int8")) || (strstr (type, "int*1")) || (strstr (type, "integer*1")) )
    {
      for (i = 0; i < data_size; i++)
      {
        dummy = mdiV0 (ent_data (e2), i);
        FWRITE ((void *) &dummy, 1, fn);
      }
    }
    else if ( (strstr (type, "short")) || (strstr (type, "int16")) || (strstr (type, "int*2"))
               || (strstr (type, "integer*2")))
    {
      for (i = 0; i < data_size; i++)
      {
        dummy = mdiV0 (ent_data (e2), i);
        FWRITE ((void *) &dummy, 2, fn);
      }
    }
    else if ( (strstr (type, "int32")) || (strstr (type, "int*4")) || (strstr (type, "int"))
               || (strstr (type, "integer*4")))
    {
      for (i = 0; i < data_size; i++)
      {
        dummy = (uint32_t) mdiV0 (ent_data (e2), i);
        FWRITE ((void *) &dummy, 4, fn);
      }
    }
    else if ( (strstr(type, "float")) || (strstr(type, "real")) || (strstr(type, "real*4")) )
    {
      for (i = 0; i < data_size; i++)
      {
        float_tmp = (float) mdrV0 (ent_data (e2), i);
        FWRITE ((void *) &float_tmp, 4, fn);
      }
    }
    else if ( (strstr(type, "double"))  || (strstr(type, "real*8")))
    {
      for (i = 0; i < data_size; i++)
      {
        double_tmp = (double) mdrV0 (ent_data (e2), i);
        FWRITE ((void *) &double_tmp, 8, fn);
      }
    }
  }
  else if (ent_type (e2) == MATRIX_DENSE_COMPLEX)
  {
          ...
          || (strstr (type, "int8")) || (strstr (type, "int*1")) || (strstr (type, "integer*1")) )
    {
      for (i = 0; i < data_size; i++)
      {
        dummy = mdcV0r (ent_data (e2), i);
        FWRITE ((void *) &dummy, 1, fn);
        dummy = mdcV0i (ent_data (e2), i);
        FWRITE ((void *) &dummy, 1, fn);
      }
    }
    else if ( (strstr (type, "short")) || (strstr (type, "int16")) || (strstr (type, "int*2"))
               ...
               || (strstr (type, "integer*4")) )
    {
      for (i = 0; i < data_size; i++)
      {
        dummy = (uint32_t) mdcV0r (ent_data (e2), i);
        FWRITE ((void *) &dummy, 4, fn);
        dummy = (uint32_t) mdcV0i (ent_data (e2), i);
        FWRITE ((void *) &dummy, 4, fn);
      }
    }
    else if ( (strstr(type, "float")) || (strstr(type, "real")) || (strstr(type, "real*4")) )
    {
      for (i = 0; i < data_size; i++)
      {
        float_tmp = (float) mdcV0r (ent_data (e2), i);
        FWRITE ((void *) &float_tmp, 4, fn);
        float_tmp = (float) mdcV0i (ent_data (e2), i);
        FWRITE ((void *) &float_tmp, 4, fn);
      }
    }
    else if ( (strstr(type, "double"))  || (strstr(type, "real*8")) )
    {
      for (i = 0; i < data_size; i++)
      {
        double_tmp = (double) mdcV0r (ent_data (e2), i);
        FWRITE ((void *) &double_tmp, 8, fn);
        double_tmp = (double) mdcV0i (ent_data (e2), i);
        FWRITE ((void *) &double_tmp, 8, fn);
      }
    }
  }
  else if (ent_type (e2) == MATRIX_DENSE_STRING)
  {
    ...
    for (i = 0; i < data_size; i++)
    {
      int slen = isvalidstring(MdsV0 (s, i));
      if (slen > 0)
      {
        FWRITE ((uint8_t *) MdsV0 (s, i), sizeof (char) * slen, fn);
      }
    }
  }
  else
...
WriteASCII (int nargs, Datum args[])
{
  ...
  // process filename
  RLABCODE_PROCESS_ARG1_S(THIS_SOLVER,0,e1,string,1);
...
//   fprintf(stderr, "name = %s\n", name);
           ...
           )
  {
    ...
    if (nargs < 2)
      rerror("open: (sqlite) Requires file access mode");
    e2 = bltin_get_ent (args[1]);
    mode = class_char_pointer (e2);
    if (isvalidstring(mode)<1)
      rerror("open: (sqlite) Unknown file access mode");

    if (mode[0]=='a' || mode[0]=='A' ||  mode[0]=='w' || mode[0]=='W')
    {
    ...
    else if (mode[0]!='r' && mode[0]!='R')
    {
      ...
      rerror ("open: (sqlite) Unknown file access mode. Supported are 'a', 'w', and 'r'");

    // get/create the file
    // open database rw and create it if it doesn't already exist
    rf = get_rfile_ds (name, RFILE_SQLITE, flags);

    // if access is "w" then delete its previous content
...
//
#undef  THIS_SOLVER
#define THIS_SOLVER "close"
Ent * Close (int nargs, Datum args[])
{
  char *fname=0;
  Ent *e1=0, *rent=0;
  Rfile *rf=0;
...
Ent * ReadS (int nargs, Datum args[])
{
  Ent *e1=0, *e2=0;
  int close_after_rw = 0;
  ...
  if (nargs == 2)
  {
    ...
    else if (ent_type (e2) == BTREE)
    {
      ...
      RLABCODE_PROCESS_BTREE_ENTRY_F_S(e2,node,RLAB_NAME_READM_SKIPROWS,iskiprows,class_int,1,1);
  ...
  //
  char * sbuf = NULL; // this will hold the read lines
  len = 0;
  irow=0;
  j=-1;
  while (!feof (rf->fileds_f))
  {
    ...
    if (set_userows)
    {
//       printf(THIS_SOLVER  ": idx_set_userows,SIZE=%i,%i", idx_set_userows, SIZE(set_userows));
...
//         printf(" [Exit]\n");
    ...
    // read line
    len = getline_string ( (char **) &sbuf, &len, '\n', rf->fileds_f);
//     printf(THIS_SOLVER ": len=%i, (%.20s)\n", len, sbuf);
    if (len < 0)
      break;

    // do we skip it?
    if ((j<iskiprows) || (len < min_line_len))
    {
      continue;
    }
...
//     len = isvalidstring(string_buff);
    ...
    // did user provide list of rows they want processed?
    if (set_userows)
    {
      if (j==0)
      {
