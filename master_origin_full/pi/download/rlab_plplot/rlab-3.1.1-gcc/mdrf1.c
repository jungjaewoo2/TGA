...
MDR * mdr_Mod_BF (MDR * m1, MDR * m2)
{
  ...
  if(MD_TYPE_INT32(m1) && MD_TYPE_INT32(m2))
  {
   m = mdi_Create (nr, nc);
    for (i = 0; i < nr; i++)
    {
      for (j = 0; j < nc; j++)
      {
        int i1 = mdi0_safe(m1,i,j);
        int i2 = mdi0_safe(m2,i,j);
  ...
  else
  {
    ...
    for (i = 0; i < nr; i++)
    {
      for (j = 0; j < nc; j++)
      {
        double x1 = mdr0_safe(m1,i,j);
        double x2 = mdr0_safe(m2,i,j);
...
MDR * mdr_MeshCeilFlooRound_BF (int is_ceil, MDR * m, MDR *mesh)
{
  int i, j, nm;
  MDR *cm=0;
...
MDR * mdr_MinMax1_ValIdx (MDR * m, int what, int ignore_infs, int flat, int row_dominant,
                          double * reject_above, double * reject_below)
{
  ...
  if ( EQVECT(m) || flat)
  {
    ...
    for (i=0; i<SIZE(m); i++)
    {
      ...
      if (reject_above)
      {
        if (mdrV0 (m, i) >= *reject_above)
          continue;
      }

      if (reject_below)
      {
        if (mdrV0 (m, i) <= *reject_below)
          continue;
      }

      if (isnand (maxr) || isnand (minr))
      {
        minr = maxr = mdrV0 (m, i);
        mini = maxi = i+1;
        continue;
  ...
  //
  // for matrices report min,then max
  //
  if (row_dominant)
  {
    if (what == 3)
    {
      mmax = mdr_Create (MNR(m),2);
    }
    else
    {
      mmax = mdr_Create (MNR(m),1);
    }
  ...
  else
  {
    ...
    for (i = 0; i < MNC (m); i++)
    {
      ...
      for (j = 1; j < MNR (m); j++)
      {
        ...
        if (ignore_infs)
        {
          if ((mdr0 (m, j, i) == create_inf()) || (mdr0 (m, j, i) == -create_inf()))
            continue;
        }
 ...
 * ************************************************************** */
MDR * mdr_Max2 (MDR * m1, MDR * m2, int ignore_inf)
{
  ...
  if( MD_TYPE_INT32(m1) && MD_TYPE_INT32(m2))
  {
    ...
    for (i=0; i<ir; i++)
    {
      for (j=0; j<jr; j++)
      {
        Mdi0(r, i, j) = MAX(Mdi0_safe(m1, i, j), Mdi0_safe(m2, i, j));
      }
    }
  }
  else
  {
    ...
    for (i=0; i<ir; i++)
    {
      for (j=0; j<jr; j++)
      {
        if ( isnand(mdr0_safe(m1, i, j))
             || ( (ignore_inf) && ( mdr0_safe(m1, i, j)==create_inf() || mdr0_safe(m1, i, j)==-create_inf() ) ) )
          Mdr0(r, i, j) = mdr0_safe(m2, i, j);
        else if ( isnand(mdr0_safe(m2, i, j))
                  || ( (ignore_inf) && ( mdr0_safe(m2, i, j)==create_inf() || mdr0_safe(m2, i, j)==-create_inf() ) ) )
          Mdr0(r, i, j) = mdr0_safe(m1, i, j);
        else
          Mdr0(r, i, j) =
              MAX( mdr0_safe(m1, i, j), mdr0_safe(m2, i, j) );
      }
    }
  }
...
MDR *
mdr_Min2 (MDR * m1, MDR * m2, int ignore_inf)
{
  int i1, i2, j1, j2, i, j, ir, jr;
  MDR *r = 0;
    ...
    for (i=0; i<ir; i++)
    {
      for (j=0; j<jr; j++)
      {
        if ( isnand(mdr0_safe(m1, i, j))
          ...
          Mdr0(r, i, j) = MIN(mdr0_safe(m1, i, j), mdr0_safe(m2, i, j));
      }
    }
  }
...
int IsFinite (MDR * m)
{
  ...
  /* incorrect entry fails by default */
  if (SIZE(m)<1)
  {
    return (0);
  }
...
void * mdr_ATan2 (MDR * m1, MDR * m2, int *type)
{
  MDR *rm=0;
...
MDR * mdi_byte_join(MDR * a, int stride, int use_lsb)
{
  MDR *rval=0;
  int i,j, n=SIZE(a);

  if (n < 1)
    return mdi_Create(0,0);

  if (n % stride)
    return mdi_Create(0,0);

  rval = mdi_Create(1, n/stride);
  mdr_Zero(rval);
  ...
  if (use_lsb)
  {
    ...
    {
      uint8_t * ptr = (uint8_t *) &MdiV0(rval,j);
      for (i=0; i<stride; i++)
        ptr[stride-1-i] = (uint8_t) (mdiV0(a,j*stride + i) & 0xff);
    }
  }
  else
  {
    ...
    for (j=0; j<n/stride; j++)
    {
      uint8_t * ptr = (uint8_t *) &MdiV0(rval,j);
      for (i=0; i<stride; i++)
        ptr[i] = (uint8_t) (mdiV0(a,j*stride + i) & 0xff);
    }
  }
...
#define ysect(p1,p2) (h[p2]*yh[p1]-h[p1]*yh[p2])/(h[p2]-h[p1])
   ...
   int *row_counter = (int *) GC_MALLOC(nc * sizeof(int));
