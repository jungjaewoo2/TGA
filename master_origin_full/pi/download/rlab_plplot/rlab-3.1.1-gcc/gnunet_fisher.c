...
static int getcluster_mean_var(MDR * dataset, MDR * clusterid, int cluster0, MDR ** mean, MDR ** var)
{
  ...
  for (l=0; l<ndata; l++)
  {
    ...
    for (j=0; j<nattr; j++)
    {
      ...
      // what do we do with non-nan values:
...
int fisher_dataset (MDR *dset, MDR *cset, int Class0, int Class1, double c, MDR *W, double *b)
{
  ...
  MDR *V0 = mdr_Create(nattr,nattr);
  if (getcluster_mean_var(dset, cset, Class0, &M0, &V0))
  ...
  MDR *V1 = mdr_Create(nattr,nattr);
  if (getcluster_mean_var(dset, cset, Class1, &M1, &V1))
    goto _exit_fisher_dataset;
  ...
  //
  ival=fisher(V0, V1, M0, M1, c, nattr, W, b);
