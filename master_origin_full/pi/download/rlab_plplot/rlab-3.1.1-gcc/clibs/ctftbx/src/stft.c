 ...
 /*--------------------------------------------------------------------*/
  ...
  for (column = 0; column < tfr.N_time; column++)
  {
    ...
    for (tau = -taumin; tau <= taumax; tau++)
    {
      ...
      wind_sig[row] = Signal.signal[time + tau] * Window[half_Window_Length + tau] / normh;
    }
    ...
    for (row = 0; row < tfr.N_freq; row++)
    {
      tfr.tfr[idx (row, column, tfr.N_freq)] = wind_sig[row];
    }
  }
  /*--------------------------------------------------------------------*/
