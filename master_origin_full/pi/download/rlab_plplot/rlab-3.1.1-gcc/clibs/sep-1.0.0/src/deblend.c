...
static int threshold_type = 0; /* for pow, 1 for lin */

int sep_lutz_threshold_type(int *val)
{
  if (val)
  {
    threshold_type = (*val!=0);
  }
  ...
  for (k=1; k<xn; k++)
  {
    ...
    if (threshold_type)
    {
      debobjlist.thresh = thresh > 0.0 ?
          (thresh0 + (thresh - thresh0) * (double)k/xn) : thresh0;
    }
    else
    {
      debobjlist.thresh = thresh > 0.0 ?
       ...
       pixt=pixelin+PLIST(pixt,nextpix))
  {
    ...
    if (!bmp[(x-xs) + (y-ys)*bmwidth])
    {
      ...
      if (p[nobj-1] > 1.0e-31)
      {
        ...
        drand = p[nobj-1]*gslrnguf_()/RAND_MAX;
