...
rfile specs
for(m in 1){
require runavg verify_wiring temperature specs
# plot=plplot;
///////////////// NEW ////////////////////////////////////////////////
// while(1) {

# DEBUG2=1; // Testing at home without lasers 
# DEBUG2=0; // set by specs
...
if( DEBUG2 == 1) { system("cp /home/pi/sandbox/slave/slaveprod.dat  /tmp/slaveprod.dat;"); sleep(1); }
dir="/tmp/";
file1="slaveprod.dat";
...
RCAL=0.0; // corrected in main by tga
RDCMAX=14.0;
RDCMIN=12.0;
# Rdc_final=sum(Rx)/length(Rx) - RCAL;
# calibrate in master tga
...
fprintf(fn2,"Coil2Rdc=%2.2f;\nCoil2short_pass=\"%s\";\nSkewGain=%2.2f;\nSkewTravel=%2.2f;\nSkewAngle=%2.2f;\n", Rdc_final,Short_pass,skew_gain,skew_travel,skew_angle);
close(fn2);
##################################################
