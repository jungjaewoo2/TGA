...
# xxx=read_ascii("no_time_stamp_rising_squarewave.dat");
xxx=read_ascii("no_timestamp_with_buffers.dat");
...
idx= extract_chunk(d[;1],  chunk, 460 ); 
...
# laser reading 
BEMF = d[;5][ idx[1] : idx[1] + LL ];
############################################################################
if ((max(BEMF) - min(BEMF)) > 50) {
############################################################################
...
############################################################################
# Vm=d[;1];
Vm=d[;1][ idx[2] -LL : idx[2] ];
Vm=Vm * (511+1333)/511;
Vm=Vm ./4096*3.3;
#Vp=d[;3];
Vp=d[;3][ idx[2] -LL : idx[2] ]; 
Vp=Vp * (511+1333)/511;
Vp=Vp ./4096*3.3;
#Im=d[;4];
Im=d[;4][ idx[2] -LL : idx[2] ]; 
#Ip=d[;2];
Ip=d[;2][ idx[2] -LL : idx[2] ]; 

# current in A
Im = (Im -2048) /4096 *3.3 / (1/(1/0.5+1/(100+13+100)) * 13/(100+13+100) * 200 );
Ip = (Ip -2048) /4096 *3.3 / (1/(1/0.5+1/(100+13+100)) * 13/(100+13+100) * 200 );

dc2 = sum([d[;1]+d[;3]][400:800])/length(d[;1][400:800])/2;
# Isteady = (dc2 -2048) /4096 *3.3 / (1/(1/0.5+1/(100+13+100)) * 13/(100+13+100) * 200 );
Isteady = sum(Im)/length(Ip);
...
# plimits(-100,+100,0,6);
plimits(-150,+150,0,6);
plstyle(["point","line","line"]);
plot(<< DT; [-100,4.4;100,4.4] ; [-100,2.76;100,2.76] >>);

# plot(DT);
 _plprint ("Decay_vs_Displacement_laser.ps2", "psc");
psprint ("Decay_vs_Displacement_laser.pdf");
