...
static int
ctftbx_enum_kernel (enum ctftbx_kernel_shape kernel_type,
    ...
    case CTFTBX_MTEK:
    {
      ...
      for (line = 0; line < ker.N_doppler; line++)
      {
        ...
        for (col = 0; col < ker.N_delay; col++)
        {
          ...
          if (inter == 0)
          {
            ker.af[idx (line, col, ker.N_doppler)] = 1.0;
          }
          else
          {
            ker.af[idx (line, col, ker.N_doppler)] = exp (-pi * powof (sqr (inter), LAMBDA));
          }
          inter = 0;
          A = 0;
        }
...
//     ***************************************************************/
    ...
    case CTFTBX_RGK:
    {
      ...
      a = (double *) GC_malloc ( order * sizeof(double) );
      b = (double *) GC_malloc ( order * sizeof(double) );
      ...
      for (line = 0; line < ker.N_doppler; line++)
      {
        ...
        for (col = 0; col < ker.N_delay; col++)
        {
          ...
          ker.af[idx (line, col, ker.N_doppler)] = inter;
      ...
      for (line = 0; line < ker.N_doppler; line++)
      {
        ...
        for (col = 0; col < ker.N_delay; col++)
        {
          ...
          ker.af[idx (line, col, ker.N_doppler)] =
              exp (-(rho2 / 2.0) /
                  sqr ( RE(ker.af[idx (line, col, ker.N_doppler)])
                  - mini + c));
    ...
    case CTFTBX_GMCWK:
    {
      ...
      for (line = 0; line < ker.N_doppler; line++)
      {
        ...
        for (col = 0; col < ker.N_delay; col++)
        {
          ...
          ker.af[idx (line, col, ker.N_doppler)] = exp (-inter / sigma);
        }
      }
    }
    break;
...
//     ***************************************************************/
    ...
    case CTFTBX_SPECTRO:
    {
      ...
      /* creation of memory */
      window.signal = (Complex *) GC_malloc (window.length * sizeof (Complex));
  ...
  /* initialization of the imaginary part for the window
      ...
      for (col = 0; col < window.length; col++)
      {
        ...
        {
          window.signal[col] = parameters[col - min_delay];
        }
        else
        {
          window.signal[col] = 0;
      ...
      mem_alloc_AF (&AF_h, NULL, NULL, NULL);
  ...
  /* Warning : if ker.N_delay != ker.N_doppler,
      ...
      for (line = 0; line < ker.N_doppler; line++)
      {
        for (col = 0; col < ker.N_delay; col++)
        {
          ...
          ker.af[idx (line, col, ker.N_doppler)] =
              sqrt (sqr (RE(AF_h.af[index]))
                  + sqr (RE(AF_h.af[index])));
        }
      }
      /* GC_free memory !! */

      mem_free_AF (&AF_h);
      GC_free (window.signal);
    }
    break;
  }
...
int
ctftbx_kernel (char * kernel_type_char,
               double *parameters, int nb_param, kernel_type_AF ker)
{
  int j;
  enum ctftbx_kernel_shape kernel_type = CTFTBX_UNKNOWN_KERNEL;

  if (!kernel_type_char)
    return 2;

  switch (kernel_type_char[0])
  {
    case 'M':
    case 'm':
      // multiform tiltable exponential kernel
      kernel_type = CTFTBX_MTEK;
      break;

    case 'R':
    case 'r':
      // Radially gaussian kernel
      kernel_type = CTFTBX_RGK;
      break;

    case 'G':
    case 'g':
      // Generalized marginals Choi-Williams kernel
      kernel_type = CTFTBX_GMCWK;
      break;

    case 'W':
    case 'w':
      // Wigner-Ville kernel
      kernel_type = CTFTBX_WIGNER;
      break;

    case 'S':
    case 's':
      // spectrogram kernel
      kernel_type = CTFTBX_SPECTRO;
      break;
  }

  if (kernel_type == CTFTBX_UNKNOWN_KERNEL)
    return 2;

  j = ctftbx_enum_kernel (kernel_type, parameters, nb_param, ker);
