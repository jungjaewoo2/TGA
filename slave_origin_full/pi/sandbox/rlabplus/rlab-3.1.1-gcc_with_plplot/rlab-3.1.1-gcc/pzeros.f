...
*    PZABERTH  :  computes the Aberth correction                          *
*    PZNEWTON  :  computes p(x)/p'(x) by means of Ruffini-Horner's rule   *
*    PZSTART   :  Selects N starting points by means of Rouche's theorem  *
*    PZCNVEX   :  Computes the convex hull, used by PZSTART                 *
*    PZCMERGE  :  Used by PZCNVEX                                           *
*    PZLEFT    :  Used by PZCMERGE                                          *
*    PZRIGHT   :  Used by PZCMERGE                                          *
*    PZCTEST   :  Convexity test, Used by PZCMERGE                          *
*************************************************************************
*                                                                       *
*                                                                       *
*************************************************************************
...
*************************************************************************
     ...
     * ROOT, RADIUS, ERR, ITER, APOLY,APOLYR)
      ...
      REAL*8 RADIUS(N),APOLY(N+1),APOLYR(N+1)
      REAL*8 AMAX,EPS,SMALL,BIG
      LOGICAL ERR(N+1)
...
C Starts Aberth's iterations
      DO 50 ITER=1,NITMAX
        DO 50 I=1,N
          IF (ERR(I)) THEN
            CALL PZNEWTON(N,POLY,APOLY,APOLYR,ROOT(I),SMALL,
     *             RADIUS(I),CORR,ERR(I))
            IF (ERR(I)) THEN
              CALL PZABERTH(N,I,ROOT,ABCORR)
...
*             .false., otherwise.                                       *
*************************************************************************
      SUBROUTINE PZNEWTON(N,POLY,APOLY,APOLYR,Z,SMALL,RADIUS,CORR,AGAIN)
      INTEGER N,I
      COMPLEX*16 POLY(N+1)
      COMPLEX*16 Z,P,P1,CORR,ZI,DEN,PPSP
      REAL*8 RADIUS,APOLY(N+1),APOLYR(N+1)
      REAL*8 AP,AZ,AZI,ABSP
      REAL*8 SMALL
      LOGICAL AGAIN
      AZ=ABS(Z)
...
*************************************************************************
      SUBROUTINE PZABERTH(N,J,ROOT,ABCORR)
      INTEGER N,I,J
...
************************************************************************* 
      ...
      LOGICAL H(N+1)
      COMPLEX*16 Y(N)
      REAL*8 A(N+1),RADIUS(N)
      REAL*8 R,TH,ANG,TEMP,SIGMA,PI2
      REAL*8 SMALL,BIG,XSMALL,XBIG
...
*************************************************************************
      SUBROUTINE PZCMERGE(N,A,I,M,H)
      INTEGER N,M,I,IR,IL,IRR,ILL
      LOGICAL H(N)
      LOGICAL TSTL,TSTR,PZCTEST
      REAL*8 A(N)
...
C hull of the left and right sets, respectively 
      CALL PZLEFT(N,H,I,IL)
      CALL PZRIGHT(N,H,I,IR)
C check the convexity of the angle formed by IL,I,IR
      IF (PZCTEST(N,A,IL,I,IR)) THEN
        RETURN
      ELSE
...
10      CONTINUE
        IF (IL.EQ.(I-M)) THEN
          TSTL=.TRUE.        
          ELSE
          CALL PZLEFT(N,H,IL,ILL)
          TSTL=PZCTEST(N,A,ILL,IL,IR)
        ENDIF
        IF (IR.EQ.MIN(N,I+M))THEN
          TSTR=.TRUE.
        ELSE
          CALL PZRIGHT(N,H,IR,IRR)
          TSTR=PZCTEST(N,A,IL,IR,IRR)
        ENDIF
        H(IL)=TSTL
        H(IR)=TSTR
...
*************************************************************************
      ...
      S2=S2*(I-IL)
      PZCTEST=.FALSE.
      IF(S1.GT.(S2+TOLER)) PZCTEST=.TRUE.
      END
