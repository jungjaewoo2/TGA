...
#define ASA_ID \
"/* $Id: asa.c,v 25.26 2005/03/03 03:11:04 ingber Exp ingber $ */"
...
#else
#if INT_LONG
    fprintf (ptr_asa_pipe, "\t%s-%i", "param", index_v);
#else
    fprintf (ptr_asa_pipe, "\t%s-%d", "param", index_v);
#endif
#endif
...
#else
#if INT_LONG
    fprintf (ptr_asa_pipe, "\t%s-%i", "param_temp", index_v);
#else
    fprintf (ptr_asa_pipe, "\t%s-%d", "param_temp", index_v);
#endif
#endif
...
// #if ASA_PRINT
// #if INT_LONG
//   fprintf (ptr_asa_out, "Initial Random Seed = %i\n\n", *seed);
// #else
...
#else
#if INT_LONG
                          "OPTIONS->User_Delta_Parameter[%i] = %*.*g\n",
#else
                          "OPTIONS->User_Delta_Parameter[%d] = %*.*g\n",
#endif
#endif
...
#else
#if INT_LONG
                          "OPTIONS->User_Quench_Param_Scale[%i] = %*.*g\n",
#else
                          "OPTIONS->User_Quench_Param_Scale[%d] = %*.*g\n",
#endif
#endif
...
#else
#if INT_LONG
                          "OPTIONS->User_Parameter_Temperature[%i] = %*.*g\n",
#else
                          "OPTIONS->User_Parameter_Temperature[%d] = %*.*g\n",
#endif
#endif
...
#else
#if INT_LONG
                          "OPTIONS->User_Temperature_Ratio[%i] = %*.*g\n",
#else
                          "OPTIONS->User_Temperature_Ratio[%d] = %*.*g\n",
#endif
#endif
...
#else
#if INT_LONG
             "temperature_scale_parameters[%i] = %*.*g\n",
#else
             "temperature_scale_parameters[%d] = %*.*g\n",
#endif
#endif
...
#else
#if INT_LONG
  fprintf (ptr_asa_out, "OPTIONS->Queue_Size = %i\n", OPTIONS->Queue_Size);
#else
  fprintf (ptr_asa_out, "OPTIONS->Queue_Size = %d\n", OPTIONS->Queue_Size);
#endif
#endif
...
#include "asa_opt"
      ...
      if ((ptr_save_opt = fopen ("asa_save_opt", "r")) == NULL) {
        ...
        fscanf (ptr_save_opt, "%i", &read_long);
...
#if ASA_PRINT_MORE
#if INT_LONG
              fprintf (ptr_asa_out, "ASA_QUEUE: %i \t %*.*g\n",
                       OPTIONS->N_Generated,
...
#else
#if INT_LONG
      fprintf (ptr_asa_pipe, "%i", *number_generated);
#else
      fprintf (ptr_asa_pipe, "%d", *number_generated);
#endif
#endif
...
#else
#if INT_LONG
      fprintf (ptr_asa_pipe, "\t%i", *number_accepted);
#else
      fprintf (ptr_asa_pipe, "\t%d", *number_accepted);
#endif
#endif
...
#else
#if INT_LONG
        fprintf (ptr_asa_out, "Present Random Seed = %i\n\n", *seed);
#else
        fprintf (ptr_asa_out, "Present Random Seed = %d\n\n", *seed);
#endif
#endif
...
#if INT_LONG
      if (OPTIONS->Asa_Recursive_Level > 0)
        sprintf (asa_save_comm, "/bin/cp asa_save_%d asa_save_%d.%i",
                 OPTIONS->Asa_Recursive_Level,
                 OPTIONS->Asa_Recursive_Level, OPTIONS->N_Accepted);
      else
        sprintf (asa_save_comm, "/bin/cp asa_save asa_save.%i",
                 OPTIONS->N_Accepted);
#else
      if (OPTIONS->Asa_Recursive_Level > 0)
...
#else
#if INT_LONG
             "current_user_parameter_temp[%i] too small = %*.*g\n",
#else
             "current_user_parameter_temp[%d] too small = %*.*g\n",
#endif
#endif
...
#if INT_LONG
    fprintf (ptr_asa_out,
             "*number_accepted at best_generated_state->cost = %i\n",
             *best_number_accepted_saved);
    fprintf (ptr_asa_out,
             "*number_generated at best_generated_state->cost = %i\n",
             *best_number_generated_saved);
#else
    fprintf (ptr_asa_out,
...
#if INT_LONG
    fprintf (ptr_asa_out,
             "Generated %i invalid states when calculating the derivatives\n",
             tmp_saved);
#else
    fprintf (ptr_asa_out,
...
#if INT_LONG
  fprintf (ptr_asa_out,
           "*index_cost_acceptances = %i, *current_cost_temperature = %*.*g\n",
           *index_cost_acceptances,
           G_FIELD, G_PRECISION, *current_cost_temperature);
 ...
 *number_invalid... = %i\n", G_FIELD, G_PRECISION, *accepted_to_generated_ratio, (*number_invalid_generated_states));
  fprintf (ptr_asa_out,
           "*number_generated = %i, *number_accepted = %i\n",
...
#if INT_LONG
  fprintf (ptr_asa_out, "MAXIMUM_REANNEAL_INDEX = %i\n",
...
#if INT_LONG
  fprintf (ptr_asa_out, "OPTIONS->Limit_Acceptances = %i\n",
           (LONG_INT) OPTIONS->Limit_Acceptances);
  fprintf (ptr_asa_out, "OPTIONS->Limit_Generated = %i\n",
...
#else
#if INT_LONG
  fprintf (ptr_asa_out, "OPTIONS->Sequential_Parameters = %i\n",
