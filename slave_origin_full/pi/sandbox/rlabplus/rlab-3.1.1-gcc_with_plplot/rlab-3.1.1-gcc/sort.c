...
int rlab_sort_method(int num,...)
{
  if (num==0)
    goto __rlab_sort_method_exit;

  int i;

  // initialize valist for first argument
  va_list valist;
  va_start(valist,num);
  i=va_arg(valist, int);  // just get the first argument. ignore the rest
  va_end(valist);

  if (i == RLAB_SORT_QUICK || i==RLAB_SORT_HEAP)
    _rlab_settings_sort_method = i;

__rlab_sort_method_exit:

  return _rlab_settings_sort_method;
}

int rlab_sort_nans(int num,...)
{
  if (num==0)
    goto __rlab_sort_nans_exit;
