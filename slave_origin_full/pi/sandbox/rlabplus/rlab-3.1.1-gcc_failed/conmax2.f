...
C STATEMENTS
C
C     SUBROUTINE CMXFNSET(NPARM,NUMGR,PTTBL,IPTB,INDM,PARAM,IPT,
...
c       IMPLICIT REAL*8 (A-H,O-Z)
c C
c       DIMENSION FUN(5),PTTBL(6,2),IWORK(178),WORK(720),PARAM(4),
c      *ERROR(24)
c C
c C*****MAC INSERT
c C     OPEN(6,FILE='TSTOT')
c C*****END MAC INSERT
c C
c C SET MACHINE AND PRECISION DEPENDENT CONSTANTS.
c       ONE=1.0D0
c       ZERO=ONE-ONE
c       TWO=ONE+ONE
c       THREE=ONE+TWO
...
c       NWRIT=I1MACH(2)
c C
c C SET PARAMETERS FOR CONMAX.
c C
c C SET IOPTN=0 SINCE NO EXTRA OPTIONS ARE TO BE USED.
c       IOPTN=0
c C
c C SET NPARM=4 SINCE THERE ARE 4 PARAMETERS (VARIABLES) A, B, C, AND D.
c       NPARM=4
c C
c C SET NUMGR=21 SINCE THERE ARE 21 CONSTRAINTS (5 OF TYPE 2, 10 OF TYPE 1,
c C 5 OF TYPE -1, AND ONE OF TYPE -2).
c       NUMGR=21
c C
c C SET ITLIM=100, OR WHATEVER LIMIT IS DESIRED ON THE NUMBER OF ITERATIONS.
c       ITLIM=100
c C
c C SET IFUN=5 (OR GREATER) SINCE THERE ARE 5 TYPE 2 CONSTRAINTS, AND USE
c C THIS NUMBER AS THE DIMENSION OF FUN ABOVE.
c       IFUN=5
c C
c C SET IPTB=6 (OR GREATER) AND SET INDM=2 (OR GREATER), SINCE THE GREATEST
c C FIRST SUBSCRIPT WE WILL USE IN PTTBL IS 6, AND THE GREATEST SECOND
c C SUBSCRIPT WE WILL USE IN PTTBL IS 2.  WE USE THESE NUMBERS TO DIMENSION
c C PTTBL ABOVE.  NOTE THAT IT IS ESSENTIAL THAT THE FIRST DIMENSION OF
c C PTTBL BE EXACTLY IPTB.
c       IPTB=6
c       INDM=2
c C
c C SET LIWRK=178 (OR GREATER), AND USE THIS NUMBER TO DIMENSION LIWRK
c C ABOVE, BECAUSE OF THE COMPUTATION 7*21 + 7*4 + 3 = 178 (SEE CONMAX
c C USERS GUIDE FOR AN EXPLANATION OF THIS AND OF LWRK BELOW).
c       LIWRK=178
c C
c C SET LWRK=720 (OR GREATER), AND USE THIS NUMBER TO DIMENSION LWRK ABOVE,
c C BECAUSE OF THE COMPUTATION 2*4**2 + 4*21*4 + 11*21 + 27*4 + 13 = 720.
c       LWRK=720
c C
c C THE DIMENSION OF PARAM ABOVE MUST BE NPARM (I.E. 4) OR GREATER, AND THE
c C DIMENSION OF ERROR ABOVE MUST BE NUMGR+3 (I.E. 24) OR GREATER.
c C
c C SET THE VALUES OF THE FUNCTION F.
c       FUN(1)=ONE/TWO
c       FUN(2)=ONE
c       FUN(3)=-ONE
c       FUN(4)=THREE/TWO
c       FUN(5)=-ONE
c C
c C SET THE COORDINATES OF THE FIVE POINTS.
c       PTTBL(1,1)=ZERO
c       PTTBL(1,2)=ZERO
c       PTTBL(2,1)=ZERO
c       PTTBL(2,2)=ONE
c       PTTBL(3,1)=-TWO/THREE
c       PTTBL(3,2)=ONE/THREE
c       PTTBL(4,1)=ONE
c       PTTBL(4,2)=ONE
c       PTTBL(5,1)=ONE
c       PTTBL(5,2)=TWO
c C
c C PUT EPS IN PTTBL(6,1) FOR TRANSMITTAL TO FNSET.  THIS IS WHY WE NEEDED
c C IPTB TO BE AT LEAST 6.
c       EPS=(5*TWO)**(-4)
c       PTTBL(6,1)=EPS
c C
c C SET THE INITIAL GUESSES FOR THE PARAMETERS.
c       PARAM(1)=ZERO
c       PARAM(2)=ZERO
c       PARAM(3)=ZERO
c       PARAM(4)=ZERO
c C
c C WRITE THE INITIAL DATA.
c       WRITE(NWRIT,100)IOPTN,NPARM,NUMGR,ITLIM,IFUN,IPTB,INDM
c   100 FORMAT(/9H IOPTN IS,I5,10H  NPARM IS,I4,10H  NUMGR IS,I4//
c      *9H ITLIM IS,I5,9H  IFUN IS,I3,9H  IPTB IS,I3,9H  INDM IS,I3)
c       WRITE(NWRIT,200)(FUN(I),I=1,5)
c   200 FORMAT(/24H THE FUNCTION VALUES ARE/(4E15.5))
c       WRITE(NWRIT,300)
c   300 FORMAT(/15H THE POINTS ARE)
c       DO 500 I=1,5
c         WRITE(NWRIT,400)(PTTBL(I,J),J=1,INDM)
c   400   FORMAT(2E15.5)
c   500   CONTINUE
c       WRITE(NWRIT,600)EPS
c   600 FORMAT(/7H EPS IS,E15.5)
c       WRITE(NWRIT,700)(PARAM(J),J=1,NPARM)
c   700 FORMAT(/27H THE INITIAL PARAMETERS ARE/(/3E23.14))
c C
c C NOW CALL CONMAX.
c       CALL CONMAX(IOPTN,NPARM,NUMGR,ITLIM,FUN,IFUN,PTTBL,IPTB,
c      *INDM,IWORK,LIWRK,WORK,LWRK,ITER,PARAM,ERROR)
c C
c C WRITE THE OUTPUT.
c C NOTE THAT WE HAVE DEFERRED WRITING LIWRK AND LWRK UNTIL AFTER CALLING
c C CONMAX SINCE CONMAX WILL CHANGE THEM TO THE NEGATIVE OF THE SMALLEST
c C ALLOWABLE VALUES AND RETURN IF THEY WERE TOO SMALL.
c       WRITE(NWRIT,800)ITER,LIWRK,LWRK
c   800 FORMAT(/26H *****AFTER CONMAX ITER IS,I4,10H  LIWRK IS,I5,
c      *9H  LWRK IS,I6)
c       WRITE(NWRIT,900)(PARAM(J),J=1,NPARM)
c   900 FORMAT(/25H THE FINAL PARAMETERS ARE/(/3E23.14))
c       WRITE(NWRIT,1000)ERROR(NUMGR+1),ERROR(NUMGR+2),ERROR(NUMGR+3),
c      *(ERROR(I),I=1,NUMGR)
c  1000 FORMAT(/20H THE ERROR NORMS ARE//3E23.13//
c      *15H THE ERRORS ARE//(3E23.14))
c       STOP
c       END
c       SUBROUTINE CMXFNSET(NPARM, NUMGR, PTTBL, IPTB, INDM, PARAM,
c      *                 IPT, INDFN, ICNTYP, CONFUN)
c C
c       IMPLICIT REAL*8 (A-H,O-Z)
c C
c       DIMENSION
c      *                  PTTBL(IPTB,INDM), PARAM(NPARM),
c      *                  ICNTYP(NUMGR), CONFUN(NUMGR,NPARM+1)
c C
c C THIS IS THE SUBROUTINE FNSET FOR THE EXAMPLE DISCUSSED IN THE CONMAX
c C USERS GUIDE.
c C
c C SET PRECISION DEPENDENT CONSTANTS.
c       ONE=1.0D0
c       ZERO=ONE-ONE
c C
c C WE BREAK FNSET INTO SECTIONS BASED ON THE VALUE OF IPT, THAT IS, ON
c C WHICH CONSTRAINT IS BEING SET.
c       IF(IPT-5)300,300,100
c   100 IF(IPT-15)600,600,200
c   200 IF(IPT-20)1100,1100,1300
c C
c C HERE IPT .LE. 5 AND WE SET A CONSTRAINT OF THE FORM
c C ABS(F(X,Y) - (AX+BY+C)/(DX+1)) .LE. W.
c C NOTE THAT SINCE THIS IS A TYPE 2 CONSTRAINT WE DO NOT NEED TO DEAL
c C WITH THE ABSOLUTE VALUE OR THE F(X,Y) HERE.
c   300 ICNTYP(IPT)=2
c       A=PARAM(1)
c       B=PARAM(2)
c       C=PARAM(3)
...
c       Q=D*X+ONE
c       CONFUN(IPT,1)=P/Q
c       IF(INDFN)400,400,500
c C
c   400 RETURN
c C
c C HERE IPT .LE. 5 AND INDFN=1, AND WE SET THE PARTIAL DERIVATIVES.
c   500 CONFUN(IPT,2)=X/Q
c       CONFUN(IPT,3)=Y/Q
c       CONFUN(IPT,4)=ONE/Q
c       CONFUN(IPT,5)=-P*X/(Q*Q)
c       RETURN
c C
c C HERE 6 .LE. IPT .LE. 15 AND IF IPT IS EVEN WE SET THE CONSTRAINT
c C (AX+BY+C)/(DX+1) .LE. W, WHICH IS HALF OF THE CONSTRAINT
c C ABS((AX+BY+C)/(DX+1)) .LE. W, WHILE IF IPT IS ODD WE SET THE CONSTRAINT
c C -(AX+BY+C)/(DX+1) .LE. W, WHICH IS THE OTHER HALF OF THE CONSTRAINT
c C ABS((AX+BY+C)/(DX+1)) .LE. W.
c   600 ICNTYP(IPT)=1
c       II=(IPT-4)/2
c       A=PARAM(1)
c       B=PARAM(2)
...
c       Q=D*X+ONE
c       IREM=IPT-4-2*II
c       IF(IREM)700,700,900
c C
c C HERE 6 .LE. IPT .LE. 15 AND IPT IS EVEN.
c   700 CONFUN(IPT,1)=P/Q
c       IF(INDFN)400,400,800
c   800 CONFUN(IPT,2)=X/Q
c       CONFUN(IPT,3)=Y/Q
c       CONFUN(IPT,4)=ONE/Q
c       CONFUN(IPT,5)=-P*X/(Q*Q)
c       RETURN
c C
c C HERE 6 .LE. IPT .LE. 15 AND IPT IS ODD.
c   900 CONFUN(IPT,1)=-P/Q
c       IF(INDFN)400,400,1000
c  1000 CONFUN(IPT,2)=-X/Q
c       CONFUN(IPT,3)=-Y/Q
c       CONFUN(IPT,4)=-ONE/Q
c       CONFUN(IPT,5)=P*X/(Q*Q)
c       RETURN
c C
c C HERE 16 .LE. IPT .LE. 20 AND WE SET A CONSTRAINT OF THE FORM
c C -DX - 1.0 + EPS .LE. 0.0
c  1100 ICNTYP(IPT)=-1
c       D=PARAM(4)
c       EPS=PTTBL(6,1)
c       II=IPT-15
c       X=PTTBL(II,1)
c       CONFUN(IPT,1)=-D*X-ONE+EPS
c       IF(INDFN)400,400,1200
c  1200 CONFUN(IPT,2)=ZERO
c       CONFUN(IPT,3)=ZERO
c       CONFUN(IPT,4)=ZERO
c       CONFUN(IPT,5)=-X
c       RETURN
c C
c C HERE IPT=21 AND WE SET THE CONSTRAINT
c C (PARTIAL DERIVATIVE OF (AX+BY+C)/(DX+1) WITH RESPECT TO X AT
c C (X,Y) = (0.0,0.0)) .LE. 0.0,
c C I.E. A - CD .LE. 0.0
c  1300 ICNTYP(IPT)=-2
c       A=PARAM(1)
c       C=PARAM(3)
c       D=PARAM(4)
c       CONFUN(IPT,1)=A-C*D
c       IF(INDFN)400,400,1400
c  1400 CONFUN(IPT,2)=ONE
c       CONFUN(IPT,3)=ZERO
c       CONFUN(IPT,4)=-D
c       CONFUN(IPT,5)=-C
c       RETURN
...
C
C
      SUBROUTINE CONMAX2(IOPTN, NPARM, NUMGR, ITLIM, FUN, IFUN,
     *                   PTTBL, IPTB, INDM, IWORK, LIWRK, WORK,
     *                   LWRK, ITER, PARAM, ERROR,
     *                   STDOUT, LSTDOUT, CMXFNSET
     *                  )
C
C
...
C
      DIMENSION
     *                  FUN(IFUN), PTTBL(IPTB,INDM), ERROR(NUMGR+3),
...
C     Definition of stream control variables 256 appears in gsl_findroot.c
     ...
     *                  CMXFNSET
      ...
      IF (LSTDOUT .gt. 1) THEN
          NWRIT = 12
          OPEN (NWRIT, FILE=STDOUT, STATUS='OLD')
      ELSE
          NWRIT = 0
      ENDIF
C     End of it. KMK
...
C ICNUSE=0, THE OTHER BEING STATEMENT 1415 BELOW..
  500 CALL ERCMP1(IOPTN,NPARM,NUMGR,FUN,IFUN,PTTBL,IPTB,INDM,PARAM,
     *0,0,IWORK,LIWRK,WORK(ILC08),IWORK(ILC17),IPMAX,ISMAX,
     *ERROR, CMXFNSET)
...
C WRITE ITER, ISUCC, IRK, ENCHG, AND THE ERROR NORMS.
 1050 CONTINUE
       IF (NWRIT .GT. 0) THEN
         WRITE(NWRIT,1100)ITER,ISUCC,IRK,ENCHG,ENORM,ENOR2,ENOR3
 1100    FORMAT(/8H ITER IS,I5,10H  ISUCC IS,I4,8H  IRK IS,I4,
     *   10H  ENCHG IS,E24.14/9H ENORM IS,E24.14,10H  ENOR2 IS,
     *   E24.14,9H ENOR3 IS,E24.14)
       ENDIF
C
C
...
C WE SET IPT=-1 TO TELL DERST IT NEED ONLY COMPUTE STANDARD CONSTRAINTS.
 1326 IPT=-1
     ...
     *WORK(ILC24),WORK(ILC35),IWORK(ILC22),WORK(ILC08),CMXFNSET)
C
 1328 M=0
      DO 1350 I=1,NUMGR
        II=ILC17-1+I
...
C SYSTEM FROM SCRATCH.
     ...
     *NUMGR,WORK(ILC40),WORK(ILC42),WDIST,NMAJ,NMIN,JFLAG, CMXFNSET)
...
C CALL ERCMP1 WITH ICNUSE=1.
     ...
     *IPMAX,ISMAX,WORK(ILC11), CMXFNSET)
      I1=ILC11-1+(NUMGR+2)
...
C  48  ZWORK(NPARM)  (OPPOSITE V, Y;  FOLLOWS PICOR)
  480 ILOC=2*NPARM**2+3*NUMGR*NPARM+6*NUMGR+18*NPARM+13
     ...
     *IPT,PARAM1,V,KCNTYP,CONFUN, CMXFNSET)
C
      IMPLICIT REAL*8 (A-H,O-Z)
C
...
C COMPUTATIONS DIRECTLY USING FORMULAS SUPPLIED BY THE USER.
  ...
  100 CALL CMXFNSET(NPARM,NUMGR,PTTBL,IPTB,INDM,PARAM,IPT,1,KCNTYP,
     *CONFUN)
      RETURN
C
...
C IPT EVALUATED AT POINT PARAM1 IN CONFUN(IPT,1).
        CALL CMXFNSET(NPARM,NUMGR,PTTBL,IPTB,INDM,PARAM1,IPT,0,
     *  KCNTYP,CONFUN)
        UP=CONFUN(IPT,1)
C
...
C EARLIER-COMPUTED PARTIAL DERIVATIVES INTO CONFUN.
      CALL CMXFNSET(NPARM,NUMGR,PTTBL,IPTB,INDM,PARAM,IPT,0,
     *KCNTYP,CONFUN)
      DO 600 L=1,NPARM
...
C CONFUN.
 ...
 2300   CONTINUE
     ...
     *LIWRK,WORK,LWRK,PARSER,ISUCC,PARAM,ERROR,CMXFNSET)
C
      IMPLICIT REAL*8 (A-H,O-Z)
C
...
C FOR THEM IN FUNTBL.
  400 CALL SETU1(IOPTN,NUMGR,NPARM,NUMIN,RCHIN,PTTBL,IPTB,INDM,
     ...
     *IWORK(ILC13),WORK(ILC35),M, CMXFNSET)
C
...
C CALL SLNPRO TO COMPUTE A SEARCH DIRECTION X.
 1000 CALL SLNPRO(WORK(ILC35),M,NPAR1,IYRCT,WORK(ILC47),
     *IWORK(ILC18),IWORK(ILC19),NPARM,NUMGR,X,INDIC, CMXFNSET)
C
...
C CALL SEARSL TO DO A LINE SEARCH IN DIRECTION X.
 1470 CALL SEARSL(IOPTN,NUMGR,NPARM,PRJLIM,TOL1,X,FUN,IFUN,PTTBL,
     ...
     *WORK(ILC25),PRJSLP,EMIN,EMIN1,PARSER,NSRCH, CMXFNSET)
C
...
2800  CONTINUE
     ...
     *IACT1,BNDLGT,IYRCT,IPHSE,IWORK,LIWRK,WORK,LWRK,CONFUN,IACT,V,
     *M,CMXFNSET)
C
      IMPLICIT REAL*8 (A-H,O-Z)
C
...
C CALL DERST TO COMPUTE BOTH FUNCTION AND GRADIENT VALUES.
     ...
     *  WORK(ILC24),V,IWORK(ILC22),CONFUN, CMXFNSET)
...
C EXCHANGE X(J) FOR Y(IYRI).
    7 CALL SJELIM(MP1,1,NP1,IYRI,J,NPARM,NUMGR,V, CMXFNSET)
      IXRCT(IYRI)=J
      IYCCT(J)=IYRI
...
C
   ...
   50 CALL SJELIM(MP1,NP1,NP1,I,K,NPARM,NUMGR,V, CMXFNSET)
      ITEMP=IYRCT(I)
      IYRCT(I)=IYCCT(K)
      IYCCT(K)=ITEMP
...
C
   ...
   72 CALL SJELIM(MP1,NP1,NP1,I,K,NPARM,NUMGR,V, CMXFNSET)
      ITEMP=IYRCT(I)
      IYRCT(I)=IYCCT(K)
      IYCCT(K)=ITEMP
C RESET REA AND IRLAX.
...
C NOW FILL IN THE REST OF IYRCT BY SCANNING IXRCT AGAIN.
   ...
   87 RETURN
      END
      SUBROUTINE SJELIM(L,LL,K,IR,IS,NPARM,NUMGR,V,CMXFNSET)
...
C REPLACE THE RESOLVENT BY ITS RECIPROCAL.
     ...
     *PROJCT,EMIN,EMIN1,PARSER,NSRCH, CMXFNSET)
C
      IMPLICIT REAL*8 (A-H,O-Z)
C
...
C OR ITYPM2 IS POSITIVE.
 ...
 4020 CALL CORRCT(IOPTN,NPARM,NUMGR,FUN,IFUN,PTTBL,IPTB,INDM,
     ...
     *WORK(ILC29),WORK(ILC08),WORK(ILC48),IWORK(ILC21),PARPRJ,
     *ICORCT,CMXFNSET)
...
10071 CALL RCHMOD(NUMGR,ERROR,ERR1,IWORK(ILC17),MACT,IACT,
     *IPMAX,ISMAX,UNIT,1,RCHDWN,RCHIN, CMXFNSET)
...
C CALL FNSET WITH INDFN=0 TO COMPUTE CONFUN(I,1) AND ICNTYP(I).
  200   CALL CMXFNSET(NPARM,NUMGR,PTTBL,IPTB,INDM,PARAM,IPT,0,
     *  ICNTYP,CONFUN)
        GO TO 1100
C
...
C WILL NOT BE USED.
  700   CALL CMXFNSET(NPARM,NUMGR,PTTBL,IPTB,INDM,PARAM,IPT,0,
     *  IWORK(ILC22),CONFUN)
C
...
C CONSTRAINTS, WHILE OTHERWISE WE SET IPT=-1.
 ...
 1940 IPT=-1
      GO TO 1980
 1960 IPT=0
 1980 CALL CMXFNSET(NPARM,NUMGR,PTTBL,IPTB,INDM,PARAM,IPT,0,
     *IWORK(ILC22),CONFUN)
C
...
C SCRATCH.
     ...
     *NPARM,NUMGR,WORK(ILC40),WORK(ILC36),WDIST,NMAJ,NMIN,
     *JFLAG, CMXFNSET)
C
...
C DO AN RK STEP.
 5000 CALL RKPAR(IOPTN,NUMGR,NPARM,ICNTYP,MACTRK,IACT,ACTDIF,PROJCT,
     ...
     *WORK(ILC43),PARPRJ,IFRKPR, CMXFNSET)
...
C CORRCT TO MOVE BACK INTO THE FEASIBLE REGION IF NECESSARY.
 ...
 5200 CALL CORRCT(IOPTN,NPARM,NUMGR,FUN,IFUN,PTTBL,IPTB,INDM,
     ...
     *PMAT,CONFUN,WORK(ILC48),IWORK(ILC21),PARPRJ,
     *ICORCT, CMXFNSET)
...
C WE SET IPT=-1 TO TELL DERST TO COMPUTE STANDARD CONSTRAINTS ONLY.
 5330 IPT=-1
      CALL DERST(IOPTN,NPARM,NUMGR,PTTBL,IPTB,INDM,PARAM,IPT,
     *WORK(ILC24),WORK(ILC35),IWORK(ILC22),CONFUN, CMXFNSET)
...
C
 5380 CALL SEARSL(IOPTN,NUMGR,NPARM,PRJLIM,TOL1,XRK,FUN,IFUN,
     ...
     *PARPRJ,PROSEA,EMIN,EMIN1,PARSER,NSRCH, CMXFNSET)
C
...
C
 ...
 5600   CONTINUE
     ...
     *PROJCT,ERROR,MACTRK,ACTDIF,IACT, CMXFNSET)
C
      IMPLICIT REAL*8 (A-H,O-Z)
C
...
C MIN (CONUP, ERROR(I)/PROJCT).
 ...
 1200   CONTINUE
     ...
     *WORK,LWRK,CONFUN,PMAT, CMXFNSET)
C
      IMPLICIT REAL*8 (A-H,O-Z)
C
...
C PUT ACTDIF IN THE LAST ROW OF PMAT.
 ...
 2300   CONTINUE
     ...
     *WORK,LWRK,CONFUN,VDERN,VDERS,WVEC,PARPRJ,IFRKPR, CMXFNSET)
C
      IMPLICIT REAL*8 (A-H,O-Z)
C
...
C FEASIBLE REGION IF POSSIBLE BEFORE CALLING PMTST.
  ...
  408 CALL PMTST(IOPTN,NUMGR,NPARM,WVEC,ICNTYP,MACTRK,
     ...
     *LIWRK,WORK,LWRK,CONFUN,PMAT, CMXFNSET)
      CALL WOLFE(NPARM,MACTRK,PMAT,1,S,NCOR,IWORK(ILC15),IWORK,
     *LIWRK,WORK,LWRK,WORK(ILC33),WORK(ILC06),WORK(ILC31),WORK(ILC30),
     *NPARM,NUMGR,WORK(ILC40),VDERN,WDIST,NMAJ,NMIN,JFLAG, CMXFNSET)
...
C FEASIBLE REGION IF POSSIBLE BEFORE CALLING PMTST.
  ...
  842 CALL CORRCT(IOPTN,NPARM,NUMGR,FUN,IFUN,PTTBL,IPTB,INDM,ICNTYP,
     *UNIT,TOLCON,RCHIN,ERROR,MACTRK,IACT,PROJCT,
     *IPHSE,IWORK,LIWRK,WORK,LWRK,WORK(ILC27),WORK(ILC11),
     *WORK(ILC10),PMAT,CONFUN,WORK(ILC48),IWORK(ILC21),PARPRJ,
     *ICORCT, CMXFNSET)
...
C THE WORK OF SCANNING ICNTYP.
      ...
      CALL ERCMP1(IOPTN,NPARM,NUMGR,FUN,IFUN,PTTBL,IPTB,INDM,
     *PARWRK,1,-3,IWORK,LIWRK,CONFUN,JCNTYP,IPMAX,ISMAX,
     *ERR1, CMXFNSET)
...
C CALL WOLFE WITH ISTRT=0 TO COMPUTE DVEC FROM SCRATCH.
     ...
     *NUMGR,WORK(ILC41),DVEC,WDIST,NMAJ,NMIN,JFLAG, CMXFNSET)
...
C CASES ZWORK WILL BE THE SAME AS BEFORE THE CALL TO SEARCR.
     ...
     *ERR1,P1,F1,PROCOR,EMIN,ISRCR, CMXFNSET)
...
C WORK OF SCANNING ICNTYP.
     ...
     *1,-3,IWORK,LIWRK,CONFUN,JCNTYP,IPMAX,ISMAX,ERR1, CMXFNSET)
      GO TO 670
C
...
C MODIFYING PROCOR.
 2100 CALL MULLER(IOPTN,NPARM,NUMGR,DVEC,FUN,IFUN,PTTBL,IPTB,
     *INDM,ZWORK,TOLCON,IPHSE,IWORK,LIWRK,WORK,LWRK,PARWRK,
     *ERR1,P1,F1,PROCOR,EMIN, CMXFNSET)
C
C NOW COMPUTE PARPRJ = ZWORK + PROCOR*DVEC, SET ICORCT=0, AND RETURN.
 ...
 2300   CONTINUE
     ...
     *PARWRK,ERR1,P1,F1,PROCOR,EMIN,ISRCR, CMXFNSET)
C
      IMPLICIT REAL*8 (A-H,O-Z)
C
...
c       IF (NWRIT .GT. 0) THEN
c         WRITE(NWRIT,11500)
c 11500   FORMAT(30H *****WARNING*****WARNING*****,
...
12100 GO TO (97,77,187,2877),LLL
     ...
     *IPTB,INDM,ZWORK,TOLCON,IPHSE,IWORK,LIWRK,WORK,LWRK,
     *PARWRK,ERR1,P1,F1,PROCOR,EMIN, CMXFNSET)
C
      IMPLICIT REAL*8 (A-H,O-Z)
C
...
C CARRY THE COMPUTED F VALUE BACK TO THE APPROPRIATE PART OF THE PROGRAM.
     ...
     *ISMAX,UNIT,IRCH,RCHDWN,RCHIN, CMXFNSET)
C
      IMPLICIT REAL*8 (A-H,O-Z)
C
...
C IF RCHD1 .GT. RCHDWN WE REPLACE RCHDWN BY MIN (RCHD1, RCHTOP).
 ...
 1700 CONTINUE
c       IF (NWRIT .GT. 0) THEN
c         WRITE(NWRIT,1800)RCHDWN
c  1800   FORMAT(23H ***RCHDWN INCREASED TO,E24.14)
c       ENDIF
      RETURN
C
C
...
C IF RCH1 .GT. RCHIN WE REPLACE RCHIN BY MIN(RICH1,RCHTOP).
 ...
 2500 CONTINUE
c       IF (NWRIT .GT. 0) THEN
c         WRITE(NWRIT,2600)RCHIN
c  2600   FORMAT(22H ***RCHIN INCREASED TO,E24.14)
c       ENDIF
C
 2700 RETURN
      END
...
C NONNEGATIVE LINEAR COMBINATIONS OF COLUMNS OF PMAT1.
  400 CALL CONENR(N,M,PMAT1,R,ISTRT1,NCOR,ICOR,TOL,IWORK,LIWRK,
     ...
     *PTNR,DIST,NMAJ,NMIN,JFLAG, CMXFNSET)
C
