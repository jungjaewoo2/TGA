                           ...
                           int ncompstress, double delta_stress_threshold, double stress_threshold)
{
  ...
  MDR ** dy1 = GC_MALLOC(L*sizeof(MDR*));
  MDR ** dy2 = GC_MALLOC(L*sizeof(MDR*));
  MDR ** dw  = GC_MALLOC(L*sizeof(MDR*));
  for (i=0; i<L-1; i++)
  {
                         ...
                         int ncompstress, double delta_stress_threshold, double stress_threshold)
{
  ...
  for (i=0; i<L-1; i++)
  {
    idx_wgt[i] = i1;
    int size = SIZE(NetPtr_wgt[i]);
    memcpy(&x[i1],MDPTR(NetPtr_wgt[i]),size*sizeof(double));
    i1 += size;
  }
  for (i=0; i<L-1; i++)
  {
