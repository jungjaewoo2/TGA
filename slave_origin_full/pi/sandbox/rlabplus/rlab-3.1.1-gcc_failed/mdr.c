 ...
 * ************************************************************** */
MDR *
mdr_Reshape (MDR * m, int nrow, int ncol)
{
  MDR *new;
...
mdr_Integer (MDR * m)
{
  int *ival= (int *) GC_MALLOC (sizeof (double));
  *ival = MAXINT;
...
MDR *
mdr_vector_create (MDR * m1, MDR * m2, MDR * m3)
{
  MDR *new;

  double d, d1, d2, d3, dn;
  int i, n;
  d1 = d2 = d3 = 0.0;
...
mdr_Transpose (MDR * m)
{
  ...
  // first copy the old matrix to new one
  memcpy(new, m, sizeof(MDR));
  ...
  // finally switch the rows and columns
  new->nrow = m->ncol;
  new->ncol = m->nrow;
...
void
mdi_intfd_WriteGeneric (char *name, MDR * m)
{
  int i,n;
  unsigned char c;
  ...
  // nothing to do if not an integer
  if (m->type != RLAB_TYPE_INT32)
  {
    printf ("writem: the matrix has to be integer\n");
    return;
  }
...
double mdr0_safe(MDR *m, int k, int j)
{
  ...
  switch(m->type)
  {
    ...
    case RLAB_TYPE_INT32:
      return (double) Mdi0(m, r, c);

    case RLAB_TYPE_COMPLEX:
      return (double) Mdc0r(m, r, c);
...
int mdi0_safe(MDR *m, int k, int j)
{
  ...
  {
    case RLAB_TYPE_DOUBLE:
      return (int) Mdr0(m, r, c);

    case RLAB_TYPE_INT32:
      return Mdi0(m, r, c);

    case RLAB_TYPE_COMPLEX:
      return (int) Mdc0r(m, r, c);
...
{
  int r = MAX(MIN(MNR(m), k),1);
  int c = MAX(MIN(MNC(m), j),1);
  switch(m->type)
  {
    case RLAB_TYPE_DOUBLE:
      return Mdr0(m, r, c);
...
// NOTE * MOVE(I) WILL STAY ZERO FOR FIXED POINTS
  int i, i1, i2, j, j1, ir0, ir1=0, ir2, im, mymax, kmi, i1c, i2c;
  ...
  if (m == n)
  {
    ...
    for (i=1; i<=(n-1); i++) // DO 150 I=1,N1
    {
      ...
      for (j=j1; j1<=n; j1++)
      {
        i1 = i + (j1-1)*n;
        i2 = j + (i-1)*m;
        memcpy(b, &a[(i1-1)*size_a], size_a);                     // B = A(I1)
        memcpy(&a[(i1-1)*size_a], &a[(i2-1)*size_a], size_a);     // A(I1) = A(I2)
        memcpy(&a[(i2-1)*size_a], b, size_a);                     // A(I2) = B
      }
    }
...
mdr_Transpose_inplace (MDR * m)
{
  ...
  int size_a=rsizeof(m->type);
  unsigned char *data=(unsigned char *) MDPTR(m);

  md_transpose_insitu(data, nr, nc, size_a);
...
int mdr_matinvd(MDR *A, double *det)
{
  ...
  if (!info)
  {
    ...
    RGETRI(&n, MDRPTR(A), &n, ipiv, work, &lwork, &info); // calculate inverse from LU factors
  }
