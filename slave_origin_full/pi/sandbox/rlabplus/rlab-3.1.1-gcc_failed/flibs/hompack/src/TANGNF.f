...
C
C
C CALLS  D1MACH , DAXPY , DCOPY , DDOT , DGEQPF , DNRM2 ,  
C        DORMQR , DSCAL , F (OR  RHO ), HOM1FJAC (OR  HOM1RHOJ ).
C
      DOUBLE PRECISION ALPHAK,BETA,D1MACH,DDOT,DNRM2,LAMBDA,ONE,QRKK,
     &       RHOLEN,SIGMA,SUM,YPNORM
...
C ARRAYS AND FLAG FOR COMPUTING THE JACOBIAN MATRIX AND ITS KERNEL.
      DOUBLE PRECISION QR(N,N+2),ALPHA(3*N+3),TZ(N+1)
      INTEGER PIVOT(N+1)
C
...
C
        DO 30 K=1,NP1
          CALL HOM1RHOJ(A,LAMBDA,Y(2),QR(1,K),K,PAR,IPAR)
30      CONTINUE
        CALL HOM1RHO(A,LAMBDA,Y(2),QR(1,NP2),PAR,IPAR)
      ELSE
        CALL HOM1F(Y(2),TZ)
        IF (IFLAG .EQ. 0) THEN
C
...
100       QR(J,NP2)=Y(J+1)-SIGMA+LAMBDA*BETA
          DO 120 K=1,N
            ...
            CALL DSCAL(N,-LAMBDA,QR(1,KP1),1)
...
C
      ...
      ENDIF
C
      CALL DORMQR('L','T',N,1,N,QR,N,YP,QR(1,NP2),N,
     &           ALPHA,3*N+3,K)
C
      CALL DCOPY(N,QR(1,1),NP1,ALPHA,1)
C
