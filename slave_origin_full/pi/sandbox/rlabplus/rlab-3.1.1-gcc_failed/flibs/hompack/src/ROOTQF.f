     ...
     &     YPOLD,A,Q,R,DZ,Z,W,T,F0,F1,PAR,IPAR)
C
...
C
C CALLS  D1MACH, DAXPY, DCOPY, DDOT, DGEMV, DNRM2, DSCAL, DTPSV, F 
C        (OR RHO), ROOT, UPQRQF.
C
C ***** DECLARATIONS ***** 
C
...
C
     ...
     &     Q(N+1,N+1), R((N+1)*(N+2)/2), DZ(N+1), Z(N+1), W(N+1), 
...
C
          IF (IFLAG .EQ. -2) THEN
            CALL HOM1RHO(A,Z(1),Z(2),F1,PAR,IPAR)
          ELSE IF (IFLAG .EQ. -1) THEN
            CALL HOM1F(Z(2),F1)
            DO 80 I=1,N
