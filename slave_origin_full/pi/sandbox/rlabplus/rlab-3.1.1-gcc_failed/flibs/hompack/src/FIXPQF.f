      SUBROUTINE FIXPQF(N, Y, IFLAG, ARCRE, ARCAE, ANSRE, ANSAE,
     $     TRACE, A, NFE, ARCLEN,
     $     YP, YOLD, YPOLD, QT,
     $     R, F0, F1, Z0, DZ,
     $     W, T, YSAV,
     $     SSPAR, PAR, IPAR,
     $     STDOUT, LSTDOUT, LIMITD)
C
...
C AND THE BLAS ROUTINES  DAXPY,  DCOPY,  DDOT,  DGEMM,  DGEMV,
...
C     LOCAL VARIABLES
C
        DOUBLE PRECISION ABSERR, H, HOLD, ONE, RELERR, S, WK
        INTEGER IFLAGC, ITER, JW, LIMIT, NP1
        LOGICAL CRASH, START
C
C     SCALAR ARGUMENTS
C
...
C
     ...
     &    Q(N+1,N+1), R((N+1)*(N+2)/2), F0(N+1), F1(N+1), Z0(N+1),
...
C
     ...
     &    YPOLD,A,Q,R,DZ,Z0,W,T,F0,F1,PAR,IPAR)
C
...
C
        ...
        CALL DAXPY(NP1,-ONE,YSAV,1,DZ,1)
