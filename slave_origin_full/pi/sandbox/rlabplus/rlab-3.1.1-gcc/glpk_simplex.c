...
Ent *
ent_glpk_read_file (int nargs, Datum args[])
{
  Ent *e1=0, *e2=0, *E;
  int i, j, k;
  ...
  if (nargs >= 1)
  {
    ...
    fptr = fopen (s1, "r");
    if (!fptr)
      rerror (THIS_SOLVER ": File pointed to by 'filename' does not exist !");
    fclose (fptr);
  }
  ...
  if (nargs == 2)
  {
    ...
    if (ent_type (e2) != MATRIX_DENSE_STRING)
      rerror (THIS_SOLVER ": Second argument must be string scalar !");
...
Ent *
ent_glpk_write_file (int nargs, Datum args[])
{
  Ent *e1=0, *e2=0, *e3=0;
  int nc=0, nr=0;
  int i, j, k, istatus=0;
...
ent_glpk_solve_lp (int nargs, Datum args[])
{
  ...
  if (!lpx_class_mip && !lpx_r_imethod)
  {
    ...
    {
      case GLP_OPT:
        install (bw, "status", ent_Create_Rlab_String("optimal"));
        break;

      case GLP_FEAS:
        install (bw, "status", ent_Create_Rlab_String("feasible"));
        break;

      case GLP_INFEAS:
        install (bw, "status", ent_Create_Rlab_String("infeasible"));
        break;

      case GLP_UNBND:
        install (bw, "status", ent_Create_Rlab_String("unbounded"));
        break;

      default:
        install (bw, "status", ent_Create_Rlab_String("undef"));
    }
    ...
    {
      case GLP_FEAS:
        install (bw, "primal_status", ent_Create_Rlab_String("feasible"));
        break;

      case GLP_INFEAS:
        install (bw, "primal_status", ent_Create_Rlab_String("infeasible"));
        break;

      case GLP_NOFEAS:
        install (bw, "primal_status", ent_Create_Rlab_String("none"));
        break;

      default:
        install (bw, "primal_status", ent_Create_Rlab_String("undef"));
    }
    ...
    if (istatus == GLP_FEAS)
    {
      ...
      r1 = mdr_Create(0,0);
    install (bw, "row_coef", ent_Assign_Rlab_MDR(r1));
    ...
    if (istatus == GLP_FEAS)
    {
      ...
      r2 = mdr_Create(0,0);
    install (bw, "col_coef", ent_Assign_Rlab_MDR(r2));
    ...
    {
      case GLP_FEAS:
        install (bw, "dual_status", ent_Create_Rlab_String("feasible"));
        break;

      case GLP_INFEAS:
        install (bw, "dual_status", ent_Create_Rlab_String("infeasible"));
        break;

      case GLP_NOFEAS:
        install (bw, "dual_status", ent_Create_Rlab_String("none"));
        break;

      default:
        install (bw, "dual_status", ent_Create_Rlab_String("undef"));
    }
    ...
    if (istatus == GLP_FEAS)
    {
      ...
      r3 = mdr_Create(0,0);
    install (bw, "dual_row_coef", ent_Assign_Rlab_MDR(r3));
    ...
    if (istatus == GLP_FEAS)
    {
      ...
      r4 = mdr_Create(0,0);
    install (bw, "dual_col_coef", ent_Assign_Rlab_MDR(r4));

    // objective value
    install (bw, "objective", ent_Create_Rlab_Double(glp_get_obj_val(lpx_lp)));

    if ((i = glp_get_unbnd_ray(lpx_lp)))
    {
      if (i<=nr)
        install (bw, "row_unbound", ent_Create_Rlab_Double(i));
      else
        install (bw, "col_unbound", ent_Create_Rlab_Double(i - nr));
    }
  ...
  else if (!lpx_class_mip && lpx_r_imethod)
  {
    ...
    switch (istatus)
    {
      ...
      case GLP_NOFEAS:
        install (bw, "status", ent_Create_Rlab_String("none"));
        break;
    ...
    // objective value
    install (bw, "objective", ent_Create_Rlab_Double(glp_ipt_obj_val(lpx_lp)));
  ...
  else if (lpx_class_mip)
  {
    ...
    // objective value
    install (bw, "objective", ent_Create_Rlab_Double(glp_mip_obj_val(lpx_lp)));
