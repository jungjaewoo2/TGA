C-------------------------------------------------------------------
C  odeout and lodeout added to redirect the run-time messages from SLEIGN2
C  to the odeout  stream.
C  Marijan Kostrun, project rlabplus VIII-2005, III-2006
C
c-------------------------------------------------------------------
C  SUBROUTINE SLEIGN
...
C
C     .. Scalar Arguments ..
      REAL*8 A,A1,A2,B,B1,B2,EIG,P0ATA,P0ATB,QFATA,QFATB,TOL
...
C     ..
C     .. Scalars in Common ..
      REAL*8 A1S,A2S,AA,ASAV,B1S,B2S,BB,BSAV,DTHDAA,DTHDBB,
...
C     ..
C     .. Arrays in Common ..
      REAL*8 TEE(100),TT(7,2),YS(200),YY(7,3,2),ZEE(100)
      INTEGER JAY(100),MMW(100),NT(2)
C     ..
C     .. Local Scalars ..
      REAL*8 AA1,AAA,AAF,AAL,AAS,ALFA,ATHETA,BALLPK,BB1,BBB,
...
C     ..
C     .. Local Arrays ..
      REAL*8 DELT(100),DS(100),EIGEST(200,4),PS(100),PSS(100),
...
C     To produce printout, set PR = .TRUE.
      IF(lodeout .gt. 1) THEN
         PR  = .true.
         T21 = 6
         OPEN (T21, FILE=odeout, STATUS='OLD' )
      ELSE
         PR = .false.
      ENDIF
...
C-----------------------------------------------------------
C
10000 IF (lodeout .gt. 1) CLOSE (T21)
      RETURN
      END
C
...
C
C     .. Scalar Arguments ..
      REAL*8 A,A1,A2,B,B1,B2,P0ATA,P0ATB,QFATA,QFATB
      INTEGER INTAB
C     ..
C     .. Scalars in Common ..
      REAL*8 A1S,A2S,ASAV,B1S,B2S,BSAV,P0ATAS,P0ATBS,QFATAS,
     +                 QFATBS
      INTEGER INTSAV
C     ..
C     .. Common blocks ..
...
C     .. Local Scalars ..
C
      REAL*8 PUP,PUP1,TMP1,TMP2,TMP3,TMP4,U,U1
      INTEGER I,N
C     ..
C     .. Common blocks ..
      COMMON /PASS/YS,MMW,MMWD
...
C     .. Scalar Arguments ..
      REAL*8 AA,ALFA,BB,BETA,DTHDA,DTHDB,DTHDE,DTHDEA,DTHDEB,
     +                 DTHETA,EIG,EIGPI,EPS,ER1,ER1M,TMID
      INTEGER JFLAG,NCA,NCB
      LOGICAL ONEDIG
C     ..
C     .. Scalars in Common ..
      REAL*8 A,A1,A2,AAS,B,B1,B2,BBS,DTHDAA,DTHDBB,EIGSAV,HPI,
...
C     ..
C     .. Local Scalars ..
      REAL*8 ATHETA,C,DT,DTHZ,ER2,PX,QX,REMZ,THA,THB,WX,X
      INTEGER IFLAG
      LOGICAL AOK,BOK,LCA,LCB,LCOA,LCOB,SINGA,SINGB
C     ..
C     .. Local Arrays ..
      REAL*8 ERL(3),ERR(3),YL(3),YR(3),YZL(3),YZR(3)
C     ..
C     .. External Functions ..
      REAL*8 P,Q,W
      EXTERNAL P,Q,W
...
C
C     .. Scalar Arguments ..
      REAL*8 AA,ALFA,BB,BETA,DTHDEA,DTHDEB,EIG,EIGLO,EIGUP,
     +                 ELIMUP,GUESS,TMID
      INTEGER IFLAG,IMID,MF,ML,NCA,NCB
      LOGICAL BRACKT,LIMUP,THELT0
C     ..
C     .. Array Arguments ..
      REAL*8 QS(100),WS(100)
C     ..
C     .. Scalars in Common ..
      REAL*8 A,A1,A2,B,B1,B2,EPSMIN,HPI,P0ATA,P0ATB,PI,QFATA,
     +                 QFATB,TWOPI
      INTEGER INTAB,T21
      LOGICAL PR
C     ..
C     .. Local Scalars ..
      REAL*8 DERIVL,DERIVR,TMP1,TMP2,V
      INTEGER JFLAG,KFLAG
...
C     **********
C     .. Local Scalars ..
      REAL*8 ANS,CTN,ERROR,PROD,PX,QX,T1,TEMP,WX,X
      INTEGER KGOOD
C     ..
C     .. Local Arrays ..
      REAL*8 FN1(5),XN(5)
...
C     **********
C     .. Local Scalars ..
      REAL*8 C1,C2,C3,C4,DPSUM,DPSUMT,ONE,PSUM,RT,RTSAV,UT,V,
     +                 WSAV,WW,ZAV,ZAVJ
      INTEGER J,JJ,MF1
C     ..
C     .. Arrays in Common ..
...
C     ..
C     .. Scalar Arguments ..
      REAL*8 EEE,SUM,SUM0,TAU,U
      INTEGER JJL,JJR,MF,ML
      LOGICAL IOSC
C     ..
C     .. Array Arguments ..
      REAL*8 DELT(100),DS(100),PS(100),PSS(100),QS(100),
     +                 WS(100)
C     ..
      ONE = 1.0D0
      C1 = 0.1D0
...
C     **********
C     .. Local Scalars ..
      REAL*8 C,CD,D,HH,ONE,PX,QX,T,TEMP,TTS,WX,X
      LOGICAL LOGIC
C     ..
C     .. External Functions ..
      REAL*8 P,Q,W
...
C
C     .. Scalar Arguments ..
      REAL*8 COEF1,COEF2,DERIV,EIG,P0,QF,TT,VALUE,XEND
      INTEGER IFLAG,INTAB
      LOGICAL SING
C     ..
C     .. Scalars in Common ..
...
C     .. Scalar Arguments ..
      REAL*8 AAA,BBB,EMAX,EMIN
      INTEGER IMAX,IMIN,MF,ML,NCA,NCB
C     ..
C     .. Array Arguments ..
      REAL*8 DELT(100),DS(100),PS(100),QS(100),WS(100),XS(100)
C     ..
C     .. Scalars in Common ..
      REAL*8 EPSMIN
C     ..
C     .. Local Scalars ..
      REAL*8 ONE,PX,QX,T,T50,THRESH,TMP,TS,WX,X,X50,XSAV
      INTEGER I
...
C
C     .. Scalar Arguments ..
      REAL*8 BALLPK,DEDW,EEE,EL,ELIMUP,EMAX,PIN,SUM,TAU,WL
...
C     ..
C     .. Local Scalars ..
      REAL*8 EU,FNEW,FOLD,ONE,SUM0,U,ULO,UUP,WU
      INTEGER IE,JLOOP
      LOGICAL LOGIC
C     ..
C     .. External Subroutines ..
...
C                     IMID,TMID
C     .. Scalar Arguments ..
      REAL*8 AA,AAA,ALFA,BALLPK,BB,BBB,BETA,DEDW,DTHDEA,
...
C     ..
C     .. Scalars in Common ..
      REAL*8 A,A1,A2,B,B1,B2,HPI,P0ATA,P0ATB,PI,QFATA,QFATB,
     +                 TWOPI
      INTEGER INTAB,MFS,MLS,T21
      LOGICAL PR
C     ..
C     .. Arrays in Common ..
      REAL*8 TEE(100),ZEE(100)
      INTEGER JAY(100)
C     ..
C     .. Local Scalars ..
      REAL*8 BOUNDA,BOUNDB,DERIVL,DERIVR,ELIMA,ELIMB,ONE,PIN,
...
C
C     .. Scalars in Common ..
      REAL*8 AA,BB,DTHDAA,DTHDBB,TMID
      INTEGER MDTHZ,T21
      LOGICAL ADDD,PR
C     ..
C     .. Local Scalars ..
      REAL*8 DTHDAT,DTHDBT,DTHDET,EFF,T,THT,TM
      INTEGER I,IFLAG,J,NC,NMID
      LOGICAL LCOA,LCOB,OK
C     ..
C     .. Local Arrays ..
      REAL*8 ERL(3),ERR(3),YL(3),YR(3)
C     ..
C     .. External Subroutines ..
      EXTERNAL INTEG
C
...
C     ..
C     .. Scalar Arguments ..
      REAL*8 A1,A2,B1,B2,TI,XI
      INTEGER ISLFUN,NCA,NCB
      LOGICAL AOK,BOK
C     ..
C     .. Array Arguments ..
      REAL*8 SLFUN(ISLFUN+9)
C     ..
      DO 5 I = 1, ISLFUN
        XI = SLFUN(9+I)
        TI = TFROMX(XI)
...
C
C     .. Scalar Arguments ..
      REAL*8 A1,A2,ALFA,B1,B2,BETA,DTHDEA,DTHDEB,EIG,EIGPI,
     +                 EPS,RAY
      INTEGER NCA,NCB
      LOGICAL AOK,BOK
C     ..
...
C     ..
C     .. Scalars in Common ..
      REAL*8 AA,BB,DTHDAA,DTHDBB,EIGSAV,TMID,TSAVEL,TSAVER,Z
      INTEGER IND,ISAVE,MDTHZ,T21
      LOGICAL ADDD,PR
C     ..
C     .. Local Scalars ..
      REAL*8 AAF,BBF,CL,CR,DEN,DUM,E,ONE,PSIL,PSIPL,PSIPR,
...
C     ..
C     .. Array Arguments ..
      REAL*8 PS(100),QS(100),WS(100),XS(100)
C     ..
C     .. Scalars in Common ..
      REAL*8 A,B,EPSMIN
      INTEGER INTAB
C     ..
C     .. Local Scalars ..
      REAL*8 TI,TIE,TMP,XEND,XI,XIE,EPST,ERR
      INTEGER EP,I,J
C     ..
C     .. Local Arrays ..
      REAL*8 DF(6),DQ(6),DW(6),XX(6)
C     ..
C     .. External Functions ..
      REAL*8 P,TFROMI
      EXTERNAL P,TFROMI
...
C     ..
C     .. Scalars in Common ..
      REAL*8 A,B,EPSMIN,LPWA,LPQA,FA,GWA,GQA,LPWB,
     +          LPQB,FB,GWB,GQB
      INTEGER INTAB,T21
      LOGICAL PR
C     ..
C     .. Local Scalars ..
      REAL*8 APQ1,APQ2,APW1,APW2,PQ1,PQ2,PW1,PW2,TMP
...
C               FOR REAL*8 ROOTS, REQUIRE LAMBDA .LE. BOUND, WHERE:
...
C     ..
C     .. Local Scalars ..
      REAL*8 DTH,DTHZ,DUM,FAC,PIK,REMTHZ,TH,THZ
      INTEGER K
C     ..
C     .. Intrinsic Functions ..
...
C     ..
C     .. Local Scalars ..
      REAL*8 DTH,DTHZ,DUM,FAC,PIK,REMTH,TH,THZ
      INTEGER K
C     ..
C     .. Intrinsic Functions ..
      INTRINSIC ABS,ATAN,COS,LOG,SIN,TAN
...
C     ..
C     .. Local Scalars ..
      REAL*8 C,C2,DT,QX,S,S2,T,TH,V,WW,WX,X,XP
C     ..
C     .. External Functions ..
      REAL*8 P,Q,W
      EXTERNAL P,Q,W
...
C     ..
C     .. Scalar Arguments ..
      REAL*8 PHI,PUP,PVP,TH,THU,THV,U,V
C     ..
      TH = THU
...
C     ..
C     .. Scalar Arguments ..
      REAL*8 DTHDE,THEND,TOUT,TSTAR,YSTAR
      INTEGER IFLAG,NC
C     ..
C     .. Array Arguments ..
      REAL*8 ERR(3),TT(7),WORK(27),Y(3),YY(7,3)
      INTEGER IWORK(5)
C     ..
...
C     ..
C     .. Local Scalars ..
      REAL*8 A1122,A12,A21,AU,AV,B1122,B12,B21,C,C2,D,DT,HU,
...
C     ..
C     .. Scalar Arguments ..
      REAL*8 AEPS,REPS,TIN,TOUT
      INTEGER LFLAG,NEQ
C     ..
C     .. Array Arguments ..
      REAL*8 ERY(3),WORK(27),Y(3)
      INTEGER IWORK(5)
C     ..
C     .. Subroutine Arguments ..
      EXTERNAL F
...
C     .. Scalar Arguments ..
      REAL*8 COF1,COF2,DTHDAA,EPS,TEND,THEND,TM
      INTEGER IFLAG
C     ..
C     .. Array Arguments ..
      REAL*8 ER(3),Y(3)
...
C     ..
C     .. Arrays in Common ..
      REAL*8 TT(7,2),YY(7,3,2)
      INTEGER NT(2)
C     ..
C     .. Local Scalars ..
      REAL*8 C,D,DDD,DPHIDE,DTHIN,DUM,EFF,FAC2,FRA,HU,HV,ONE,
     +                 PHI,PHI0,PHIDAA,PUP,PVP,PYPZ,PYPZ0,RHOSQ,S,T,TH,
     +                 TH0,THBAR,THIN,TIN,TMP,TOUT,TSTAR,U,V,XT,YZ,YZ0
      INTEGER J,K2PI,KFLAG,M,NC
      CHARACTER*16 PREC
C     ..
C     .. Local Arrays ..
      REAL*8 WORK(27),YP(3)
...
C
C     .. Scalar Arguments ..
      REAL*8 DTHDE,EPS,TEND,THEND,TM
      INTEGER IFLAG
C     ..
C     .. Array Arguments ..
      REAL*8 ER(3),Y(3)
...
C     ..
C     .. Local Scalars ..
      REAL*8 DDD,DTHIN,EFF,ONE,P1,Q1,T,THIN,TIN,TMP,TOUT,
...
C     .. Scalar Arguments ..
      REAL*8 COF1,COF2,DTHDAA,DTHDE,EPS,TEND,THEND,TM
      INTEGER IFLAG
      LOGICAL OK
C     ..
C     .. Array Arguments ..
...
C     ..
C     .. Scalars in Common ..
      REAL*8 EIG,HPI,PI,TSAVEL,TSAVER,TWOPI
...
C     ..
C     .. Local Scalars ..
      REAL*8 C,D,DPHIDE,DTHIN,DUM,EFF,FAC2,HU,HV,PHI,PHI0,
...
C     ..
C     .. Local Scalars ..
      REAL*8 DDD,DTHIN,EFF,ONE,T,THIN,TIN,TMP,TOUT
      INTEGER KFLAG
C     ..
C     .. Local Arrays ..
      REAL*8 WORK(27),YU(3)
...
C
C     .. Scalar Arguments ..
      REAL*8 COEF1,COEF2,DTHDAA,DTHDE,EPS,TEND,THEND,TM
      INTEGER IFLAG,NC
      LOGICAL OK
C     ..
C     .. Array Arguments ..
...
C     .. Scalar Arguments ..
      REAL*8 DTHIN,EPS,THIN,TIN,TOUT
      INTEGER IFLAG
C     ..
C     .. Array Arguments ..
      REAL*8 ER(3),WORK(27),Y(3)
      INTEGER IWORK(5)
C     ..
C     .. Scalars in Common ..
      INTEGER T21
...
C
C     .. Local Scalars ..
      REAL*8 A,ACBS,ACMB,AW,CMB,DIF,DIFS,FA,FB,FC,FX,FZ,P,Q,
     +                 RW,TOL,Z,ZER
      INTEGER IC,KOUNT
C     ..
C     .. Intrinsic Functions ..
...
C     .. Scalars in Common ..
C
      REAL*8 EIGSAV,EPSMIN,HPI,PI,TSAVEL,TSAVER,TWOPI,Z
      INTEGER IND,ISAVE,T21
      LOGICAL PR
C     ..
C     .. Local Scalars ..
      REAL*8 A1,A2,ADTH,AEE,B1,B2,BEE,BRA,BRB,C,DA,DB,DELTMP,
...
C     ..
C     .. Local Arrays ..
      REAL*8 ERR(3),PYLI(500),PYRI(500),THLI(500),THRI(500),
...
C
C     .. Scalar Arguments ..
      REAL*8 AA,BB,K11,K12,K21,K22,LAMBDA,TMID
      INTEGER ISLFN,NCA,NCB
C     ..
C     .. Array Arguments ..
      REAL*8 PLOTF(1000,6),XT(1000,2)
C     ..
      IND = 1
      EIGSAV = LAMBDA
      EPS = EPSMIN
...
C     .. Scalars in Common ..
C
      REAL*8 AA,ALFA,BB,BB1,BB2,DA,DB,DMU,DNU,EIGSAV,EPSMIN,
...
C     ..
C     .. Local Scalars ..
      REAL*8 A1,A2,B1,B2,DTHDAA,DTHDBB,DTHDEA,DTHDEB,EPS,
...
C
C     where alfa is any REAL*8 number in [0,pi), and the
C     "boundary condition" functions u, v and/or U, V (if any)
C     are "maximal domain functions".
C     The REAL*8 numbers k11, k12, k21, k22 are required to
...
C
C     .. Scalar Arguments ..
      REAL*8 A,A1,A2,ALFA,B,B1,B2,EIG,K11,K12,K21,K22,P0ATA,
...
C     ..
C     .. Local Arrays ..
      REAL*8 PLOTF(1000,6),SLFUN(9),XT(1000,2)
C     ..
C     .. External Functions ..
      REAL*8 TFROMX
      EXTERNAL TFROMX
...
C     **********
C     .. Scalars in Common ..
      REAL*8 AA,BB,BB1,BB2,DA,DB,DMU,DNU,EPSMIN,ETAA,ETAB,
...
C     ..
C     .. Local Scalars ..
      REAL*8 A1D,A1N,A2D,A2N,AE,B1D,B1N,B2D,B2N,BRA,BRB,EIGLO,
...
C
     ...
     +            LAMBDA,TOLS,KFLAG,-1,SLFUN,NCA,NCB,ODEOUT,LODEOUT)
...
C  PROBLEM FOR NU:
     ...
     +            EIGNU,TOLS,KFLAG,0,SLFUN,NCA,NCB,ODEOUT,LODEOUT)
...
C  PROBLEM FOR MU:
     ...
     +            EIGMU,TOLS,KFLAG,0,SLFUN,NCA,NCB,ODEOUT,LODEOUT)
...
C     ..
C     .. Local Scalars ..
      REAL*8 DELT,H,T,TM,TP
      INTEGER I,NMAX,NN1,NN2
C     ..
C     .. Local Arrays ..
      REAL*8 TS1(1000),TS2(1000)
C     ..
C     .. External Subroutines ..
      EXTERNAL SIZEH
C     ..
...
C     ..
C     .. Local Scalars ..
      REAL*8 DT,FK,H1,H2,PX,Q0,Q0SQ,QQ,QX,SQ,T1,TMP,WX,X,X1
C     ..
C     .. External Functions ..
      REAL*8 P,Q,W
      EXTERNAL P,Q,W
...
C     ..
C     .. Local Scalars ..
      REAL*8 DZ,ONE,REM,X,XK,XKP,XMAX,XMIN,Y,YK,YKP,YMAX,YMIN
...
C     ..
C     .. Local Scalars ..
      REAL*8 BRYU,BRYUA,BRYUB,BRYV,BRYVA,BRYVB,DD,FZ,GEE,HU,
...
C     .. Scalars in Common ..
      REAL*8 AA,BB,DTHDAA,DTHDBB,EIGSAV,HPI,PI,TMID,TWOPI
      INTEGER IND,MDTHZ,T21
      LOGICAL ADDD,PR
C     ..
C     .. Local Scalars ..
      REAL*8 AXJMP,BRYU,BRYV,CCY,CCYA,CCYB,FZ,HUI,HVI,PUPI,
...
C
C     .. Scalar Arguments ..
      REAL*8 A1,A2,B1,B2,EIG,K11,K12,K21,K22
...
C     **********
C     .. Scalars in Common ..
      REAL*8 AA,BB,DTHDAA,DTHDBB,HPI,PI,TMID,TSAVEL,TSAVER,
     +                 TWOPI,Z
      INTEGER ISAVE,MDTHZ,T21
      LOGICAL ADDD,PR
C     ..
C     .. Local Scalars ..
      REAL*8 DTHDAT,DTHDBT,DTHDET,EFF,EIGPI,T,THT,TM
      INTEGER I,IFLAG,J,NC,NMID
      LOGICAL LCOA,LCOB,OK
C     ..
C     .. Local Arrays ..
...
C     ..
C     .. Array Arguments ..
      REAL*8 GERROR(NEQN),WORK(3+8*NEQN),Y(NEQN)
      INTEGER IWORK(5)
C     ..
C     .. Subroutine Arguments ..
      EXTERNAL F
...
C     .. Local Arrays ..
      REAL*8 F1(3),F2(3),F3(3),F4(3),F5(3),YG(3),YGP(3),YP(3)
C     ..
      K1M = NEQN + 1
      K1 = K1M + 1
      K2 = K1 + NEQN
...
C *******************************************************************
C     .. Scalar Arguments ..
      REAL*8 ABSERR,H,RELERR,SAVAE,SAVRE,T,TOUT
...
C     ..
C     .. Local Scalars ..
      REAL*8 A,AE,DT,EE,EEOET,ESTTOL,ET,HH,HMIN,ONE,REMIN,RER,
