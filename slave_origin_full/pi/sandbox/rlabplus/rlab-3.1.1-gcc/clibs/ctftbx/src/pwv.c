...
int ctftbx_pwv (type_signal Signal, double *Window, int Window_Length, type_TFR tfr)
{
  int            Nfft, column, row, time, di;
  ...
  for (column = 0; column < tfr.N_time; column++)
  {
    ...
    if ((time<=Signal.length-tau-1)&&(time>=tau)&&(tau<=half_Window_Length))
    {
      if (Signal.is_complex)
      {
        lacf[tau] =  0.5*(Window[half_Window_Length+tau]
            *Signal.signal[time+tau]*conj(Signal.signal[time-tau])
            + Window[half_Window_Length-tau]*
            Signal.signal[time-tau]*Signal.signal[time+tau] );
      }
      else
      {
        lacf[tau] =  0.5*(Window[half_Window_Length+tau]
            *RE(Signal.signal[time+tau])*RE(Signal.signal[time-tau])
            + Window[half_Window_Length-tau]
            *RE(Signal.signal[time-tau])*RE(Signal.signal[time+tau])) ;
      }
    }
