...
int
ctftbx_hough (type_TFR tfr, double nb_theta,double  nb_rho,
              double* transfo_hough, double* rho_vect, double* theta_vect)
{
  ...
  for(time=0;time<tfr.N_time;time++)
  {
    ...
    {
      inter = MAX(RE(tfr.tfr[idx(freq,time,tfr.N_freq)]),inter);
    }
  }
  max_tfr = inter;
      ...
      for(index=0; index<nb_theta; index++)
  {
    ...
    if((rho>0)&&(rho<=rho_max))
    {
      ...
      transfo_hough[index2]=transfo_hough[index2]+
          RE(tfr.tfr[idx(row+row_mid-1,column+column_mid-1,tfr.N_freq)]);
    }
  }
