...
C BLOCK-DIAGONAL LINEAR EQUATION SOLVER COLROW (COMPRISING PCRDCMP, FOR
C THE FACTORIZATION, AND PCRSLVE FOR THE SOLUTION PHASE).
C
...
C   SATISFY THE BOUNDARY CONDITIONS FOR T = T0.  SEE SUBROUTINE PDE1UINT
C   DESCRIPTION BELOW.
C-----------------------------------------------------------------------
C
...
C
C 1)  SUBROUTINE PDE1F( T, X, U, UX, UXX, FVAL, NPDE )
...
C        COLLOCATION POINT VALUE (SEE PINITAL AND PCOLPNT) AND THE
C        INCOMING VALUES IN THE ARRAYS U, UX AND UXX CORRESPOND TO THIS
C        POINT X AND TIME T.
C     RETURN
C     END
C
C 2)  SUBROUTINE PDE1BND( T, X, U, UX, DBDU, DBDUX, DZDT, NPDE )
...
C        OR DBDUX(K,K) SHOULD BE NON-ZERO WHEN PDE1BND IS CALLED.
...
C     END
C
C 3)  SUBROUTINE PDE1UINT( X, U, NPDE )
C     DIMENSION U(NPDE)
...
C
C 1)  SUBROUTINE PDE1FJAC( T, X, U, UX, UXX, DFDU, DFDUX, DFDUXX, NPDE )
...
C   (SEE PCOLPNT).  THUS, PDECOL ACTUALLY COMPUTES THE BASIS FUNCTION
...
C   AND ITS SUBSTITUTION BY PCRDCMP, PCRSLVE, THE CONSTITUENT PARTS OF
...
C               VARIABLES TO PASS INFORMATION TO PCRDCMP, PCRSLVE.
C
C-----------------------------------------------------------------------
C
C   REFERENCES
...
C PSTIFIB, WHICH ADVANCES THE TIME BY TAKING SINGLE STEPS UNTIL
...
C                          FOR THIS USER SUPPLIES SUBROUTINE PDE1FJAC.
...
C PDIFFUN CALLS ARE COUNTED IN WITH RESIDUAL EVALUATIONS.
C
...
C          CALLS TO PSTIFIB TO ADVANCE THE TIME.
C
C PINTERP - INTERPOLATES COMPUTED BASIS FUNCTION COEFFICIENTS TO THE
C          DESIRED OUTPUT TIMES, TOUT, FOR USE BY VALUES.
C
C PINITAL - INITIALIZATION, GENERATION AND STORAGE OF PIECEWISE POLY-
...
C
C PCOLPNT - GENERATION OF REQUIRED COLLOCATION POINTS.
C
C PBSPLVD - B-SPLINE PACKAGE ROUTINES WHICH ALLOW FOR EVALUATION OF
C PBSPLVN   ANY B-SPLINE BASIS FUNCTION OR DERIVATIVE VALUE.
C PINTERV
C
...
C          THE USER.
C
C PSTIFIB - CORE INTEGRATOR, TAKES SINGLE TIME STEPS TO ADVANCE THE
...
C
C PCOSET  - GENERATES INTEGRATION FORMULA AND ERROR CONTROL COEFFICIENTS.
C
C PRES    - COMPUTES PRESIDUAL VECTORS USED IN SOLVING THE NONLINEAR
C          EQUATIONS BY A MODIFIED NEWTON METHOD.
C
C PDIFFUN - COMPUTES A**-1 * G(T,Y) WHERE A AND G ARE AS ABOVE (STIFIB).
C
C PADDA   - ADDS THE A MATRIX TO A GIVEN MATRIX IN ALMOST BLOCK DIAGONAL
C
C PEVAL   - PEVALUATES THE COMPUTED PIECEWISE POLYNOMIAL SOLUTION AND
C          DERIVATIVES AT COLLOCATION POINTS.
C
C PGFUN   - EVALUATES THE FUNCTION G(T,Y) BY CALLING PEVAL AND THE USER
C          SUBROUTINES F AND PDE1BND.
C
C PSETIB - GENERATES PROPER JACOBIAN MATRICES REQUIRED BY THE MODIFIED
C          NEWTON METHOD.
C
C DIFFF  - PERFORMS SAME ROLE AS THE USER ROUTINE PDE1FJAC. COMPUTES
C          DERIVATIVE APPROXIMATIONS BY USE OF FINITE DIFFERENCES.
C
C PCRDCMP - PERFORM AN LU DECOMPOSTION  OF THE ALMOST BLOCK DIAGONAL
C          LINEAR SYSTEMS ARISING.
C PCRSLVE - PERFORM THE RIGHT-HAND SIDE MODIFICATIONS AND THE BACK-
...
C DESIRED WHEN NCC = 2 (SEE ABOVE AND PCOLPNT).  MAXDER IS SET
...
C DESIRED.  SEE PCOSET FOR RESTRICTIONS.  THESE CHANGES MAY BE MADE BY
...
C
C PACKAGE ROUTINES CALLED..   PEVAL,INITAL,PINTERP,STIFIB
C USER ROUTINES CALLED..      PDE1BND
C CALLED BY..                 USERS MAIN PROGRAM
C FORTRAN FUNCTIONS USED..    ABS,MAX,FLOAT,SQRT
C-----------------------------------------------------------------------
      SUBROUTINE PDECOL(T0, TOUT, DT,
     *                  XBKPT, EPS, NINT, KORD,
     *                  NCC, NPDE, MF, INDEX,
     *                  WORK, IWORK,
     *                  LSTDOUT, STDOUT, MDER, NOG
     *                  )
     ...
     *                  BOTPOS, SIZE
      ...
      COMMON /GEAR1/    T, DTC, DTMN, DTMX, EPSC, UROUND, N, MFC,
     *                  KFLAG, JSTART
      COMMON /GEAR9/    EPSJ, R0, IDUM(6)
      COMMON /OPTION/   NOGAUS, MAXDER
      COMMON /SIZES/    NIN, KOR, NC, NPD, NCPTS, NEQN, IFLAG
      COMMON /ABDPAR/   NROWS, NCLBLK, NOVRLP, NELS, NELTOP, BOTPOS,
     *                  SIZE, KRPT
     ...
     *                  IW11,IW12,IW13,IW14,IW15,IW16,IW17,IW18
      ...
      SAVE
C
C     CONTROL BLOCK FROM THE MAIN CODE
C
      INTEGER       LSTDOUT, MDER
...
C-----------------------------------------------------------------------
c            write (*,*) 'before inital'
      CALL PINITAL(KOR,WORK(IW1),WORK(IW6),XBKPT,WORK(IW2),WORK(IW3),
     *            WORK(IW17),IWORK(IW18),IWORK)
c            write (*,*) 'after inital'
...
C
   90 DTMX = DT
  100 IF ((T+DTC) .EQ. T .AND. LOUT .GT. 0) WRITE(LOUT,110)
  110 FORMAT(36H WARNING..  T + DT = T ON NEXT STEP.)
C-----------------------------------------------------------------------
C TAKE A TIME STEP BY CALLING THE INTEGRATOR.
C-----------------------------------------------------------------------
c            write (*,*) 'before stifib'
      CALL PSTIFIB (NEQN,WORK(IW10),WORK(IW4),WORK(IW5),WORK(IW6),
     *       WORK(IW7),WORK(IW8),WORK(IW17),IWORK(IW18),WORK,IWORK)
c            write (*,*) 'after stifib'
C
      KGO = 1 - KFLAG
...
C-----------------------------------------------------------------------
  ...
  140 IF ((T-TOUT)*DTC .LT. 0.) GO TO 100
      CALL PINTERP(TOUT,WORK(IW10),NEQN,WORK(IW6))
      GO TO 360
C
...
C-----------------------------------------------------------------------
  160 IF (LOUT .GT. 0) WRITE (LOUT,170) T
...
C
  ...
  310 FORMAT(//45H INDEX = -1 ON INPUT WITH (T-TOUT)*DT .GE. 0./
     ...
     *  CLOSE (LOUT)
      RETURN
C
  320 IF (LOUT .GT. 0) WRITE(LOUT,330) IERID
  330 FORMAT(//24H ILLEGAL INPUT...INDEX= ,I3//)
     ...
     *  CLOSE (LOUT)
      RETURN
C
  335 IF (LOUT .GT. 0) WRITE(LOUT,336) IWSTOR,IWSAVE,IISTOR,IISAVE
...
C
  ...
  360 INDEX = KFLAG
     ...
     *  CLOSE (LOUT)
      ...
      SUBROUTINE PADDA(PW,A,BC,NPDE)
*
      IMPLICIT REAL*8(A-H,O-Z)
*
C-----------------------------------------------------------------------
C SUBROUTINE PADDA ADDS ELEMENTS OF THE MATRIX A(Y,T) TO THE MATRIX
...
*
      COMMON  /SIZES/
     *                  NINT, KORD, NCC, NPD, NCPTS, NEQN, IDUM
      COMMON  /ABDPAR/
     *                  NROWS, NCLBLK, NOVRLP, NELS, NELTOP,
     *                  BOTPOS, SIZE, KRPT
...
C
C   NROWS  - THE NUMBER OF ROWS IN EACH BLOCK TO BE PROVIDED TO PCRDCMP.
...
*
      RETURN
      END
      SUBROUTINE PDIFFUN (N, T, Y, YDOT, IFLAG, PW, IPIV, WORK, IWORK)
*
      IMPLICIT REAL*8(A-H,O-Z)
*
C-----------------------------------------------------------------------
C PSTIFIB FINDS THE SOLUTION TO A SYSTEM OF O.D.E.'S OF THE FORM
C A(Y,T) (DY/DT) = G(Y,T). THIS REQUIRES THE CALCULATION OF THE QUANTITY
C A(Y,T)**-1 * G(Y,T). PDIFFUN COMPUTES YDOT = A(Y,T)**-1 * G(Y,T) BY
...
C
C PACKAGE ROUTINES CALLED..  PADDA,DECB,GFUN,SOLB
C USER ROUTINES CALLED..     NONE
C CALLED BY..                PSTIFIB
C FORTRAN FUNCTIONS USED..   NONE
*
...
C
C NROWS  - THE NUMBER OF ROWS IN EACH BLOCK TO BE PROVIDED TO PCRDCMP.
C NCLBLK - THE NUMBER OF COLUMNS IN EACH BLOCK TO BE PROVIDED TO PCRDCMP.
...
C CONDITION INFORMATION IS ALSO UPDATED BY THIS CALL TO PGFUN.
*
      CALL PGFUN (T, Y, YDOT, NPDE, NCPTS, WORK(IW1), WORK, WORK(IW14),
     *           WORK(IW15), WORK(IW16), WORK(IW3), WORK(IW9), IWORK)
*
C THE FOLLOWING CALL TO PGFUN IS EQUVIVALENT TO THE ABOVE CALL WITH THE
C ACTUAL PARAMETER NAMES SUBSTITUED FOR THE WORK VECTOR ADDRESSES.
C
C     CALL PGFUN (T, Y, YDOT, NPDE, NCPTS, A, BC, DBDU,
...
*
      CALL PADDA (PW, WORK(IW1), WORK, NPDE)
*
C THE FOLLOWING CALL TO PADDA IS EQUVIVALENT TO THE ABOVE CALL WITH THE
C ACTUAL PARAMETER NAMES SUBSTITUED FOR THE WORK VECTOR ADDRESSES.
C
C     CALL PADDA (PW, A, BC, NPDE)
*
...
*
 1021 CONTINUE
      RETURN
      END
      SUBROUTINE PINITAL(K,A,RHS,X,XT,XC,PW,IPIV,ILEFT)
*
      IMPLICIT REAL*8(A-H,O-Z)
*
C-----------------------------------------------------------------------
C PINITAL IS CALLED ONLY ONCE BY PDECOL TO PERFORM INITIALIZATION TASKS.
C THESE TASKS INCLUDE - 1) DEFINING THE PIECEWISE POLYNOMIAL SPACE
C BREAKPOINT SEQUENCE, 2) CALLING THE SUBROUTINE PCOLPNT TO DEFINE THE
...
C         OF THE K-TH NON-ZERO BASIS FUNCTION (K=1,...,KORD) AT THE I-TH
C         COLLOCATION POINT.
C RHS   - IN THIS ROUTINE THE RIGHT HAND SIDE OF THE PINITAL CONDITIONS
...
C PW    - IN PINITAL, THIS VECTOR, WHICH IS OF DIMENSION SIZE, WILL BE
...
C
C PACKAGE ROUTINES CALLED..  PBSPLVD,PCOLPNT,PCRDCMP,INTERV,PCRSLVE
C USER ROUTINES CALLED..     PDE1UINT
C CALLED BY..                PDECOL
C FORTRAN FUNCTIONS USED..   MAX0,MIN0

      DIMENSION
     *                  A(K,3,1), RHS(1), X(1), XT(1), XC(1),
     *                  PW(1), IPIV(1), ILEFT(1)
...
*
      MFLAG = -2
       CALL PINTERV(XT,NCPTS,XC(1),ILEFT(1),MFLAG)
*
C     HERE PINTERV COMPUTES THE VALUE OF ILEFT(1). MFLAG IS INITIALIZED T
C     -2 TO INSTRUCT PINTERV TO BEGIN ITS SEARCH FOR THE APPROPRIATE ILEF
C     WITH THE VALUE 1.
*
       CALL BSPLVD(XT,KORD,XC(1),ILEFT(1),A(1,1,1),3)
C
C     PBSPLVD IS CALLED TO COMPUTE THE COMPONENTS OF A(K,I,J) ASSOCIATED
C     THE FIRST COLLOCATION POINT.
C
       CALL PDE1UINT(XC(1),RHS(1),NPDE)
C
C     PDE1UINT IS CALLED TO EVALUATE THE FIRST NPDE COMPONENTS OF THE RHS V
...
C         RIGHT HAND SIDE OF THE PINITAL CONDITIONS, EVALUATED AT THE ICC
C         COLLOCATION POINT ARE STORED.
*
C         COMPUTE INFORMATION FOR ICC-TH COLLOCATION POINT
*
           CALL PINTERV(XT,NCPTS,XC(ICC),ILEFT(ICC),MFLAG)
           CALL BSPLVD(XT,KORD,XC(ICC),ILEFT(ICC),A(1,1,ICC),3)
           CALL PDE1UINT(XC(ICC),RHS(IN+1),NPDE)
*
...
*
       CALL PINTERV(XT,NCPTS,XC(NCPTS),ILEFT(NCPTS),MFLAG)
       CALL BSPLVD(XT,KORD,XC(NCPTS),ILEFT(NCPTS),A(1,1,NCPTS),3)
      IN = NEQN - NPDE +1
       CALL PDE1UINT(XC(NCPTS),RHS(IN),NPDE)
*
      DO 3040 N = 1,NPDE
*
...
*
C-----------------------------------------------------------------------
C PSTIFIB IS USED TO SOLVE A SYSTEM OF ODE'S OF THE FORM A(C,T)*DC/DT = G
...
C WHERE THE CONSTANT, CON, IS DEFINED BELOW. PSETIB IS CALLED BY PSTIFIB
...
C         JACOBIAN WILL BE PROVIDED THROUGH THE ROUTINE PDE1FJAC;
...
C NPDE - NUMBER OF PARTIAL DIFFERENTIAL EQUATIONS
C
C PACKAGE ROUTINES CALLED..  PADDA,PCRDCMP,DIFFF,EVAL,GFUN
C USER ROUTINES CALLED..     PDE1BND,PDE1FJAC
C CALLED BY..                PSTIFIB
...
C H      - THE CURRENT STEPSIZE BEING USED IN THE PSTIFIB ROUTINE.
C UROUND - THE UNIT ROUND-OFF OF THE MACHINE.
C N      - THE NUMBER OF FIRST ORDER DIFFERENTIAL EQUATIONS
C          IN THE SYSTEMS TO BE SOLVED BY PSTIFIB.
C
...
C     MITER IS NOT EQUAL TO 1), THEN, BY CALLING PGFUN, COMPUTE
...
*
      CALL PGFUN (T, C, SAVE2, NPDE, NCPTS,A,BC,DBDU,DBDUX,DZDT,XC,
     *           UVAL,ILEFT)
*
...
C         OF THE I-TH SUBINTERVAL
*
          CALL PEVAL(ICC,NPDE,C,UVAL,A,ILEFT)
*
C         PEVAL RETURNS AN EVALUATION OF THE SOLUTION COMPONENTS AND THEI
C         FIRST AND SECOND DERIVATIVES IN THE MATRIX UVAL, WITH THE PEVAL
C         UATIONS BEING DONE AT THE ICC-TH COLLOCATION POINT.
*
          IF ( MITER .EQ. 1 )
     *       CALL PDE1FJAC(T,XC(ICC),UVAL,UVAL(1,2),UVAL(1,3),
     *                              DFDU,DFDUX,DFDUXX,NPDE)
*
C         PDE1FJAC RETURNS AN EVALUATION OF THE JACOBIAN OF F AS WELL
...
*
C          IF ( MITER .EQ. 2 )
C     *       CALL DIFFF(T,XC(ICC),ICC,UVAL,UVAL(1,2),UVAL(1,3),
C     *                     DFDU,DFDUX,DFDUXX,NPDE,CMAX,SAVE2)
*
...
*
      RETURN
      END
      SUBROUTINE PSTIFIB (N0,Y,YMAX,ERROR,SAVE1,SAVE2,SAVE3,
     *                   PW,IPIV,WORK,IWORK)
      IMPLICIT REAL*8(A-H,O-Z)
C-----------------------------------------------------------------------
C PSTIFIB PERFORMS ONE STEP OF THE INTEGRATION OF AN INITIAL VALUE
C PROBLEM FOR A SYSTEM OF ORDINARY DIFFERENTIAL EQUATIONS OF THE FORM,
C   A(Y,T)*(DY/DT) = G(Y,T),   WHERE   Y = (Y(1),Y(2), ... ,Y(N)).
C PSTIFIB IS FOR USE WHEN THE MATRICES A AND DG/DY HAVE AN ALMOST BLOCK
C DIAGONAL FORM.  THE DEPENDENCE OF A(Y,T) ON Y IS ASSUMED TO BE WEAK.
C
C---------------------------------------------------------------------
C THE MODIFICATION TO THE ORIGINAL PSTIFIB PROGRAM CONSISTS ENTIRELY OF T
C REMOVAL OF THE BAND LINEAR EQUATION SOLVER, SOLB, AND ITS SUBSTITUTION
C BY PCRSLVE, THE ALMOST BLOCK DIAGONAL LINEAR EQUATION SOLVER. THE CHANG
C IN THE PSTIFIB ROUTINE ARE LIMITED TO:
C
C       (1) REPLACEMENT OF THE CALL TO SOLV BY A CALL TO PCRSLVE
C       (2) THE REMOVAL OF SOME VARIABLES USED SOLELY TO PROVIDE
C           INFORMATION TO SOLB AND THE INSERTION OF NEW VARIABLES
C           TO PASS INFORMATION TO PCRSLVE.
C-----------------------------------------------------------------------
C
C REFERENCE
C
...
C     1976.
C
C COMMUNICATION WITH PSTIFIB IS DONE WITH THE FOLLOWING VARIABLES..
C
...
C             IS THE MAXIMUM ORDER AVAILABLE.  SEE SUBROUTINE PCOSET.
...
C                  PDECOL.
C
C PACKAGE ROUTINES CALLED..  PCOSET,DIFFUN,PSETIB,RES,PCRSLVE
C USER ROUTINES CALLED..     NONE
C CALLED BY..                PDECOL
C FORTRAN FUNCTIONS USED..   ABS,MAX,MIN,FLOAT
C-----------------------------------------------------------------------
      DIMENSION
     *                  Y(N0,*), YMAX(N0), ERROR(N0), SAVE1(N0),
     *                  SAVE2(N0), SAVE3(N0), PW(1), IPIV(1),
     *                  WORK(1), IWORK(1)
*
      COMMON /SIZES/    NINT, KORD, NCC, NPDE, NCPTS, NEQN, IQUAD
      COMMON /ISTART/   IW1, IW2, IW3, IW4, IW5, IW6, IW7, IW8, IW9,
     *                  IW10, IW11, IW12,IW13, IW14, IW15, IW16,
     *                  IW17,IW18
...
C-----------------------------------------------------------------------
      NQ = 1
      IER = 0
      CALL PDIFFUN (N, T, Y, SAVE1, IER, PW, IPIV, WORK, IWORK)
  ...
  110   Y(I,2) = H*SAVE1(I)
      ...
      GO TO 130
C-----------------------------------------------------------------------
C IF THE CALLER HAS CHANGED METH, PCOSET IS CALLED TO SET
...
C-----------------------------------------------------------------------
  ...
  230   ERROR(I) = 0.
      M = 0
      CALL PRES (T, H, Y, SAVE2, SAVE3, NPDE, NCPTS, WORK(IW1),
     *           IWORK, WORK, WORK(IW14), WORK(IW15), WORK(IW16),
     *           WORK(IW3), WORK(IW9))
      NFE = NFE + 1
...
C-----------------------------------------------------------------------
  400 IF (M .NE. 0) CRATE = MAX(.9*CRATE,D/D1)
      ...
      M = M + 1
      IF (M .EQ. 3) GO TO 410
      CALL PRES(T, H, SAVE1, SAVE2, SAVE3, NPDE, NCPTS, WORK(IW1),
     *          IWORK, WORK, WORK(IW14), WORK(IW15), WORK(IW16),
     *          WORK(IW3), WORK(IW9))
      GO TO 350
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
  ...
  700 HOLD = H
      ...
      RETURN
      END
        SUBROUTINE PCRDCMP(N,TOPBLK,NRWTOP,NOVRLP,ARRAY,NRWBLK,
...
1000    CONTINUE
        ...
        RETURN
        END
        SUBROUTINE PCRSLVE(TOPBLK,NRWTOP,NOVRLP,ARRAY,NRWBLK,
...
C      NUMER. ANAL., VOL. 14, NO. 3, JUNE 1977, PP. 441-472.
C
C PACKAGE ROUTINES CALLED..  PBSPLVN
C USER ROUTINES CALLED..     NONE
C CALLED BY..                PCOLPNT,INITAL,VALUES
...
C-----------------------------------------------------------------------
   ...
   40 RETURN
      END
      SUBROUTINE PINTERV ( XT, LXT, X, ILEFT, MFLAG )
      IMPLICIT REAL*8(A-H,O-Z)
C-----------------------------------------------------------------------
C THIS SUBROUTINE IS PART OF THE B-SPLINE PACKAGE FOR THE STABLE
C EVALUATION OF ANY B-SPLINE BASIS FUNCTION OR DERIVATIVE VALUE.
...
C-----------------------------------------------------------------------
   ...
   45 IF (X .GE. XT(LXT)) GO TO 110
      IHI = LXT
C-----------------------------------------------------------------------
C NOW XT(ILO) .LE. X .LT. XT(IHI).  NARROW THE PINTERVAL.
C-----------------------------------------------------------------------
   50 MIDDLE = (ILO + IHI)/2
...
C-----------------------------------------------------------------------
  ...
  110 MFLAG = 1
      ...
      RETURN
      END
      SUBROUTINE PCOLPNT(X, XC, XT)
      IMPLICIT REAL*8(A-H,O-Z)
C-----------------------------------------------------------------------
C PCOLPNT IS CALLED ONLY ONCE BY PINITAL TO DEFINE THE REQUIRED COLLOCA-
...
C
C PACKAGE ROUTINES CALLED..  PBSPLVD,INTERV
C USER ROUTINES CALLED..     NONE
C CALLED BY..                PINITAL
C FORTRAN FUNCTIONS USED..   NONE
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
  ...
  210   XNEW = .5 * (XL + XR)
        ...
        CALL PINTERV(XT,NCPTS,XNEW,ILEFT,MFLAG)
  ...
  240 XC(I) = XR
      RETURN
      END
      SUBROUTINE PCOSET (METH, NQ, EL, TQ)
      IMPLICIT REAL*8(A-H,O-Z)
C-----------------------------------------------------------------------
C PCOSET IS CALLED BY THE INTEGRATOR AND SETS COEFFICIENTS USED THERE.
...
C LMAX = MAXDER + 1 IS THE NUMBER OF COLUMNS IN THE Y ARRAY (SEE PSTIFIB
...
c C-----------------------------------------------------------------------
c C SUBROUTINE DIFFF IS USED IF MITER=2 TO PROVIDE FINITE DIFFERENCE
c C APPROXIMATIONS FOR THE PARTIAL DERIVATIVES OF THE K-TH USER DEFINED
c C FUNCTION IN THE F ROUTINE WITH RESPECT TO THE VARIABLES U, UX, AND
c C UXX.  THESE PARTIALS WITH RESPECT TO U, UX, AND UXX ARE COMPUTED,
c C STORED, AND RETURNED IN THE NPDE BY NPDE ARRAYS DFDU, DFDUX, AND
c C DFDUXX, RESPECTIVELY.
c C
c C PACKAGE ROUTINES CALLED..  NONE
c C USER ROUTINES CALLED..     F
c C CALLED BY..                PSETIB
c C FORTRAN FUNCTIONS USED..   MAX
c C-----------------------------------------------------------------------
c       DIMENSION U(NPDE),UX(NPDE),UXX(NPDE),DFDU(NPDE,NPDE),
c      *   DFDUX(NPDE,NPDE),DFDUXX(NPDE,NPDE),CMAX(1),SAVE2(1)
c       COMMON /GEAR9/ EPSJ,R0,ML,MU,MW,NM1,N0ML,N0W
c       ID = (IPT-1) * NPDE
c       DO 40 J=1,NPDE
c         UJ = U(J)
c         R = EPSJ * CMAX(J)
...
c         U(J) = U(J) + R
c         RINV = 1. / R
c         CALL PDE1F(T,X,U,UX,UXX,DFDU(1,J),NPDE)
c         DO 10 I=1,NPDE
c    10     DFDU(I,J) = ( DFDU(I,J) - SAVE2(I+ID) ) * RINV
c         U(J) = UJ
c         UJ = UX(J)
c         UX(J) = UX(J) + R
c         CALL PDE1F(T,X,U,UX,UXX,DFDUX(1,J),NPDE)
c         DO 20 I=1,NPDE
c    20     DFDUX(I,J) = ( DFDUX(I,J) - SAVE2(I+ID) ) * RINV
c         UX(J) = UJ
c         UJ = UXX(J)
c         UXX(J) = UXX(J) + R
c         CALL PDE1F(T,X,U,UX,UXX,DFDUXX(1,J),NPDE)
c         DO 30 I=1,NPDE
c    30     DFDUXX(I,J) = ( DFDUXX(I,J) - SAVE2(I+ID) ) * RINV
c         UXX(J) = UJ
c    40 CONTINUE
c       RETURN
c       END
      SUBROUTINE PEVAL(ICPT,NPDE,C,UVAL,A,ILEFT)
      IMPLICIT REAL*8(A-H,O-Z)
C-----------------------------------------------------------------------
C SUBROUTINE PEVAL EVALUATES U(K), UX(K), AND UXX(K), K=1 TO NPDE,
...
C PACKAGE ROUTINES CALLED..  NONE
C USER ROUTINES CALLED..     NONE
C CALLED BY..                PGFUN,PDECOL,PSETIB
C FORTRAN FUNCTIONS USED..   NONE
C-----------------------------------------------------------------------
   ...
   10 CONTINUE
      RETURN
      END
      SUBROUTINE PGFUN (T,C,UDOT,NPDE,NCPTS,A,BC,DBDU,DBDUX,DZDT,
     *                 XC,UVAL,ILEFT)
      IMPLICIT REAL*8(A-H,O-Z)
C-----------------------------------------------------------------------
C SUBROUTINE PGFUN COMPUTES THE FUNCTION G(C,T), THE RIGHT-
...
C
C PACKAGE ROUTINES CALLED..  PEVAL
C USER ROUTINES CALLED..     PDE1BND,F
C CALLED BY..                PDIFFUN,PSETIB,RES
C FORTRAN FUNCTIONS USED..   NONE
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      CALL PEVAL(1,NPDE,C,UVAL,A,ILEFT)
      CALL PDE1BND(T,XC(1),UVAL,UVAL(1,2),DBDU,DBDUX,DZDT,NPDE)
      CALL PDE1F(T,XC(1),UVAL,UVAL(1,2),UVAL(1,3),UDOT,NPDE)
      ILIM = KORD + 2
      DO 30 K=1,NPDE
        BC(K,K,1) = 1.
...
C-----------------------------------------------------------------------
      ILIM = NCPTS - 1
      DO 40 I=2,ILIM
        CALL PEVAL(I,NPDE,C,UVAL,A,ILEFT)
        CALL PDE1F(T,XC(I),UVAL,UVAL(1,2),UVAL(1,3),UDOT(1,I),NPDE)
   40 CONTINUE
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      CALL PEVAL(NCPTS,NPDE,C,UVAL,A,ILEFT)
      CALL PDE1F(T, XC(NCPTS), UVAL, UVAL(1,2), UVAL(1,3),
     *           UDOT(1,NCPTS), NPDE)
      CALL PDE1BND(T,XC(NCPTS),UVAL,UVAL(1,2),DBDU,DBDUX,DZDT,NPDE)
   ...
   60 CONTINUE
      RETURN
      END
      SUBROUTINE PINTERP (TOUT, Y, N0, Y0)
      IMPLICIT REAL*8(A-H,O-Z)
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
   ...
   30 CONTINUE
      RETURN
      END
      SUBROUTINE PRES(T,H,C,V,R,NPDE,NCPTS,A,ILEFT,BC,DBDU,DBDUX,DZDT,
     *               XC,UVAL)
      IMPLICIT REAL*8(A-H,O-Z)
C-----------------------------------------------------------------------
C SUBROUTINE PRES COMPUTES THE RESIDUAL VECTOR R = H*G(C,T) - A(C,T)*V
C WHERE H IS THE CURRENT TIME STEP SIZE, G IS A VECTOR, A IS A
C MATRIX, V IS A VECTOR, AND T IS THE CURRENT TIME.
C
C PACKAGE ROUTINES CALLED..  PGFUN
C USER ROUTINES CALLED..     NONE
C CALLED BY..                PSTIFIB
C FORTRAN FUNCTIONS USED..   NONE
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      CALL PGFUN(T,C,R,NPDE,NCPTS,A,BC,DBDU,DBDUX,DZDT,XC,UVAL,ILEFT)
C-----------------------------------------------------------------------
...
c       IMPLICIT REAL*8(A-H,O-Z)
c C-----------------------------------------------------------------------
c C SUBROUTINE VALUES COMPUTES THE SOLUTION U AND THE FIRST NDERV
c C DERIVATIVES OF U AT THE NPTS POINTS X AND AT TIME TOUT AND RETURNS
c C THEM IN THE ARRAY USOL.  THIS ROUTINE MUST BE USED TO OBTAIN
c C SOLUTION VALUES SINCE PDECOL DOES NOT RETURN ANY SOLUTION VALUES
c C TO THE USER.  SEE PDECOL.
c C
c C THE CALLING PARAMETERS ARE...
c C   X     =  AN ARBITRARY VECTOR OF SPATIAL POINTS OF LENGTH NPTS AT
c C            WHICH THE SOLUTION AND THE FIRST NDERV DERIVATIVE VALUES
c C            ARE TO BE CALCULATED.  IF X .LT. XLEFT ( X .GT. XRIGHT )
c C            THEN THE PIECEWISE POLYNOMIAL OVER THE LEFTMOST ( RIGHT-
c C            MOST ) INTERVAL IS EVALUATED TO CALCULATE THE SOLUTION
c C            VALUES AT THIS UNUSUAL VALUE OF  X.  SEE PDECOL.
c C
c C   USOL  =  AN ARRAY WHICH CONTAINS THE SOLUTION AND THE FIRST
c C            NDERV DERIVATIVES OF THE SOLUTION AT ALL THE POINTS IN
c C            THE INPUT VECTOR X.  IN PARTICULAR, USOL(J,I,K) CONTAINS
c C            THE VALUE OF THE (K-1)-ST DERIVATIVE OF THE J-TH PDE
c C            COMPONENT AT THE I-TH POINT OF THE X VECTOR FOR
c C            J = 1 TO NPDE, I = 1 TO NPTS, AND K = 1 TO NDERV+1.
c C
c C   SCTCH =  A USER SUPPLIED WORKING STORAGE ARRAY OF LENGTH AT LEAST
c C            KORD*(NDERV+1).  SEE BELOW AND PDECOL FOR DEFINITIONS OF
c C            THESE PARAMETERS.
c C
c C   NDIM1 =  THE FIRST DIMENSION OF THE OUTPUT ARRAY USOL IN THE CALLING
c C            PROGRAM.  NDIM1 MUST BE .GE. NPDE.
c C
c C   NDIM2 =  THE SECOND DIMENSION OF THE OUTPUT ARRAY USOL IN THE
c C            CALLING PROGRAM.  NDIM2 MUST BE .GE. NPTS.
c C
c C   NPTS  =  THE NUMBER OF POINTS IN THE X VECTOR.
c C
c C   NDERV =  THE NUMBER OF DERIVATIVE VALUES OF THE SOLUTION THAT ARE
c C            TO BE CALCULATED.  NDERV SHOULD BE LESS THAN KORD SINCE
c C            THE KORD-TH DERIVATIVE OF A POLYNOMIAL OF DEGREE KORD-1
c C            IS EQUAL TO ZERO.  SEE PDECOL.
c C
c C   WORK  =  THE WORKING STORAGE ARRAY DESCRIBED IN PDECOL.
c C
c C PACKAGE ROUTINES CALLED..  PBSPLVD,INTERV
c C USER ROUTINES CALLED..     NONE
c C CALLED BY..                USERS MAIN PROGRAM
c C FORTRAN FUNCTIONS USED..   NONE
c C
c C-----------------------------------------------------------------------
c       DIMENSION USOL(NDIM1,NDIM2,NDERV),X(NPTS),SCTCH(1),WORK(1)
c       COMMON /SIZES/ NINT,KORD,NCC,NPDE,NCPTS,NEQN,IQUAD
c       COMMON /ISTART/ IW1,IW2,IW3,IW4,IW5,IW6,IDUM(12)
c       DATA ILEFT/0/, MFLAG/0/
c       NDERV1 = NDERV + 1
c       DO 20 IPTS=1,NPTS
c         CALL PINTERV(WORK(IW2),NCPTS,X(IPTS),ILEFT,MFLAG)
c         CALL BSPLVD(WORK(IW2),KORD,X(IPTS),ILEFT,SCTCH,NDERV1)
c         IK = ILEFT - KORD
c         DO 10 M=1,NDERV1
c           I1 = (M-1)*KORD
c           DO 10 K=1,NPDE
c             USOL(K,IPTS,M) = 0.
c             DO 10 I=1,KORD
c               I2 = (I+IK-1)*NPDE + IW6 - 1
c               USOL(K,IPTS,M) = USOL(K,IPTS,M) + WORK(I2+K) * SCTCH(I+I1)
c    10   CONTINUE
c    20 CONTINUE
c       RETURN
c       END
      SUBROUTINE PDE1VAL(X,USOL,SCTCH,NDIM1,NDIM2,NPTS,WORK)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
C-----------------------------------------------------------------------
C SUBROUTINE PDE1VAL COMPUTES THE SOLUTION U AND THE FIRST NDERV
...
C-----------------------------------------------------------------------
      DOUBLE PRECISION
     *                  USOL(NDIM2,NDIM1), X(NPTS), SCTCH(1),
     *                  WORK(1)
     ...
     *                  IDUM(12)
      ...
      COMMON /SIZES/    NINT, KORD, NCC, NPDE, NCPTS, NEQN, IQUAD
      COMMON /ISTART/   IW1, IW2, IW3, IW4, IW5, IW6, IDUM
     ...
     *                  MFLAG/0/
      ...
      NDERV1 = 1
      DO 20 IPTS=1,NPTS
        CALL PINTERV(WORK(IW2),NCPTS,X(IPTS),ILEFT,MFLAG)
        ...
        DO 10 K=1,NPDE
          ...
          DO 10 I=1,KORD
            ...
            USOL(IPTS,K) = USOL(IPTS,K) + WORK(I2+K) * SCTCH(I)
   10   CONTINUE
   20 CONTINUE
      RETURN
      END
