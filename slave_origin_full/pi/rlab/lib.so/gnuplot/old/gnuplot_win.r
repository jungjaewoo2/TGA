...
// ----------------------------------------
// $Header: /home/ians/dev/CVS/rlab/rlab2/misc/gnuplot_win.r,v 1.1 2000/03/25 22:14:43 ians Exp $
//-------------------------------------------------------------------//
...
static(PLOTPRG)
PLOTPRG = "|e:/Applications/gp371w32/pgnuplot.exe"

// To be able to distinguish different RLaB2 processes, use the date
static (randprocnr);
randprocnr = getline("|date /t").[1];
...
copyplot = function ( I1, I2 )
{
  ...
  fprintf (p.[I1].prog, saveCmd);
  loadCmd = "load '" + saveName + "'\n"
  fprintf (p.[I2].prog, loadCmd);

  system("del /Q /F " + saveName);
};
...
cont = function ( datax, datay, datazz, keynames, N )
{
    ...
    // Check the existence of p.[N]
    if (max(size(N)) != 1) { error ("cont(): N must be a 1-by-1"); }
    if (exist(p.[N])) {
	pobj_Reset (N);
	else
	pobj_Create (N);
   ...
   if (p.[N].mpp.nr != 0) {
	...
	// Time to plot
	if (class(datax) != "num") {
	    error ("Invalid data class for cont() in x-vector (must be num)");
	    else if (class(datay) != "num") {
		error ("Invalid data class for cont() in y-vector (must be num)");
		else if (class(datazz) != "num") {
		    error ("Invalid data class for cont() in x-matrix (must be num)");
		    else
                    pview(0, 0, 1);
	...
	error ("Invalid data for cont(), bad x/y-range in z-matrix");
    }
...
psplot = function ( file, mode, N )
{
  ...
  if (!exist(mode)) { mode = "default enh"; }

  // Make hardcopy file
  if (class(mode) == "num") { mode = "default enh"; }
...
epsplot = function ( file, mode, N )
{
  ...
  fprintf (p.[N].prog, "set term post epsf %s\n", mode);
  fprintf (p.[N].prog, "set output \"%s\"\n", file);
  fprintf (p.[N].prog, "replot\n");
...
pset = function ( string, N )
{
    ...
    // Check the existence of p.[N]
    if (!exist(p.[N])) { pobj_Create (N); }
    if (!exist(string)){ error("String missing!");}
    if (class(string) != "string"){ error("String must be a string!");}
...
// They could be done with string("set key") etc, but this is more convenient.
//
...
pobj_TmpFileName = function ( N, i, j )
{
    ...
    // Get the temporary filename from the OS
    tmp = tmpnam();

    // Add the name to a list so we can be sure to
...
pobj_Rm = function ( N )
{
  if (length(p.[N].files) != 0) {
    // Create del command
    delcmd = "del /Q /F " + p.[N].files;
    // Have the OS do the work.
    system(delcmd);
  }
};
